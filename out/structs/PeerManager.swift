...
extension Bindings {
	...
	public class PeerManager: NativeTypeWrapper {
		...
		internal init(
			cType: LDKPeerManager, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		@available(
			*, deprecated,
			message: "This method passes the following non-cloneable, but freeable objects by value: `messageHandler`."
		)
		public init(
			...
			let nativeCallResult =
				withUnsafePointer(to: tupledEphemeralRandomData) {
			...
			self.instanceNumber = Self.instanceCounter
			super
				.init(conflictAvoidingVariableName: 0, instantiationContext: "PeerManager.swift::\(#function):\(#line)")
		...
		public func newOutboundConnection(
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (thisArgPointer: UnsafePointer<LDKPeerManager>) in
					PeerManager_new_outbound_connection(
						thisArgPointer, theirNodeIdPrimitiveWrapper.cType!, descriptor.activate().cType!,
						remoteNetworkAddressOption.cType!)
				}
			...
			// return value (do some wrapping)
			let returnValue = Result_CVec_u8ZPeerHandleErrorZ(
				cType: nativeCallResult, instantiationContext: "PeerManager.swift::\(#function):\(#line)", anchor: self
			)
			.dangle(false)
			...
			-> Result_NonePeerHandleErrorZ
		{
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (thisArgPointer: UnsafePointer<LDKPeerManager>) in
					PeerManager_new_inbound_connection(
						thisArgPointer, descriptor.activate().cType!, remoteNetworkAddressOption.cType!)
				}
			...
			// return value (do some wrapping)
			let returnValue = Result_NonePeerHandleErrorZ(
				cType: nativeCallResult, instantiationContext: "PeerManager.swift::\(#function):\(#line)", anchor: self
			)
			.dangle(false)
		...
		public func writeBufferSpaceAvail(descriptor: SocketDescriptor) -> Result_NonePeerHandleErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (thisArgPointer: UnsafePointer<LDKPeerManager>) in
					...
					withUnsafeMutablePointer(to: &descriptor.activate().cType!) {
						(descriptorPointer: UnsafeMutablePointer<LDKSocketDescriptor>) in
		...
		public func readEvent(peerDescriptor: SocketDescriptor, data: [UInt8]) -> Result_boolPeerHandleErrorZ {
			...
			let dataPrimitiveWrapper = u8slice(
				value: data, instantiationContext: "PeerManager.swift::\(#function):\(#line)")
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (thisArgPointer: UnsafePointer<LDKPeerManager>) in
					...
					withUnsafeMutablePointer(to: &peerDescriptor.activate().cType!) {
						(peerDescriptorPointer: UnsafeMutablePointer<LDKSocketDescriptor>) in
		...
		public func socketDisconnected(descriptor: SocketDescriptor) {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (thisArgPointer: UnsafePointer<LDKPeerManager>) in
					...
					withUnsafePointer(to: descriptor.activate().cType!) {
						(descriptorPointer: UnsafePointer<LDKSocketDescriptor>) in
		...
		public func disconnectByNodeId(nodeId: [UInt8]) {
			...
			let nodeIdPrimitiveWrapper = PublicKey(
				value: nodeId, instantiationContext: "PeerManager.swift::\(#function):\(#line)")
		...
		public func broadcastNodeAnnouncement(rgb: [UInt8], alias: [UInt8], addresses: [NetAddress]) {
			...
			let rgbPrimitiveWrapper = ThreeBytes(
				value: rgb, instantiationContext: "PeerManager.swift::\(#function):\(#line)")
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (thisArgPointer: UnsafePointer<LDKPeerManager>) in
					PeerManager_broadcast_node_announcement(
						thisArgPointer, rgbPrimitiveWrapper.cType!, aliasPrimitiveWrapper.cType!, addressesVector.cType!
					)
				}
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing PeerManager \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
