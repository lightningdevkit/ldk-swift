			...
			extension Bindings {
				...
				public class AnchorDescriptor: NativeTypeWrapper {
					...
					internal init(cType: LDKAnchorDescriptor, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				AnchorDescriptor_get_channel_derivation_parameters(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = ChannelDerivationParameters(cType: nativeCallResult, instantiationContext: "AnchorDescriptor.swift::\(#function):\(#line)", anchor: self).dangle(false)
					...
					public func setChannelDerivationParameters(val: ChannelDerivationParameters) {
						...
						// native method call
						let nativeCallResult = 
						withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKAnchorDescriptor>) in
				AnchorDescriptor_set_channel_derivation_parameters(thisPtrPointer, val.dynamicallyDangledClone().cType!)
						}
				...
				AnchorDescriptor_get_outpoint(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = OutPoint(cType: nativeCallResult, instantiationContext: "AnchorDescriptor.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				AnchorDescriptor_set_outpoint(thisPtrPointer, val.dynamicallyDangledClone().cType!)
					...
					public init(channelDerivationParametersArg: ChannelDerivationParameters, outpointArg: OutPoint) {
						...
						// native method call
						let nativeCallResult = AnchorDescriptor_new(channelDerivationParametersArg.dynamicallyDangledClone().cType!, outpointArg.dynamicallyDangledClone().cType!)
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "AnchorDescriptor.swift::\(#function):\(#line)")
				...
				AnchorDescriptor_previous_utxo(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = TxOut(cType: nativeCallResult, instantiationContext: "AnchorDescriptor.swift::\(#function):\(#line)", anchor: self)
				...
				AnchorDescriptor_unsigned_tx_input(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = TxIn(cType: nativeCallResult, instantiationContext: "AnchorDescriptor.swift::\(#function):\(#line)", anchor: self)
				...
				AnchorDescriptor_witness_script(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "AnchorDescriptor.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public func txInputWitness(signature: [UInt8]) -> [UInt8] {
						...
						let signaturePrimitiveWrapper = ECDSASignature(value: signature, instantiationContext: "AnchorDescriptor.swift::\(#function):\(#line)")
				...
				AnchorDescriptor_tx_input_witness(thisArgPointer, signaturePrimitiveWrapper.cType!)
						...
						// return value (do some wrapping)
						let returnValue = Witness(cType: nativeCallResult, instantiationContext: "AnchorDescriptor.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
				...
				AnchorDescriptor_derive_channel_signer(thisArgPointer, signerProviderPointer)
						...
						// return value (do some wrapping)
						let returnValue = NativelyImplementedWriteableEcdsaChannelSigner(cType: nativeCallResult, instantiationContext: "AnchorDescriptor.swift::\(#function):\(#line)", anchor: self)
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing AnchorDescriptor \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing AnchorDescriptor \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
