			...
			extension Bindings {
				...
				public class Bolt12InvoiceFeatures: NativeTypeWrapper {
					...
					internal init(cType: LDKBolt12InvoiceFeatures) {
					...
					internal init(cType: LDKBolt12InvoiceFeatures, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				Bolt12InvoiceFeatures_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Bolt12InvoiceFeatures(cType: nativeCallResult)
