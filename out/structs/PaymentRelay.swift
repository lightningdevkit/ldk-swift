			...
			extension Bindings {
				...
				public class PaymentRelay: NativeTypeWrapper {
					...
					internal init(cType: LDKPaymentRelay, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "PaymentRelay.swift::\(#function):\(#line)")
				...
				PaymentRelay_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "PaymentRelay.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public class func read(ser: [UInt8]) -> Result_PaymentRelayDecodeErrorZ {
						...
						let serPrimitiveWrapper = u8slice(value: ser, instantiationContext: "PaymentRelay.swift::\(#function):\(#line)")
						...
						// return value (do some wrapping)
						let returnValue = Result_PaymentRelayDecodeErrorZ(cType: nativeCallResult, instantiationContext: "PaymentRelay.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing PaymentRelay \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing PaymentRelay \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
