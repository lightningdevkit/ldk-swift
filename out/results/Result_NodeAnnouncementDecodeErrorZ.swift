			...
			extension Bindings {
				...
				public class Result_NodeAnnouncementDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_NodeAnnouncementDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_NodeAnnouncementDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> NodeAnnouncement? {
						if self.cType?.result_ok == true {
							return NodeAnnouncement(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
