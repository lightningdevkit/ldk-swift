...
extension Bindings {
	...
	public class Result_OnionMessageDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_OnionMessageDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: OnionMessage) -> Result_OnionMessageDecodeErrorZ {
			...
			let returnValue = Result_OnionMessageDecodeErrorZ(
				...
				instantiationContext: "Result_OnionMessageDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_OnionMessageDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_OnionMessageDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_OnionMessageDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_OnionMessageDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_OnionMessageDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_OnionMessageDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
