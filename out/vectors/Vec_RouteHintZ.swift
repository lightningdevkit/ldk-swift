...
extension Bindings {
	...
	internal class Vec_RouteHintZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_RouteHintZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getValue() -> [RouteHint] {
			...
			let swiftArray = array.map { (currentCType: LDKRouteHint) -> RouteHint in
				RouteHint(
					cType: currentCType, instantiationContext: "Vec_RouteHintZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		internal func dangleRecursively() -> Vec_RouteHintZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_RouteHintZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Vec_RouteHintZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
