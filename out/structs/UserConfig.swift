...
extension Bindings {
	...
	public class UserConfig: NativeTypeWrapper {
		...
		internal init(
			cType: LDKUserConfig, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getChannelHandshakeConfig() -> ChannelHandshakeConfig {
			...
			let returnValue = ChannelHandshakeConfig(
				cType: nativeCallResult, instantiationContext: "UserConfig.swift::\(#function):\(#line)", anchor: self
			)
			.dangle(false)
		...
		public init(
			...
			// native method call
			let nativeCallResult = UserConfig_new(
		...
		internal func clone() -> UserConfig {
			...
			let returnValue = UserConfig(
				cType: nativeCallResult, instantiationContext: "UserConfig.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> UserConfig {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing UserConfig \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
