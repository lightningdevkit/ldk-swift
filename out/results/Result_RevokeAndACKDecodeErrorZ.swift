			...
			extension Bindings {
				...
				public class Result_RevokeAndACKDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_RevokeAndACKDecodeErrorZ) {
					...
					internal init(cType: LDKCResult_RevokeAndACKDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_RevokeAndACKDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> RevokeAndACK? {
						if self.cType?.result_ok == true {
							return RevokeAndACK(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
