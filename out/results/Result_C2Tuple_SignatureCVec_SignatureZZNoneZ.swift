...
extension Bindings {
	...
	public class Result_C2Tuple_SignatureCVec_SignatureZZNoneZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_C2Tuple_SignatureCVec_SignatureZZNoneZ, instantiationContext: String,
		...
		public class func initWithOk(o: ([UInt8], [[UInt8]])) -> Result_C2Tuple_SignatureCVec_SignatureZZNoneZ {
			...
			let oTuple = Tuple_SignatureCVec_SignatureZZ(
				tuple: o,
				instantiationContext: "Result_C2Tuple_SignatureCVec_SignatureZZNoneZ.swift::\(#function):\(#line)"
			)
			.danglingClone()
		...
		internal func clone() -> Result_C2Tuple_SignatureCVec_SignatureZZNoneZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_C2Tuple_SignatureCVec_SignatureZZNoneZ>) in
		...
		public func getValue() -> ([UInt8], [[UInt8]])? {
			if self.cType?.result_ok == true {
				return Tuple_SignatureCVec_SignatureZZ(
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_C2Tuple_SignatureCVec_SignatureZZNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_C2Tuple_SignatureCVec_SignatureZZNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
