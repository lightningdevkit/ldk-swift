			...
			extension Bindings {
				...
				NodeId_as_pubkey(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = Result_PublicKeySecp256k1ErrorZ(cType: nativeCallResult, instantiationContext: "NodeId.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				NodeId_write(objPointer)
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing NodeId \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
