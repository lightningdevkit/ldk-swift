...
extension Bindings {
	...
	public class ChannelPublicKeys: NativeTypeWrapper {
		...
		/// states.
		public func getRevocationBasepoint() -> RevocationBasepoint {
			// native call variable prep
		...
		/// counterparty to create a secret which the counterparty can reveal to revoke previous
		/// states.
		public func setRevocationBasepoint(val: RevocationBasepoint) {
			// native call variable prep
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKChannelPublicKeys>) in
					ChannelPublicKeys_set_revocation_basepoint(thisPtrPointer, val.dynamicallyDangledClone().cType!)
				}
		...
		public func getDelayedPaymentBasepoint() -> DelayedPaymentBasepoint {
			// native call variable prep
		...
		/// public key which receives non-HTLC-encumbered funds which are only available for spending
		/// after some delay (or can be claimed via the revocation path).
		public func setDelayedPaymentBasepoint(val: DelayedPaymentBasepoint) {
			// native call variable prep
		...
		public func getHtlcBasepoint() -> HtlcBasepoint {
			// native call variable prep
		...
		/// The base point which is used (with derive_public_key) to derive a per-commitment public key
		/// which is used to encumber HTLC-in-flight outputs.
		public func setHtlcBasepoint(val: HtlcBasepoint) {
			// native call variable prep
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKChannelPublicKeys>) in
					ChannelPublicKeys_set_htlc_basepoint(thisPtrPointer, val.dynamicallyDangledClone().cType!)
				}
		...
		public init(
			fundingPubkeyArg: [UInt8], revocationBasepointArg: RevocationBasepoint, paymentPointArg: [UInt8],
			delayedPaymentBasepointArg: DelayedPaymentBasepoint, htlcBasepointArg: HtlcBasepoint
		) {
			// native call variable prep
			...
			let nativeCallResult = ChannelPublicKeys_new(
				fundingPubkeyArgPrimitiveWrapper.cType!, revocationBasepointArg.dynamicallyDangledClone().cType!,
				paymentPointArgPrimitiveWrapper.cType!, delayedPaymentBasepointArg.dynamicallyDangledClone().cType!,
				htlcBasepointArg.dynamicallyDangledClone().cType!)
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKChannelPublicKeys>) in
					ChannelPublicKeys_hash(oPointer)
				}
