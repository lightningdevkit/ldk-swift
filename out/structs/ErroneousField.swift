			...
			extension Bindings {
				...
				public class ErroneousField: NativeTypeWrapper {
					...
					internal init(cType: LDKErroneousField, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				ErroneousField_get_tlv_fieldnum(thisPtrPointer)
					...
					public func setTlvFieldnum(val: UInt64) {
						...
						// native method call
						let nativeCallResult = 
						withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKErroneousField>) in
				...
				ErroneousField_get_suggested_value(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_CVec_u8ZZ(cType: nativeCallResult, instantiationContext: "ErroneousField.swift::\(#function):\(#line)", anchor: self).getValue()
					...
					public func setSuggestedValue(val: [UInt8]?) {
						...
						let valOption = Option_CVec_u8ZZ(some: val, instantiationContext: "ErroneousField.swift::\(#function):\(#line)").danglingClone()
				...
				ErroneousField_set_suggested_value(thisPtrPointer, valOption.cType!)
					...
					public init(tlvFieldnumArg: UInt64, suggestedValueArg: [UInt8]?) {
						...
						let suggestedValueArgOption = Option_CVec_u8ZZ(some: suggestedValueArg, instantiationContext: "ErroneousField.swift::\(#function):\(#line)").danglingClone()
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "ErroneousField.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing ErroneousField \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing ErroneousField \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
