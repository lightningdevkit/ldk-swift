...
extension Bindings {
	...
	public class PrintableString: NativeTypeWrapper {
		...
		internal init(
			cType: LDKPrintableString, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getA() -> String {
			...
			// return value (do some wrapping)
			let returnValue = Str(
				cType: nativeCallResult, instantiationContext: "PrintableString.swift::\(#function):\(#line)",
				anchor: self
			)
		...
		public func setA(val: String) {
			...
			let valPrimitiveWrapper = Str(
				value: val, instantiationContext: "PrintableString.swift::\(#function):\(#line)"
			)
			.dangle()
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKPrintableString>) in
		...
		public init(aArg: String) {
			...
			let aArgPrimitiveWrapper = Str(
				value: aArg, instantiationContext: "PrintableString.swift::\(#function):\(#line)"
			)
			.dangle()
			...
			super
				.init(
					...
					instantiationContext: "PrintableString.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> PrintableString {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing PrintableString \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing PrintableString \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
