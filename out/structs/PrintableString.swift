			...
			extension Bindings {
				...
				public class PrintableString: NativeTypeWrapper {
					...
					internal init(cType: LDKPrintableString) {
					...
					internal init(cType: LDKPrintableString, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				self.initialCFreeability = nativeCallResult.is_owned
						...
						/*
						// return value (do some wrapping)
						let returnValue = PrintableString(cType: nativeCallResult)
						*/
				...
				super.init(conflictAvoidingVariableName: 0)
					...
					deinit {
						...
						if !self.dangling {
							Bindings.print("Freeing PrintableString \(self.instanceNumber).")
							
							self.free()
						} else {
							Bindings.print("Not freeing PrintableString \(self.instanceNumber) due to dangle.")
						}
					}
