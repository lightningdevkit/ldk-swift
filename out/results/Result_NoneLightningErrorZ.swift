...
extension Bindings {
	...
	public class Result_NoneLightningErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_NoneLightningErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk() -> Result_NoneLightningErrorZ {
			...
			let returnValue = Result_NoneLightningErrorZ(
				cType: nativeCallResult, instantiationContext: "Result_NoneLightningErrorZ.swift::\(#function):\(#line)"
			)
		...
		public func getError() -> LightningError? {
			if self.cType?.result_ok == false {
				return LightningError(
					...
					instantiationContext: "Result_NoneLightningErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_NoneLightningErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_NoneLightningErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_NoneLightningErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
