...
extension Bindings {
	...
	internal class Option_ScalarZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCOption_ScalarZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(some: [UInt8]?, instantiationContext: String) {
			...
			if let some = some {
				...
				let somePrimitiveWrapper = BigEndianScalar(
					value: some, instantiationContext: "Option_ScalarZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> [UInt8]? {
			...
			if self.cType!.tag == LDKCOption_ScalarZ_Some {
				return BigEndianScalar(
					cType: self.cType!.some, instantiationContext: "Option_ScalarZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle().getValue()
			}
		...
		internal func dangleRecursively() -> Option_ScalarZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Option_ScalarZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Option_ScalarZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
