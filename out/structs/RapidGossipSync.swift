...
extension Bindings {
	...
	public class RapidGossipSync: NativeTypeWrapper {
		...
		internal init(
			cType: LDKRapidGossipSync, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init(networkGraph: NetworkGraph, logger: Logger) {
			...
			super
				.init(
					...
					instantiationContext: "RapidGossipSync.swift::\(#function):\(#line)")
		...
		public func updateNetworkGraphNoStd(updateData: [UInt8], currentTimeUnix: UInt64?) -> Result_u32GraphSyncErrorZ
		{
			...
			let currentTimeUnixOption = Option_u64Z(
				some: currentTimeUnix, instantiationContext: "RapidGossipSync.swift::\(#function):\(#line)"
			)
			.danglingClone()
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (thisArgPointer: UnsafePointer<LDKRapidGossipSync>) in
					RapidGossipSync_update_network_graph_no_std(
						thisArgPointer, updateDataPrimitiveWrapper.cType!, currentTimeUnixOption.cType!)
				}
		...
		internal func dangleRecursively() -> RapidGossipSync {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing RapidGossipSync \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing RapidGossipSync \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
