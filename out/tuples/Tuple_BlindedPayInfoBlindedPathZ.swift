			...
			extension Bindings {
				...
				internal class Tuple_BlindedPayInfoBlindedPathZ: NativeTypeWrapper {
					...
					internal init(cType: LDKC2Tuple_BlindedPayInfoBlindedPathZ, instantiationContext: String, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(cType: LDKC2Tuple_BlindedPayInfoBlindedPathZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				super.init(conflictAvoidingVariableName: 0, instantiationContext: instantiationContext)
					...
					public func getA() -> BlindedPayInfo {
						// return value (do some wrapping)
						let returnValue = BlindedPayInfo(cType: self.cType!.a, instantiationContext: "Tuple_BlindedPayInfoBlindedPathZ.swift::\(#function):\(#line)", anchor: self).dangle()
					...
					public func getB() -> BlindedPath {
						// return value (do some wrapping)
						let returnValue = BlindedPath(cType: self.cType!.b, instantiationContext: "Tuple_BlindedPayInfoBlindedPathZ.swift::\(#function):\(#line)", anchor: self).dangle()
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Tuple_BlindedPayInfoBlindedPathZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Tuple_BlindedPayInfoBlindedPathZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
