...
extension Bindings {
	...
	public class Result_CommitmentSignedDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_CommitmentSignedDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: CommitmentSigned) -> Result_CommitmentSignedDecodeErrorZ {
			...
			let returnValue = Result_CommitmentSignedDecodeErrorZ(
				cType: nativeCallResult,
				instantiationContext: "Result_CommitmentSignedDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_CommitmentSignedDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_CommitmentSignedDecodeErrorZ>) in
		...
		internal func dangleRecursively() -> Result_CommitmentSignedDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_CommitmentSignedDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_CommitmentSignedDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
