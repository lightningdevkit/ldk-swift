			...
			extension Bindings {
				...
				public class OnionMessage: NativeTypeWrapper {
					...
					internal init(cType: LDKOnionMessage, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				OnionMessage_get_onion_routing_packet(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Packet(cType: nativeCallResult, instantiationContext: "OnionMessage.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				OnionMessage_set_onion_routing_packet(thisPtrPointer, val.dynamicallyDangledClone().cType!)
					...
					public init(blindingPointArg: [UInt8], onionRoutingPacketArg: Packet) {
						...
						let blindingPointArgPrimitiveWrapper = PublicKey(value: blindingPointArg, instantiationContext: "OnionMessage.swift::\(#function):\(#line)")
				

						// native method call
						let nativeCallResult = OnionMessage_new(blindingPointArgPrimitiveWrapper.cType!, onionRoutingPacketArg.dynamicallyDangledClone().cType!)
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "OnionMessage.swift::\(#function):\(#line)")
