			...
			extension Bindings {
				...
				UnsignedNodeAnnouncement_get_addresses(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_SocketAddressZ(cType: nativeCallResult, instantiationContext: "UnsignedNodeAnnouncement.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public func setAddresses(val: [SocketAddress]) {
						...
						let valVector = Vec_SocketAddressZ(array: val, instantiationContext: "UnsignedNodeAnnouncement.swift::\(#function):\(#line)").dangle()
