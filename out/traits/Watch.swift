	...
	open class Watch: NativeTraitWrapper {
		...
		public init() {
				...
				-> LDKChannelMonitorUpdateStatus
			{
				...
				// Swift callback call
				let swiftCallbackResult = instance.watchChannel(
					fundingTxo: OutPoint(
						cType: funding_txo, instantiationContext: "Watch.swift::init()::\(#function):\(#line)"),
					monitor: ChannelMonitor(
						cType: monitor, instantiationContext: "Watch.swift::init()::\(#function):\(#line)"))
			...
			func updateChannelLambda(
				this_arg: UnsafeRawPointer?, funding_txo: LDKOutPoint, update: UnsafePointer<LDKChannelMonitorUpdate>
			) -> LDKChannelMonitorUpdateStatus {
				let instance: Watch = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "Watch::updateChannelLambda")

				// Swift callback variable prep


				// Swift callback call
				let swiftCallbackResult = instance.updateChannel(
					fundingTxo: OutPoint(
						cType: funding_txo, instantiationContext: "Watch.swift::init()::\(#function):\(#line)"),
					update: ChannelMonitorUpdate(
						cType: update.pointee, instantiationContext: "Watch.swift::init()::\(#function):\(#line)"
					)
					.dangle().clone())
		...
		open func updateChannel(fundingTxo: OutPoint, update: ChannelMonitorUpdate) -> ChannelMonitorUpdateStatus {
			...
			Bindings.print(
				"Error: Watch::updateChannel MUST be overridden! Offending class: \(String(describing: self)). Aborting.",
				severity: .ERROR)
			abort()
		}
		...
		internal func dangleRecursively() -> Watch {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Watch \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
	}
	...
	internal class NativelyImplementedWatch: Watch {
		...
		public override func watchChannel(fundingTxo: OutPoint, monitor: ChannelMonitor) -> ChannelMonitorUpdateStatus {
			...
			let nativeCallResult = self.cType!
				.watch_channel(
					self.cType!.this_arg, fundingTxo.dynamicallyDangledClone().cType!,
					monitor.dynamicallyDangledClone().cType!)
			...
			-> ChannelMonitorUpdateStatus
		{
			...
			let nativeCallResult =
				withUnsafePointer(to: update.cType!) { (updatePointer: UnsafePointer<LDKChannelMonitorUpdate>) in
					self.cType!
						.update_channel(
							self.cType!.this_arg, fundingTxo.dynamicallyDangledClone().cType!, updatePointer)
				}
