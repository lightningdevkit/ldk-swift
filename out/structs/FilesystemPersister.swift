...
extension Bindings {
	...
	public class FilesystemPersister: NativeTypeWrapper {
		...
		internal init(
			cType: LDKFilesystemPersister, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init(pathToChannelData: String) {
			...
			let pathToChannelDataPrimitiveWrapper = Str(
				value: pathToChannelData, instantiationContext: "FilesystemPersister.swift::\(#function):\(#line)"
			)
			.dangle()
			...
			super
				.init(
					...
					instantiationContext: "FilesystemPersister.swift::\(#function):\(#line)")
		...
		public func getDataDir() -> String {
			...
			// return value (do some wrapping)
			let returnValue = Str(
				cType: nativeCallResult, instantiationContext: "FilesystemPersister.swift::\(#function):\(#line)",
				anchor: self
			)
		...
		public func readChannelmonitors(entropySource: EntropySource, signerProvider: SignerProvider)
				...
				withUnsafePointer(to: self.cType!) { (thisArgPointer: UnsafePointer<LDKFilesystemPersister>) in
					...
					withUnsafePointer(to: signerProvider.activate().cType!) {
						...
						FilesystemPersister_read_channelmonitors(
							thisArgPointer, entropySource.activate().cType!, signerProviderPointer)
					}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing FilesystemPersister \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing FilesystemPersister \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
