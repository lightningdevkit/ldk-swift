			...
			extension Bindings {
				...
				public class ChannelUsage: NativeTypeWrapper {
					...
					internal init(cType: LDKChannelUsage, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing ChannelUsage \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing ChannelUsage \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
