...
extension Bindings {
	...
	public class Bolt11InvoiceSignature: NativeTypeWrapper {
		...
		internal init(
			cType: LDKBolt11InvoiceSignature, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> Bolt11InvoiceSignature {
			...
			let returnValue = Bolt11InvoiceSignature(
				cType: nativeCallResult, instantiationContext: "Bolt11InvoiceSignature.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Bolt11InvoiceSignature \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Bolt11InvoiceSignature \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
