			...
			extension Bindings {
				...
				public class InvoiceRequest: NativeTypeWrapper {
					...
					internal init(cType: LDKInvoiceRequest, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				InvoiceRequest_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = InvoiceRequest(cType: nativeCallResult)
				...
				InvoiceRequest_write(objPointer)
					...
					internal func dangle(_ shouldDangle: Bool = true) -> InvoiceRequest {
					...
					deinit {
						...
						if !self.dangling {
							Bindings.print("Freeing InvoiceRequest \(self.instanceNumber).")
							
							self.free()
						} else {
							Bindings.print("Not freeing InvoiceRequest \(self.instanceNumber) due to dangle.")
						}
					}
