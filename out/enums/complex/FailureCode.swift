...
extension Bindings {
	...
	public class FailureCode: NativeTypeWrapper {
		...
		internal init(
			cType: LDKFailureCode, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> FailureCode {
			...
			// return value (do some wrapping)
			let returnValue = FailureCode(
				cType: nativeCallResult, instantiationContext: "FailureCode.swift::\(#function):\(#line)")
		...
		public class func initWithInvalidOnionPayload(a: (UInt64, UInt16)?) -> FailureCode {
			...
			let aOption = Option_C2Tuple_u64u16ZZ(
				some: a, instantiationContext: "FailureCode.swift::\(#function):\(#line)"
			)
			.danglingClone()
		...
		public func getValueAsInvalidOnionPayload() -> (UInt64, UInt16)?? {
			...
			return Option_C2Tuple_u64u16ZZ(
				cType: self.cType!.invalid_onion_payload,
				instantiationContext: "FailureCode.swift::\(#function):\(#line)", anchor: self
			)
			.getValue()
		}
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing FailureCode \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
