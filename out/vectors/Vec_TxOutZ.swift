			...
			extension Bindings {
				...
				internal class Vec_TxOutZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_TxOutZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
...
TxOut(cType: currentCType, instantiationContext: "Vec_TxOutZ.swift::\(#function):\(#line)", anchor: self).dangle()
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing Vec_TxOutZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
