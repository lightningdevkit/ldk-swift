			...
			extension Bindings {
				...
				public class Result_OnionMessagePathNoneZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_OnionMessagePathNoneZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: OnionMessagePath) -> Result_OnionMessagePathNoneZ {
						...
						let returnValue = Result_OnionMessagePathNoneZ(cType: nativeCallResult, instantiationContext: "Result_OnionMessagePathNoneZ.swift::\(#function):\(#line)")
					...
					*/
					...
					public func getValue() -> OnionMessagePath? {
						if self.cType?.result_ok == true {
							return OnionMessagePath(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_OnionMessagePathNoneZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_OnionMessagePathNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_OnionMessagePathNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
