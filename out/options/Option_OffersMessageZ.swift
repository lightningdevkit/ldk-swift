...
extension Bindings {
	...
	internal class Option_OffersMessageZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCOption_OffersMessageZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> Option_OffersMessageZ {
			...
			let returnValue = Option_OffersMessageZ(
				cType: nativeCallResult, instantiationContext: "Option_OffersMessageZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> OffersMessage? {
			...
			if self.cType!.tag == LDKCOption_OffersMessageZ_Some {
				return OffersMessage(
					cType: self.cType!.some, instantiationContext: "Option_OffersMessageZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Option_OffersMessageZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Option_OffersMessageZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
