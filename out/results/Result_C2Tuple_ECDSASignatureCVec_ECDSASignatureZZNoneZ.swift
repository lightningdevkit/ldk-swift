...
public typealias Result_C2Tuple_ECDSASignatureCVec_ECDSASignatureZZNoneZ = Bindings
...
extension Bindings {
	...
	public class Result_C2Tuple_ECDSASignatureCVec_ECDSASignatureZZNoneZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_C2Tuple_ECDSASignatureCVec_ECDSASignatureZZNoneZ, instantiationContext: String,
		...
		public class func initWithOk(o: ([UInt8], [[UInt8]])) -> Result_C2Tuple_ECDSASignatureCVec_ECDSASignatureZZNoneZ
		{
			...
			let oTuple = Tuple_ECDSASignatureCVec_ECDSASignatureZZ(
				tuple: o,
				instantiationContext:
					"Result_C2Tuple_ECDSASignatureCVec_ECDSASignatureZZNoneZ.swift::\(#function):\(#line)"
			)
			.danglingClone()
		...
		internal func clone() -> Result_C2Tuple_ECDSASignatureCVec_ECDSASignatureZZNoneZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_C2Tuple_ECDSASignatureCVec_ECDSASignatureZZNoneZ>) in
		...
		public func getValue() -> ([UInt8], [[UInt8]])? {
			if self.cType?.result_ok == true {
				return Tuple_ECDSASignatureCVec_ECDSASignatureZZ(
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_C2Tuple_ECDSASignatureCVec_ECDSASignatureZZNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_C2Tuple_ECDSASignatureCVec_ECDSASignatureZZNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
