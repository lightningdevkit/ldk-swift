...
extension Bindings {
	...
	public class FundingSigned: NativeTypeWrapper {
		...
		internal init(
			cType: LDKFundingSigned, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func setChannelId(val: [UInt8]) {
			...
			let valPrimitiveWrapper = ThirtyTwoBytes(
				value: val, instantiationContext: "FundingSigned.swift::\(#function):\(#line)")
		...
		public func getSignature() -> [UInt8] {
			...
			let returnValue = ECDSASignature(
				cType: nativeCallResult, instantiationContext: "FundingSigned.swift::\(#function):\(#line)",
				anchor: self
			)
		...
		public init(channelIdArg: [UInt8], signatureArg: [UInt8]) {
			...
			let channelIdArgPrimitiveWrapper = ThirtyTwoBytes(
				value: channelIdArg, instantiationContext: "FundingSigned.swift::\(#function):\(#line)")

			let signatureArgPrimitiveWrapper = ECDSASignature(
				value: signatureArg, instantiationContext: "FundingSigned.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult = FundingSigned_new(
				channelIdArgPrimitiveWrapper.cType!, signatureArgPrimitiveWrapper.cType!)
			...
			super
				.init(
					conflictAvoidingVariableName: 0, instantiationContext: "FundingSigned.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKFundingSigned>) in
					FundingSigned_hash(oPointer)
				}
		...
		public class func read(ser: [UInt8]) -> Result_FundingSignedDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "FundingSigned.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing FundingSigned \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing FundingSigned \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
