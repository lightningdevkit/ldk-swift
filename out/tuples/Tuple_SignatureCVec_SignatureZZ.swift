...
extension Bindings {
	...
	internal class Tuple_SignatureCVec_SignatureZZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKC2Tuple_SignatureCVec_SignatureZZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> Tuple_SignatureCVec_SignatureZZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKC2Tuple_SignatureCVec_SignatureZZ>) in
			...
			let returnValue = Tuple_SignatureCVec_SignatureZZ(
				cType: nativeCallResult,
				instantiationContext: "Tuple_SignatureCVec_SignatureZZ.swift::\(#function):\(#line)")
		...
		public init(a: [UInt8], b: [[UInt8]], instantiationContext: String) {
			...
			let aPrimitiveWrapper = Signature(
				value: a, instantiationContext: "Tuple_SignatureCVec_SignatureZZ.swift::\(#function):\(#line)")

			let bVector = Vec_SignatureZ(
				array: b, instantiationContext: "Tuple_SignatureCVec_SignatureZZ.swift::\(#function):\(#line)"
			)
			.dangle()
		...
		public func getA() -> [UInt8] {
			...
			let returnValue = Signature(
				cType: self.cType!.a,
				instantiationContext: "Tuple_SignatureCVec_SignatureZZ.swift::\(#function):\(#line)", anchor: self
			)
			.dangle().getValue()
		...
		internal func dangleRecursively() -> Tuple_SignatureCVec_SignatureZZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Tuple_SignatureCVec_SignatureZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Tuple_SignatureCVec_SignatureZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
