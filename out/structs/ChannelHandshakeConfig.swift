			...
			extension Bindings {
				...
				public class ChannelHandshakeConfig: NativeTypeWrapper {
					...
					internal init(cType: LDKChannelHandshakeConfig, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing ChannelHandshakeConfig \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing ChannelHandshakeConfig \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
