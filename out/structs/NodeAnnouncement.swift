...
extension Bindings {
	...
	public class NodeAnnouncement: NativeTypeWrapper {
		...
		internal init(
			cType: LDKNodeAnnouncement, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func setSignature(val: [UInt8]) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKNodeAnnouncement>) in
		...
		public init(signatureArg: [UInt8], contentsArg: UnsignedNodeAnnouncement) {
			...
			// native method call
			let nativeCallResult = NodeAnnouncement_new(
			...
			super
				.init(
					...
					instantiationContext: "NodeAnnouncement.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKNodeAnnouncement>) in
					NodeAnnouncement_hash(oPointer)
				}
		...
		public class func read(ser: [UInt8]) -> Result_NodeAnnouncementDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "NodeAnnouncement.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_NodeAnnouncementDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "NodeAnnouncement.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing NodeAnnouncement \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing NodeAnnouncement \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
