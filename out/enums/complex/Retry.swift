			...
			extension Bindings {
				...
				Retry_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "Retry.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public class func read(ser: [UInt8]) -> Result_RetryDecodeErrorZ {
						...
						// return value (do some wrapping)
						let returnValue = Result_RetryDecodeErrorZ(cType: nativeCallResult, instantiationContext: "Retry.swift::\(#function):\(#line)")
