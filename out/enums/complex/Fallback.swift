	...
	public class Fallback: NativeTypeWrapper {
		...
		internal func clone() -> Fallback {
			...
			let returnValue = Fallback(
				cType: nativeCallResult, instantiationContext: "Fallback.swift::\(#function):\(#line)")
		...
		public class func initWithSegWitProgram(version: UInt8, program: [UInt8]) -> Fallback {
			...
			let versionPrimitiveWrapper = WitnessVersion(
				value: version, instantiationContext: "Fallback.swift::\(#function):\(#line)")

			let programVector = Vec_u8Z(array: program, instantiationContext: "Fallback.swift::\(#function):\(#line)")
				.dangle()
		...
		public func getValueAsSegWitProgram() -> SegWitProgram? {
			...
			return Fallback_LDKSegWitProgram_Body(
				cType: self.cType!.seg_wit_program, instantiationContext: "Fallback.swift::\(#function):\(#line)",
				anchor: self)
		}
		...
		public func getValueAsPubKeyHash() -> [UInt8]? {
			...
			return TwentyBytes(
				cType: self.cType!.pub_key_hash, instantiationContext: "Fallback.swift::\(#function):\(#line)",
				anchor: self
			)
			.getValue()
		}
		...
		public func getValueAsScriptHash() -> [UInt8]? {
			...
			return TwentyBytes(
				cType: self.cType!.script_hash, instantiationContext: "Fallback.swift::\(#function):\(#line)",
				anchor: self
			)
			.getValue()
		}
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Fallback \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
		...
		public class SegWitProgram: NativeTypeWrapper {
			...
			internal init(
				cType: LDKFallback_LDKSegWitProgram_Body, instantiationContext: String, anchor: NativeTypeWrapper
			) {
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				self.cType = cType
			...
			public func getVersion() -> UInt8 {
				...
				let returnValue = WitnessVersion(
					cType: self.cType!.version, instantiationContext: "Fallback.swift::\(#function):\(#line)",
					anchor: self
				)
				.getValue()
			...
			public func getProgram() -> [UInt8] {
				...
				let returnValue = Vec_u8Z(
					cType: self.cType!.program, instantiationContext: "Fallback.swift::\(#function):\(#line)",
					anchor: self
				)
				.getValue()
