			...
			extension Bindings {
				...
				public class Result_NodeFeaturesDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_NodeFeaturesDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_NodeFeaturesDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> NodeFeatures? {
						if self.cType?.result_ok == true {
							return NodeFeatures(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
