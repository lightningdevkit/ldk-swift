...
extension Bindings {
	...
	public class PathFailure: NativeTypeWrapper {
		...
		internal init(
			cType: LDKPathFailure, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> PathFailure {
			...
			let returnValue = PathFailure(
				cType: nativeCallResult, instantiationContext: "PathFailure.swift::\(#function):\(#line)")
		...
		public class func initWithOnPath(networkUpdate: NetworkUpdate?) -> PathFailure {
			...
			let networkUpdateOption = Option_NetworkUpdateZ(
				some: networkUpdate, instantiationContext: "PathFailure.swift::\(#function):\(#line)"
			)
			.danglingClone()
		...
		public func write() -> [UInt8] {
			...
			let returnValue = Vec_u8Z(
				cType: nativeCallResult, instantiationContext: "PathFailure.swift::\(#function):\(#line)", anchor: self
			)
		...
		public class func read(ser: [UInt8]) -> Result_COption_PathFailureZDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "PathFailure.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_COption_PathFailureZDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "PathFailure.swift::\(#function):\(#line)")
		...
		public func getValueAsInitialSend() -> InitialSend? {
			...
			return PathFailure_LDKInitialSend_Body(
				cType: self.cType!.initial_send, instantiationContext: "PathFailure.swift::\(#function):\(#line)",
				anchor: self)
		}
		...
		public func getValueAsOnPath() -> OnPath? {
			...
			return PathFailure_LDKOnPath_Body(
				cType: self.cType!.on_path, instantiationContext: "PathFailure.swift::\(#function):\(#line)",
				anchor: self)
		}
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing PathFailure \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
		...
		public class InitialSend: NativeTypeWrapper {
			...
			internal init(
				cType: LDKPathFailure_LDKInitialSend_Body, instantiationContext: String, anchor: NativeTypeWrapper
			) {
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				self.cType = cType
			...
			public func getErr() -> APIError {
				...
				let returnValue = APIError(
					cType: self.cType!.err, instantiationContext: "PathFailure.swift::\(#function):\(#line)",
					anchor: self)
		...
		public class OnPath: NativeTypeWrapper {
			...
			internal init(
				cType: LDKPathFailure_LDKOnPath_Body, instantiationContext: String, anchor: NativeTypeWrapper,
				dangle: Bool = false
			) {
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
			...
			public func getNetworkUpdate() -> NetworkUpdate? {
				...
				let returnValue = Option_NetworkUpdateZ(
					cType: self.cType!.network_update, instantiationContext: "PathFailure.swift::\(#function):\(#line)",
					anchor: self
				)
				.getValue()
