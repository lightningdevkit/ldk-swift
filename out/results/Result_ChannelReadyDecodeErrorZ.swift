			...
			extension Bindings {
				...
				public class Result_ChannelReadyDecodeErrorZ: NativeTypeWrapper {
					...
					public class func initWithOk(o: ChannelReady) -> Result_ChannelReadyDecodeErrorZ {
						...
						let returnValue = Result_ChannelReadyDecodeErrorZ(cType: nativeCallResult, instantiationContext: "#{swift_class_name}::\(#function):\(#line)")
				...
				CResult_ChannelReadyDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> ChannelReady? {
						if self.cType?.result_ok == true {
							return ChannelReady(cType: self.cType!.contents.result.pointee, instantiationContext: "#{swift_class_name}::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
