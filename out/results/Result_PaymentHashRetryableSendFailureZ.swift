			...
			extension Bindings {
				...
				public class Result_PaymentHashRetryableSendFailureZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_PaymentHashRetryableSendFailureZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: [UInt8]) -> Result_PaymentHashRetryableSendFailureZ {
						...
						let returnValue = Result_PaymentHashRetryableSendFailureZ(cType: nativeCallResult)
