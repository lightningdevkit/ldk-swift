...
extension Bindings {
	...
	public class ProbabilisticScoringParameters: NativeTypeWrapper {
		...
		internal init(cType: LDKProbabilisticScoringParameters, instantiationContext: String) {
		...
		internal init(cType: LDKProbabilisticScoringParameters, instantiationContext: String, anchor: NativeTypeWrapper)
		{
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(
			cType: LDKProbabilisticScoringParameters, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func getBasePenaltyMsat() -> UInt64 {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(thisPtrPointer: UnsafePointer<LDKProbabilisticScoringParameters>) in
		...
		public func setBasePenaltyMsat(val: UInt64) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKProbabilisticScoringParameters>) in
		...
		internal func clone() -> ProbabilisticScoringParameters {
			...
			// return value (do some wrapping)
			let returnValue = ProbabilisticScoringParameters(
				cType: nativeCallResult,
				instantiationContext: "ProbabilisticScoringParameters.swift::\(#function):\(#line)")
		...
		public func addBannedFromList(nodeIds: [NodeId]) {
			...
			let nodeIdsVector = Vec_NodeIdZ(
				array: nodeIds, instantiationContext: "ProbabilisticScoringParameters.swift::\(#function):\(#line)"
			)
			.dangle()
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisArgPointer: UnsafeMutablePointer<LDKProbabilisticScoringParameters>) in
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing ProbabilisticScoringParameters \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing ProbabilisticScoringParameters \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
