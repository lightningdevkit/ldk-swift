...
extension Bindings {
	...
	public class Result_PaymentSecretNoneZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_PaymentSecretNoneZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: [UInt8]) -> Result_PaymentSecretNoneZ {
			...
			let oPrimitiveWrapper = ThirtyTwoBytes(
				value: o, instantiationContext: "Result_PaymentSecretNoneZ.swift::\(#function):\(#line)")
			...
			let returnValue = Result_PaymentSecretNoneZ(
				cType: nativeCallResult, instantiationContext: "Result_PaymentSecretNoneZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> [UInt8]? {
			if self.cType?.result_ok == true {
				return ThirtyTwoBytes(
					...
					instantiationContext: "Result_PaymentSecretNoneZ.swift::\(#function):\(#line)", anchor: self
				)
				.getValue()
			}
			return nil
		...
		internal func dangleRecursively() -> Result_PaymentSecretNoneZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_PaymentSecretNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_PaymentSecretNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
