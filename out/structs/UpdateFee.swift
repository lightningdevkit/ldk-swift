			...
			extension Bindings {
				...
				public class UpdateFee: NativeTypeWrapper {
					...
					internal init(cType: LDKUpdateFee, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				UpdateFee_write(objPointer)
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing UpdateFee \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
