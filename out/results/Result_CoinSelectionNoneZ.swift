...
extension Bindings {
	...
	public class Result_CoinSelectionNoneZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_CoinSelectionNoneZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: CoinSelection) -> Result_CoinSelectionNoneZ {
			...
			let returnValue = Result_CoinSelectionNoneZ(
				cType: nativeCallResult, instantiationContext: "Result_CoinSelectionNoneZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> CoinSelection? {
			if self.cType?.result_ok == true {
				return CoinSelection(
					...
					instantiationContext: "Result_CoinSelectionNoneZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_CoinSelectionNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_CoinSelectionNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
