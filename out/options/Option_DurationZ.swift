			...
			extension Bindings {
				...
				internal class Option_DurationZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCOption_DurationZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				COption_DurationZ_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_DurationZ(cType: nativeCallResult, instantiationContext: "Option_DurationZ.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Option_DurationZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Option_DurationZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
