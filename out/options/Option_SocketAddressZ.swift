...
extension Bindings {
	...
	internal class Option_SocketAddressZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCOption_SocketAddressZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> Option_SocketAddressZ {
			...
			let returnValue = Option_SocketAddressZ(
				cType: nativeCallResult, instantiationContext: "Option_SocketAddressZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> SocketAddress? {
			...
			if self.cType!.tag == LDKCOption_SocketAddressZ_Some {
				return SocketAddress(
					cType: self.cType!.some, instantiationContext: "Option_SocketAddressZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Option_SocketAddressZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Option_SocketAddressZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
