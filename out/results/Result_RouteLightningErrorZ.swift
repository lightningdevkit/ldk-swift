...
extension Bindings {
	...
	public class Result_RouteLightningErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_RouteLightningErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: Route) -> Result_RouteLightningErrorZ {
			...
			let returnValue = Result_RouteLightningErrorZ(
				...
				instantiationContext: "Result_RouteLightningErrorZ.swift::\(#function):\(#line)")
		...
		public func getError() -> LightningError? {
			if self.cType?.result_ok == false {
				return LightningError(
					...
					instantiationContext: "Result_RouteLightningErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangle(_ shouldDangle: Bool = true) -> Result_RouteLightningErrorZ {
			self.dangling = shouldDangle
			return self
		}

		internal func dangleRecursively() -> Result_RouteLightningErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_RouteLightningErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_RouteLightningErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
