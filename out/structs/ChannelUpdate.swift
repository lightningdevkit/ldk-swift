			...
			extension Bindings {
				...
				public class ChannelUpdate: NativeTypeWrapper {
					...
					internal init(cType: LDKChannelUpdate, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				ChannelUpdate_get_signature(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = ECDSASignature(cType: nativeCallResult, instantiationContext: "ChannelUpdate.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public func setSignature(val: [UInt8]) {
						...
						let valPrimitiveWrapper = ECDSASignature(value: val, instantiationContext: "ChannelUpdate.swift::\(#function):\(#line)")
				...
				ChannelUpdate_get_contents(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = UnsignedChannelUpdate(cType: nativeCallResult, instantiationContext: "ChannelUpdate.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				ChannelUpdate_set_contents(thisPtrPointer, val.dynamicallyDangledClone().cType!)
					...
					public init(signatureArg: [UInt8], contentsArg: UnsignedChannelUpdate) {
						...
						let signatureArgPrimitiveWrapper = ECDSASignature(value: signatureArg, instantiationContext: "ChannelUpdate.swift::\(#function):\(#line)")
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "ChannelUpdate.swift::\(#function):\(#line)")
				...
				ChannelUpdate_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "ChannelUpdate.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public class func read(ser: [UInt8]) -> Result_ChannelUpdateDecodeErrorZ {
						...
						let serPrimitiveWrapper = u8slice(value: ser, instantiationContext: "ChannelUpdate.swift::\(#function):\(#line)")
						...
						// return value (do some wrapping)
						let returnValue = Result_ChannelUpdateDecodeErrorZ(cType: nativeCallResult, instantiationContext: "ChannelUpdate.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing ChannelUpdate \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing ChannelUpdate \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
