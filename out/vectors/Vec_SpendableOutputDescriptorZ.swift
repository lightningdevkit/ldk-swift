			...
			extension Bindings {
				...
				internal class Vec_SpendableOutputDescriptorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_SpendableOutputDescriptorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Vec_SpendableOutputDescriptorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Vec_SpendableOutputDescriptorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
