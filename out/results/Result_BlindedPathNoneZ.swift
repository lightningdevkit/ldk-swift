...
extension Bindings {
	...
	public class Result_BlindedPathNoneZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_BlindedPathNoneZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: BlindedPath) -> Result_BlindedPathNoneZ {
			...
			let returnValue = Result_BlindedPathNoneZ(
				cType: nativeCallResult, instantiationContext: "Result_BlindedPathNoneZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> BlindedPath? {
			if self.cType?.result_ok == true {
				return BlindedPath(
					...
					instantiationContext: "Result_BlindedPathNoneZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_BlindedPathNoneZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_BlindedPathNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_BlindedPathNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
