...
extension Bindings {
	...
	internal class Vec_SignatureZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_SignatureZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(array: [[UInt8]], instantiationContext: String) {
			...
			let rustArray = array.map { (currentValueDepth1: [UInt8]) -> LDKSignature in
				...
				let currentValueDepth1PrimitiveWrapper = Signature(
					value: currentValueDepth1, instantiationContext: "Vec_SignatureZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> [[UInt8]] {
			...
			let swiftArray = array.map { (currentCType: LDKSignature) -> [UInt8] in
				Signature(cType: currentCType, instantiationContext: "Vec_SignatureZ.swift::\(#function):\(#line)")
					.dangle().getValue()
			}
			return swiftArray
		}
		...
		internal func dangleRecursively() -> Vec_SignatureZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_SignatureZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Vec_SignatureZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
