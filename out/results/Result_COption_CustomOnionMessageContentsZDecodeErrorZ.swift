			...
			extension Bindings {
				...
				public class Result_COption_CustomOnionMessageContentsZDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_COption_CustomOnionMessageContentsZDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_COption_CustomOnionMessageContentsZDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> CustomOnionMessageContents? {
						if self.cType?.result_ok == true {
							return Option_CustomOnionMessageContentsZ(cType: self.cType!.contents.result.pointee, anchor: self).getValue()
						}
						return nil
					}
