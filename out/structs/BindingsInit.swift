			...
			extension Bindings {
				...
				Init_get_networks(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_CVec_ThirtyTwoBytesZZ(cType: nativeCallResult, instantiationContext: "BindingsInit.swift::\(#function):\(#line)", anchor: self).getValue()
					...
					public func setNetworks(val: [[UInt8]]?) {
						...
						let valOption = Option_CVec_ThirtyTwoBytesZZ(some: val, instantiationContext: "BindingsInit.swift::\(#function):\(#line)").danglingClone()
				...
				Init_get_remote_network_address(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_SocketAddressZ(cType: nativeCallResult, instantiationContext: "BindingsInit.swift::\(#function):\(#line)", anchor: self).getValue()
					...
					public func setRemoteNetworkAddress(val: SocketAddress?) {
						...
						let valOption = Option_SocketAddressZ(some: val, instantiationContext: "BindingsInit.swift::\(#function):\(#line)").danglingClone()
				...
				Init_set_remote_network_address(thisPtrPointer, valOption.cType!)
					...
					public init(featuresArg: InitFeatures, networksArg: [[UInt8]]?, remoteNetworkAddressArg: SocketAddress?) {
						...
						let networksArgOption = Option_CVec_ThirtyTwoBytesZZ(some: networksArg, instantiationContext: "BindingsInit.swift::\(#function):\(#line)").danglingClone()
				
						let remoteNetworkAddressArgOption = Option_SocketAddressZ(some: remoteNetworkAddressArg, instantiationContext: "BindingsInit.swift::\(#function):\(#line)").danglingClone()
				

						// native method call
						let nativeCallResult = Init_new(featuresArg.dynamicallyDangledClone().cType!, networksArgOption.cType!, remoteNetworkAddressArgOption.cType!)
