			...
			extension Bindings {
				...
				public class OfferFeatures: NativeTypeWrapper {
					...
					internal init(cType: LDKOfferFeatures, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				OfferFeatures_set_required_feature_bit(thisArgPointer, bit)
						...
						// return value (do some wrapping)
						let returnValue = Result_NoneNoneZ(cType: nativeCallResult, instantiationContext: "OfferFeatures.swift::\(#function):\(#line)", anchor: self).dangle(false)
