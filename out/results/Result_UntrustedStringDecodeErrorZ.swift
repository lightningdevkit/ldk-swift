...
extension Bindings {
	...
	public class Result_UntrustedStringDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_UntrustedStringDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: UntrustedString) -> Result_UntrustedStringDecodeErrorZ {
			...
			let returnValue = Result_UntrustedStringDecodeErrorZ(
				...
				instantiationContext: "Result_UntrustedStringDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_UntrustedStringDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_UntrustedStringDecodeErrorZ>) in
		...
		internal func dangleRecursively() -> Result_UntrustedStringDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_UntrustedStringDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_UntrustedStringDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
