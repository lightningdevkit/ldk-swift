...
extension Bindings {
	...
	open class SocketDescriptor: NativeTraitWrapper {
		...
		internal init(
			cType: LDKSocketDescriptor, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init() {
			...
			super
				.init(
					...
					instantiationContext: "SocketDescriptor.swift::\(#function):\(#line)")

			let thisArg = Bindings.instanceToPointer(instance: self)


			func sendDataLambda(this_arg: UnsafeMutableRawPointer?, data: LDKu8slice, resume_read: Bool) -> UInt {
				let instance: SocketDescriptor = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "SocketDescriptor::sendDataLambda")

				// Swift callback variable prep


				// Swift callback call
				let swiftCallbackResult = instance.sendData(
					data: u8slice(
						cType: data, instantiationContext: "SocketDescriptor.swift::init()::\(#function):\(#line)"
					)
					.dangle().getValue(), resumeRead: resume_read)
			...
			func disconnectSocketLambda(this_arg: UnsafeMutableRawPointer?) {
				let instance: SocketDescriptor = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "SocketDescriptor::disconnectSocketLambda")
			...
			func eqLambda(this_arg: UnsafeRawPointer?, other_arg: UnsafePointer<LDKSocketDescriptor>) -> Bool {
				let instance: SocketDescriptor = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "SocketDescriptor::eqLambda")

				// Swift callback variable prep


				// Swift callback call
				let swiftCallbackResult = instance.eq(
					otherArg: NativelyImplementedSocketDescriptor(
						cType: other_arg.pointee,
						instantiationContext: "SocketDescriptor.swift::init()::\(#function):\(#line)"))
			...
			func hashLambda(this_arg: UnsafeRawPointer?) -> UInt64 {
				let instance: SocketDescriptor = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "SocketDescriptor::hashLambda")
			...
			func freeLambda(this_arg: UnsafeMutableRawPointer?) {
				let instance: SocketDescriptor = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "SocketDescriptor::freeLambda")
		...
		open func sendData(data: [UInt8], resumeRead: Bool) -> UInt {
			...
			Bindings.print(
				"Error: SocketDescriptor::sendData MUST be overridden! Offending class: \(String(describing: self)). Aborting.",
				severity: .ERROR)
			abort()
		}
		...
		open func disconnectSocket() {
			...
			Bindings.print(
				"Error: SocketDescriptor::disconnectSocket MUST be overridden! Offending class: \(String(describing: self)). Aborting.",
				severity: .ERROR)
			abort()
		}
		...
		open func eq(otherArg: SocketDescriptor) -> Bool {
			...
			Bindings.print(
				"Error: SocketDescriptor::eq MUST be overridden! Offending class: \(String(describing: self)). Aborting.",
				severity: .ERROR)
			abort()
		}
		...
		open func hash() -> UInt64 {
			...
			Bindings.print(
				"Error: SocketDescriptor::hash MUST be overridden! Offending class: \(String(describing: self)). Aborting.",
				severity: .ERROR)
			abort()
		}
		...
		internal func clone() -> SocketDescriptor {
			...
			// return value (do some wrapping)
			let returnValue = NativelyImplementedSocketDescriptor(
				cType: nativeCallResult, instantiationContext: "SocketDescriptor.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing SocketDescriptor \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}
				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing SocketDescriptor \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
	}
	...
	internal class NativelyImplementedSocketDescriptor: SocketDescriptor {
		...
		public override func sendData(data: [UInt8], resumeRead: Bool) -> UInt {
			...
			let dataPrimitiveWrapper = u8slice(
				value: data, instantiationContext: "SocketDescriptor.swift::\(#function):\(#line)")
