			...
			extension Bindings {
				...
				WatchedOutput_get_block_hash(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_ThirtyTwoBytesZ(cType: nativeCallResult, instantiationContext: "WatchedOutput.swift::\(#function):\(#line)", anchor: self).getValue()
					...
					public func setBlockHash(val: [UInt8]?) {
						...
						let valOption = Option_ThirtyTwoBytesZ(some: val, instantiationContext: "WatchedOutput.swift::\(#function):\(#line)").danglingClone()
				...
				WatchedOutput_set_script_pubkey(thisPtrPointer, valVector.cType!)
					...
					public init(blockHashArg: [UInt8]?, outpointArg: OutPoint, scriptPubkeyArg: [UInt8]) {
						...
						let blockHashArgOption = Option_ThirtyTwoBytesZ(some: blockHashArg, instantiationContext: "WatchedOutput.swift::\(#function):\(#line)").danglingClone()
				
						let scriptPubkeyArgVector = Vec_u8Z(array: scriptPubkeyArg, instantiationContext: "WatchedOutput.swift::\(#function):\(#line)").dangle()
				

						// native method call
						let nativeCallResult = WatchedOutput_new(blockHashArgOption.cType!, outpointArg.dynamicallyDangledClone().cType!, scriptPubkeyArgVector.cType!)
