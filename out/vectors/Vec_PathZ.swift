			...
			extension Bindings {
				...
				internal class Vec_PathZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_PathZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
...
Path(cType: currentCType, instantiationContext: "Vec_PathZ.swift::\(#function):\(#line)", anchor: self).dangle()
						}
						return swiftArray
					}
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing Vec_PathZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
