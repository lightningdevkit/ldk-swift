...
extension Bindings {
	...
	public class Result_UpdateAddHTLCDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_UpdateAddHTLCDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: UpdateAddHTLC) -> Result_UpdateAddHTLCDecodeErrorZ {
			...
			let returnValue = Result_UpdateAddHTLCDecodeErrorZ(
				...
				instantiationContext: "Result_UpdateAddHTLCDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_UpdateAddHTLCDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_UpdateAddHTLCDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_UpdateAddHTLCDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_UpdateAddHTLCDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_UpdateAddHTLCDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_UpdateAddHTLCDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
