...
extension Bindings {
	...
	public class Result_VerifiedInvoiceRequestNoneZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_VerifiedInvoiceRequestNoneZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: VerifiedInvoiceRequest) -> Result_VerifiedInvoiceRequestNoneZ {
			...
			let returnValue = Result_VerifiedInvoiceRequestNoneZ(
				...
				instantiationContext: "Result_VerifiedInvoiceRequestNoneZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_VerifiedInvoiceRequestNoneZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_VerifiedInvoiceRequestNoneZ>) in
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_VerifiedInvoiceRequestNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_VerifiedInvoiceRequestNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
