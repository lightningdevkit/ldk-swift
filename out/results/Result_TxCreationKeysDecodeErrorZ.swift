...
extension Bindings {
	...
	public class Result_TxCreationKeysDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_TxCreationKeysDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: TxCreationKeys) -> Result_TxCreationKeysDecodeErrorZ {
			...
			let returnValue = Result_TxCreationKeysDecodeErrorZ(
				...
				instantiationContext: "Result_TxCreationKeysDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_TxCreationKeysDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_TxCreationKeysDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_TxCreationKeysDecodeErrorZ.swift::\(#function):\(#line)", anchor: self
				)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_TxCreationKeysDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_TxCreationKeysDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_TxCreationKeysDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
