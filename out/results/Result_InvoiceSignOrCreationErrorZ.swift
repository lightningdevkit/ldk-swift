...
extension Bindings {
	...
	public class Result_InvoiceSignOrCreationErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_InvoiceSignOrCreationErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: Invoice) -> Result_InvoiceSignOrCreationErrorZ {
			...
			let returnValue = Result_InvoiceSignOrCreationErrorZ(
				...
				instantiationContext: "Result_InvoiceSignOrCreationErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_InvoiceSignOrCreationErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_InvoiceSignOrCreationErrorZ>) in
		...
		internal func dangleRecursively() -> Result_InvoiceSignOrCreationErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_InvoiceSignOrCreationErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_InvoiceSignOrCreationErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
