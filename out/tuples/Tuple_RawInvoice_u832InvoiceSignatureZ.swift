...
extension Bindings {
	...
	internal class Tuple_RawInvoice_u832InvoiceSignatureZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKC3Tuple_RawInvoice_u832InvoiceSignatureZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> Tuple_RawInvoice_u832InvoiceSignatureZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKC3Tuple_RawInvoice_u832InvoiceSignatureZ>) in
			...
			let returnValue = Tuple_RawInvoice_u832InvoiceSignatureZ(
				cType: nativeCallResult,
				instantiationContext: "Tuple_RawInvoice_u832InvoiceSignatureZ.swift::\(#function):\(#line)")
		...
		public init(a: RawInvoice, b: [UInt8], c: InvoiceSignature, instantiationContext: String) {
			...
			let bPrimitiveWrapper = ThirtyTwoBytes(
				value: b, instantiationContext: "Tuple_RawInvoice_u832InvoiceSignatureZ.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult = C3Tuple_RawInvoice_u832InvoiceSignatureZ_new(
		...
		public func getC() -> InvoiceSignature {
			// return value (do some wrapping)
			let returnValue = InvoiceSignature(
				cType: self.cType!.c,
				instantiationContext: "Tuple_RawInvoice_u832InvoiceSignatureZ.swift::\(#function):\(#line)",
				anchor: self
			)
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Tuple_RawInvoice_u832InvoiceSignatureZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Tuple_RawInvoice_u832InvoiceSignatureZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
