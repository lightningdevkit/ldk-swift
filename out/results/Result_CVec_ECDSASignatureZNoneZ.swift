			...
			extension Bindings {
				...
				public class Result_CVec_ECDSASignatureZNoneZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_CVec_ECDSASignatureZNoneZ, instantiationContext: String, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(cType: LDKCResult_CVec_ECDSASignatureZNoneZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: [[UInt8]]) -> Result_CVec_ECDSASignatureZNoneZ {
						...
						let oVector = Vec_ECDSASignatureZ(array: o, instantiationContext: "Result_CVec_ECDSASignatureZNoneZ.swift::\(#function):\(#line)").dangle()
						...
						let returnValue = Result_CVec_ECDSASignatureZNoneZ(cType: nativeCallResult, instantiationContext: "Result_CVec_ECDSASignatureZNoneZ.swift::\(#function):\(#line)")
					...
					internal func clone() -> Result_CVec_ECDSASignatureZNoneZ {
						...
						withUnsafePointer(to: self.cType!) { (origPointer: UnsafePointer<LDKCResult_CVec_ECDSASignatureZNoneZ>) in
					...
					*/
					...
					public func getValue() -> [[UInt8]]? {
						if self.cType?.result_ok == true {
							return Vec_ECDSASignatureZ(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_CVec_ECDSASignatureZNoneZ.swift::\(#function):\(#line)", anchor: self).getValue()
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_CVec_ECDSASignatureZNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_CVec_ECDSASignatureZNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
