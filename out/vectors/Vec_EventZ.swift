...
extension Bindings {
	...
	internal class Vec_EventZ: NativeTypeWrapper {
		...
		internal init(cType: LDKCVec_EventZ, instantiationContext: String) {
		...
		internal init(cType: LDKCVec_EventZ, instantiationContext: String, anchor: NativeTypeWrapper) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(
			cType: LDKCVec_EventZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(array: [Event], instantiationContext: String) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			super.init(conflictAvoidingVariableName: 0, instantiationContext: instantiationContext)
		...
		public func getValue() -> [Event] {
			...
			let swiftArray = array.map { (currentCType: LDKEvent) -> Event in
				Event(
					cType: currentCType, instantiationContext: "Vec_EventZ.swift::\(#function):\(#line)", anchor: self
				)
				.dangle()
			}
			return swiftArray
		...
		internal func dangleRecursively() -> Vec_EventZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print("Freeing Vec_EventZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Vec_EventZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
