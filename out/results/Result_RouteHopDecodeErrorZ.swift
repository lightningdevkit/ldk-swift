			...
			extension Bindings {
				...
				public class Result_RouteHopDecodeErrorZ: NativeTypeWrapper {
					...
					public class func initWithOk(o: RouteHop) -> Result_RouteHopDecodeErrorZ {
						...
						let returnValue = Result_RouteHopDecodeErrorZ(cType: nativeCallResult, instantiationContext: "#{swift_class_name}::\(#function):\(#line)")
				...
				CResult_RouteHopDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> RouteHop? {
						if self.cType?.result_ok == true {
							return RouteHop(cType: self.cType!.contents.result.pointee, instantiationContext: "#{swift_class_name}::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
