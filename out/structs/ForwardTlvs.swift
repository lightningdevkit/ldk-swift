			...
			extension Bindings {
				...
				public class ForwardTlvs: NativeTypeWrapper {
					...
					internal init(cType: LDKForwardTlvs, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				ForwardTlvs_get_payment_relay(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = PaymentRelay(cType: nativeCallResult, instantiationContext: "ForwardTlvs.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				ForwardTlvs_get_payment_constraints(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = PaymentConstraints(cType: nativeCallResult, instantiationContext: "ForwardTlvs.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				ForwardTlvs_get_features(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = BlindedHopFeatures(cType: nativeCallResult, instantiationContext: "ForwardTlvs.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				ForwardTlvs_set_features(thisPtrPointer, val.dynamicallyDangledClone().cType!)
					...
					public init(shortChannelIdArg: UInt64, paymentRelayArg: PaymentRelay, paymentConstraintsArg: PaymentConstraints, featuresArg: BlindedHopFeatures) {
						// native call variable prep
						

						// native method call
						let nativeCallResult = ForwardTlvs_new(shortChannelIdArg, paymentRelayArg.dynamicallyDangledClone().cType!, paymentConstraintsArg.dynamicallyDangledClone().cType!, featuresArg.dynamicallyDangledClone().cType!)
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "ForwardTlvs.swift::\(#function):\(#line)")
				...
				ForwardTlvs_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "ForwardTlvs.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing ForwardTlvs \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
