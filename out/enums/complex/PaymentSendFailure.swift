...
extension Bindings {
	...
	public class PaymentSendFailure: NativeTypeWrapper {
		...
		internal init(
			cType: LDKPaymentSendFailure, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> PaymentSendFailure {
			...
			// return value (do some wrapping)
			let returnValue = PaymentSendFailure(
				cType: nativeCallResult, instantiationContext: "PaymentSendFailure.swift::\(#function):\(#line)")
		...
		public class func initWithPathParameterError(a: [Result_NoneAPIErrorZ]) -> PaymentSendFailure {
			...
			let aVector = Vec_CResult_NoneAPIErrorZZ(
				array: a, instantiationContext: "PaymentSendFailure.swift::\(#function):\(#line)"
			)
			.dangle()
		...
		public class func initWithPartialFailure(
			results: [Result_NoneAPIErrorZ], failedPathsRetry: Bindings.RouteParameters, paymentId: [UInt8]
		) -> PaymentSendFailure {
			// native call variable prep

			let resultsVector = Vec_CResult_NoneAPIErrorZZ(
				array: results, instantiationContext: "PaymentSendFailure.swift::\(#function):\(#line)"
			)
			.dangle()

			let paymentIdPrimitiveWrapper = ThirtyTwoBytes(
				value: paymentId, instantiationContext: "PaymentSendFailure.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult = PaymentSendFailure_partial_failure(
				resultsVector.cType!, failedPathsRetry.dynamicallyDangledClone().cType!,
		...
		public func getValueAsParameterError() -> APIError? {
			...
			return APIError(
				cType: self.cType!.parameter_error,
				instantiationContext: "PaymentSendFailure.swift::\(#function):\(#line)", anchor: self)
		}
		...
		public func getValueAsPathParameterError() -> [Result_NoneAPIErrorZ]? {
			...
			return Vec_CResult_NoneAPIErrorZZ(
				cType: self.cType!.path_parameter_error,
				instantiationContext: "PaymentSendFailure.swift::\(#function):\(#line)", anchor: self
			)
			.getValue()
		}
		...
		public func getValueAsAllFailedResendSafe() -> [APIError]? {
			...
			return Vec_APIErrorZ(
				cType: self.cType!.all_failed_resend_safe,
				instantiationContext: "PaymentSendFailure.swift::\(#function):\(#line)", anchor: self
			)
			.getValue()
		}
		...
		public func getValueAsPartialFailure() -> PartialFailure? {
			...
			return PaymentSendFailure_LDKPartialFailure_Body(
				cType: self.cType!.partial_failure,
				instantiationContext: "PaymentSendFailure.swift::\(#function):\(#line)", anchor: self)
		}
		...
		internal func dangleRecursively() -> PaymentSendFailure {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing PaymentSendFailure \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing PaymentSendFailure \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
		...
		public class PartialFailure: NativeTypeWrapper {
			...
			internal init(
				cType: LDKPaymentSendFailure_LDKPartialFailure_Body, instantiationContext: String,
			...
			public func getResults() -> [Result_NoneAPIErrorZ] {
				// return value (do some wrapping)
				let returnValue = Vec_CResult_NoneAPIErrorZZ(
					cType: self.cType!.results, instantiationContext: "PaymentSendFailure.swift::\(#function):\(#line)",
					anchor: self
				)
				.getValue()
			...
			public func getFailedPathsRetry() -> Bindings.RouteParameters {
				// return value (do some wrapping)
				let returnValue = Bindings.RouteParameters(
					cType: self.cType!.failed_paths_retry,
					instantiationContext: "PaymentSendFailure.swift::\(#function):\(#line)", anchor: self)
			...
			internal func dangleRecursively() -> PartialFailure {
				self.dangling = true
				for currentAnchor in self.anchors {
					currentAnchor.dangleRecursively()
				}
