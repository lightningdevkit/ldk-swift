	...
	def generate_trait(self, struct_name, struct_details):
		...
		for current_lambda in struct_details.lambdas:
			...
			if is_default_return_error or ALWAYS_ABORT_ON_UNIMPLEMENTED:
				swift_default_return = f'Bindings.print("Error: {swift_struct_name}::{current_lambda_name} MUST be overridden! Offending class: \(String(describing: self)). Aborting.", severity: .ERROR)\n\nabort()'
