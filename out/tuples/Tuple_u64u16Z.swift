...
extension Bindings {
	...
	internal class Tuple_u64u16Z: NativeTypeWrapper {
		...
		internal init(
			cType: LDKC2Tuple_u64u16Z, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> Tuple_u64u16Z {
			...
			let returnValue = Tuple_u64u16Z(
				cType: nativeCallResult, instantiationContext: "Tuple_u64u16Z.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Tuple_u64u16Z \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Tuple_u64u16Z \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
