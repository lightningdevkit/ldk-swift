			...
			extension Bindings {
				...
				internal class Vec_UtxoZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_UtxoZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
...
Utxo(cType: currentCType, instantiationContext: "Vec_UtxoZ.swift::\(#function):\(#line)", anchor: self).dangle()
						}
						return swiftArray
					}
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing Vec_UtxoZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
