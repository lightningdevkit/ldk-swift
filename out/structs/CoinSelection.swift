...
extension Bindings {
	...
	public class CoinSelection: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCoinSelection, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getConfirmedUtxos() -> [Utxo] {
			...
			// return value (do some wrapping)
			let returnValue = Vec_UtxoZ(
				cType: nativeCallResult, instantiationContext: "CoinSelection.swift::\(#function):\(#line)",
				anchor: self
			)
		...
		public func setConfirmedUtxos(val: [Utxo]) {
			...
			let valVector = Vec_UtxoZ(array: val, instantiationContext: "CoinSelection.swift::\(#function):\(#line)")
				.dangle()
		...
		public func setChangeOutput(val: TxOut?) {
			...
			let valOption = Option_TxOutZ(some: val, instantiationContext: "CoinSelection.swift::\(#function):\(#line)")
				.danglingClone()
		...
		public init(confirmedUtxosArg: [Utxo], changeOutputArg: TxOut?) {
			...
			let confirmedUtxosArgVector = Vec_UtxoZ(
				array: confirmedUtxosArg, instantiationContext: "CoinSelection.swift::\(#function):\(#line)"
			)
			.dangle()

			let changeOutputArgOption = Option_TxOutZ(
				some: changeOutputArg, instantiationContext: "CoinSelection.swift::\(#function):\(#line)"
			)
			.danglingClone()
			...
			super
				.init(
					conflictAvoidingVariableName: 0, instantiationContext: "CoinSelection.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing CoinSelection \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing CoinSelection \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
