...
extension Bindings {
	...
	public class Result_SocketAddressDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_SocketAddressDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: SocketAddress) -> Result_SocketAddressDecodeErrorZ {
			...
			let returnValue = Result_SocketAddressDecodeErrorZ(
				...
				instantiationContext: "Result_SocketAddressDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_SocketAddressDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_SocketAddressDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_SocketAddressDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_SocketAddressDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_SocketAddressDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
