...
extension Bindings {
	...
	public class Result_ThirtyTwoBytesPaymentErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_ThirtyTwoBytesPaymentErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: [UInt8]) -> Result_ThirtyTwoBytesPaymentErrorZ {
			...
			let oPrimitiveWrapper = ThirtyTwoBytes(
				value: o, instantiationContext: "Result_ThirtyTwoBytesPaymentErrorZ.swift::\(#function):\(#line)")
			...
			let returnValue = Result_ThirtyTwoBytesPaymentErrorZ(
				...
				instantiationContext: "Result_ThirtyTwoBytesPaymentErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_ThirtyTwoBytesPaymentErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_ThirtyTwoBytesPaymentErrorZ>) in
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_ThirtyTwoBytesPaymentErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_ThirtyTwoBytesPaymentErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
