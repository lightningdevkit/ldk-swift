...
extension Bindings {
	...
	public class Result_TxAbortDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_TxAbortDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: TxAbort) -> Result_TxAbortDecodeErrorZ {
			...
			let returnValue = Result_TxAbortDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "Result_TxAbortDecodeErrorZ.swift::\(#function):\(#line)"
			)
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_TxAbortDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_TxAbortDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_TxAbortDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
