			...
			extension Bindings {
				...
				internal class Option_NetworkUpdateZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCOption_NetworkUpdateZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				COption_NetworkUpdateZ_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_NetworkUpdateZ(cType: nativeCallResult, instantiationContext: "Option_NetworkUpdateZ.swift::\(#function):\(#line)")
					...
					public func getValue() -> NetworkUpdate? {
						...
						if self.cType!.tag == LDKCOption_NetworkUpdateZ_Some {
							return NetworkUpdate(cType: self.cType!.some, instantiationContext: "Option_NetworkUpdateZ.swift::\(#function):\(#line)", anchor: self).dangle()
						}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Option_NetworkUpdateZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Option_NetworkUpdateZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
