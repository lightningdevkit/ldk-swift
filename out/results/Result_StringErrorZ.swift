			...
			extension Bindings {
				...
				public class Result_StringErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_StringErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: String) -> Result_StringErrorZ {
						...
						let returnValue = Result_StringErrorZ(cType: nativeCallResult, instantiationContext: "Result_StringErrorZ.swift::\(#function):\(#line)")
				...
				CResult_StringErrorZ_clone(origPointer)
					...
					public func getValue() -> String? {
						if self.cType?.result_ok == true {
							return Str(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_StringErrorZ.swift::\(#function):\(#line)", anchor: self).getValue()
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_StringErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_StringErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
