...
extension Bindings {
	...
	public class Result_RevokeAndACKDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_RevokeAndACKDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: RevokeAndACK) -> Result_RevokeAndACKDecodeErrorZ {
			...
			let returnValue = Result_RevokeAndACKDecodeErrorZ(
				...
				instantiationContext: "Result_RevokeAndACKDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_RevokeAndACKDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_RevokeAndACKDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_RevokeAndACKDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_RevokeAndACKDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_RevokeAndACKDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_RevokeAndACKDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
