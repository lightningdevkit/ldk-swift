...
extension Bindings {
	...
	public class Result_SiPrefixParseErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_SiPrefixParseErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: SiPrefix) -> Result_SiPrefixParseErrorZ {
			...
			let returnValue = Result_SiPrefixParseErrorZ(
				cType: nativeCallResult, instantiationContext: "Result_SiPrefixParseErrorZ.swift::\(#function):\(#line)"
			)
		...
		public func getError() -> ParseError? {
			if self.cType?.result_ok == false {
				return ParseError(
					...
					instantiationContext: "Result_SiPrefixParseErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_SiPrefixParseErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_SiPrefixParseErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_SiPrefixParseErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
