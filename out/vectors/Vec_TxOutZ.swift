...
extension Bindings {
	...
	internal class Vec_TxOutZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_TxOutZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getValue() -> [TxOut] {
			...
			let swiftArray = array.map { (currentCType: LDKTxOut) -> TxOut in
				TxOut(
					cType: currentCType, instantiationContext: "Vec_TxOutZ.swift::\(#function):\(#line)", anchor: self
				)
				.dangle()
			}
			return swiftArray
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print("Freeing Vec_TxOutZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Vec_TxOutZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
