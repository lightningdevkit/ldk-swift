			...
			extension Bindings {
				...
				public class Refund: NativeTypeWrapper {
					...
					internal init(cType: LDKRefund, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				Refund_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Refund(cType: nativeCallResult)
				...
				Refund_features(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = InvoiceRequestFeatures(cType: nativeCallResult, anchor: self).dangle(false)
				...
				Refund_write(objPointer)
					...
					deinit {
						...
						if !self.dangling {
							Bindings.print("Freeing Refund \(self.instanceNumber).")
							
							self.free()
						} else {
							Bindings.print("Not freeing Refund \(self.instanceNumber) due to dangle.")
						}
					}
