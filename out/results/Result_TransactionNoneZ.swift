			...
			extension Bindings {
				...
				public class Result_TransactionNoneZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_TransactionNoneZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					*/
					...
					public func getValue() -> [UInt8]? {
						if self.cType?.result_ok == true {
							return Transaction(cType: self.cType!.contents.result.pointee, anchor: self).getValue()
						}
						return nil
					}
