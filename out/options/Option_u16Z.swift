...
extension Bindings {
	...
	internal class Option_u16Z: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCOption_u16Z, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> Option_u16Z {
			...
			let returnValue = Option_u16Z(
				cType: nativeCallResult, instantiationContext: "Option_u16Z.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> Option_u16Z {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print("Freeing Option_u16Z \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Option_u16Z \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
