			...
			extension Bindings {
				...
				internal class Option_C2Tuple_u64u16ZZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCOption_C2Tuple_u64u16ZZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(some: (UInt64, UInt16)?, instantiationContext: String) {
						...
						if let some = some {
							...
							let someTuple = Tuple_u64u16Z(tuple: some, instantiationContext: "Option_C2Tuple_u64u16ZZ.swift::\(#function):\(#line)").danglingClone()
				...
				COption_C2Tuple_u64u16ZZ_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_C2Tuple_u64u16ZZ(cType: nativeCallResult, instantiationContext: "Option_C2Tuple_u64u16ZZ.swift::\(#function):\(#line)")
					...
					public func getValue() -> (UInt64, UInt16)? {
						...
						if self.cType!.tag == LDKCOption_C2Tuple_u64u16ZZ_Some {
							return Tuple_u64u16Z(cType: self.cType!.some, instantiationContext: "Option_C2Tuple_u64u16ZZ.swift::\(#function):\(#line)", anchor: self).dangle().getValue()
						}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Option_C2Tuple_u64u16ZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Option_C2Tuple_u64u16ZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
