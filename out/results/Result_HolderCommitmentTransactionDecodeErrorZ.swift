			...
			extension Bindings {
				...
				public class Result_HolderCommitmentTransactionDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_HolderCommitmentTransactionDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: HolderCommitmentTransaction) -> Result_HolderCommitmentTransactionDecodeErrorZ {
						...
						let returnValue = Result_HolderCommitmentTransactionDecodeErrorZ(cType: nativeCallResult, instantiationContext: "Result_HolderCommitmentTransactionDecodeErrorZ.swift::\(#function):\(#line)")
				...
				CResult_HolderCommitmentTransactionDecodeErrorZ_clone(origPointer)
					...
					public func getError() -> DecodeError? {
						if self.cType?.result_ok == false {
							return DecodeError(cType: self.cType!.contents.err.pointee, instantiationContext: "Result_HolderCommitmentTransactionDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					public func getValue() -> HolderCommitmentTransaction? {
						if self.cType?.result_ok == true {
							return HolderCommitmentTransaction(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_HolderCommitmentTransactionDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_HolderCommitmentTransactionDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_HolderCommitmentTransactionDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
