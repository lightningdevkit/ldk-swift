...
extension Bindings {
	...
	internal class Vec_UpdateFailHTLCZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_UpdateFailHTLCZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func getValue() -> [UpdateFailHTLC] {
			...
			let swiftArray = array.map { (currentCType: LDKUpdateFailHTLC) -> UpdateFailHTLC in
				UpdateFailHTLC(
					cType: currentCType, instantiationContext: "Vec_UpdateFailHTLCZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		internal func dangleRecursively() -> Vec_UpdateFailHTLCZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_UpdateFailHTLCZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Vec_UpdateFailHTLCZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
