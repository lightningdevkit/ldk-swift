			...
			extension Bindings {
				...
				open class BroadcasterInterface: NativeTraitWrapper {
					...
					internal init(cType: LDKBroadcasterInterface, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public init() {
						...
						func broadcastTransactionsLambda(this_arg: UnsafeRawPointer?, txs: LDKCVec_TransactionZ) -> Void {
							let instance: BroadcasterInterface = Bindings.pointerToInstance(pointer: this_arg!, sourceMarker: "BroadcasterInterface::broadcastTransactionsLambda")

							// Swift callback variable prep
											

							// Swift callback call
							let swiftCallbackResult = instance.broadcastTransactions(txs: Vec_TransactionZ(cType: txs, instantiationContext: "BroadcasterInterface.swift::init()::\(#function):\(#line)").getValue())
			...
			open func broadcastTransactions(txs: [[UInt8]]) -> Void {
				
				Bindings.print("Error: BroadcasterInterface::broadcastTransactions MUST be overridden! Offending class: \(String(describing: self)). Aborting.", severity: .ERROR)
				abort()
			}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing BroadcasterInterface \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing BroadcasterInterface \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
				}
			...
			public override func broadcastTransactions(txs: [[UInt8]]) {
				// native call variable prep
						...
						let txsVector = Vec_TransactionZ(array: txs, instantiationContext: "BroadcasterInterface.swift::\(#function):\(#line)").dangle()
