...
extension Bindings {
	...
	public class Result_NoneErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_NoneErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk() -> Result_NoneErrorZ {
			...
			let returnValue = Result_NoneErrorZ(
				cType: nativeCallResult, instantiationContext: "Result_NoneErrorZ.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> Result_NoneErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_NoneErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Result_NoneErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
