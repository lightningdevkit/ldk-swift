			...
			extension Bindings {
				...
				Hostname_len(thisArgPointer)
					...
					public func write() -> [UInt8] {
						...
						// native method call
						let nativeCallResult = 
						withUnsafePointer(to: self.cType!) { (objPointer: UnsafePointer<LDKHostname>) in
				Hostname_write(objPointer)
						}
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "Hostname.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public class func read(ser: [UInt8]) -> Result_HostnameDecodeErrorZ {
						// native call variable prep
						
						let serPrimitiveWrapper = u8slice(value: ser, instantiationContext: "Hostname.swift::\(#function):\(#line)")
				

						// native method call
						let nativeCallResult = Hostname_read(serPrimitiveWrapper.cType!)
						...
						// return value (do some wrapping)
						let returnValue = Result_HostnameDecodeErrorZ(cType: nativeCallResult, instantiationContext: "Hostname.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing Hostname \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
