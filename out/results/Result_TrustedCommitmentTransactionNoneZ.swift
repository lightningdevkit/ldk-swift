...
extension Bindings {
	...
	public class Result_TrustedCommitmentTransactionNoneZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_TrustedCommitmentTransactionNoneZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(
			cType: LDKCResult_TrustedCommitmentTransactionNoneZ, instantiationContext: String,
		...
		public class func initWithOk(o: TrustedCommitmentTransaction) -> Result_TrustedCommitmentTransactionNoneZ {
			...
			let returnValue = Result_TrustedCommitmentTransactionNoneZ(
				...
				instantiationContext: "Result_TrustedCommitmentTransactionNoneZ.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> Result_TrustedCommitmentTransactionNoneZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_TrustedCommitmentTransactionNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_TrustedCommitmentTransactionNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
