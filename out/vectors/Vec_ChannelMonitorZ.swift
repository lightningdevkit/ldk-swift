			...
			extension Bindings {
				...
				internal class Vec_ChannelMonitorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_ChannelMonitorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
...
ChannelMonitor(cType: currentCType, instantiationContext: "Vec_ChannelMonitorZ.swift::\(#function):\(#line)", anchor: self).dangle()
						}
						return swiftArray
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Vec_ChannelMonitorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Vec_ChannelMonitorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
