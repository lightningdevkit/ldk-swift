			...
			extension Bindings {
				...
				public class BlindedPayInfo: NativeTypeWrapper {
					...
					internal init(cType: LDKBlindedPayInfo) {
					...
					internal init(cType: LDKBlindedPayInfo, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				BlindedPayInfo_get_features(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = BlindedHopFeatures(cType: nativeCallResult, anchor: self).dangle(false)
				...
				self.initialCFreeability = nativeCallResult.is_owned
						...
						/*
						// return value (do some wrapping)
						let returnValue = BlindedPayInfo(cType: nativeCallResult)
						*/
				...
				BlindedPayInfo_write(objPointer)
					...
					deinit {
						...
						if !self.dangling {
							Bindings.print("Freeing BlindedPayInfo \(self.instanceNumber).")
							
							self.free()
						} else {
							Bindings.print("Not freeing BlindedPayInfo \(self.instanceNumber) due to dangle.")
						}
					}
