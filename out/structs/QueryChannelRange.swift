			...
			extension Bindings {
				...
				public class QueryChannelRange: NativeTypeWrapper {
					...
					internal init(cType: LDKQueryChannelRange, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing QueryChannelRange \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing QueryChannelRange \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
