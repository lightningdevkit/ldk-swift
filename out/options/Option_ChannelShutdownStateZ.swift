...
extension Bindings {
	...
	internal class Option_ChannelShutdownStateZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCOption_ChannelShutdownStateZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> Option_ChannelShutdownStateZ {
			...
			let returnValue = Option_ChannelShutdownStateZ(
				...
				instantiationContext: "Option_ChannelShutdownStateZ.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Option_ChannelShutdownStateZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Option_ChannelShutdownStateZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
