			...
			extension Bindings {
				...
				public class PeerManager: NativeTypeWrapper {
					...
					public init(messageHandler: MessageHandler, currentTime: UInt32, ephemeralRandomData: [UInt8], logger: Logger, nodeSigner: NodeSigner) {
						// native call variable prep
				...
				PeerManager_new(messageHandler.dangle().cType!, currentTime, tupledEphemeralRandomDataPointer, logger.activate().cType!, nodeSigner.activate().cType!)
						}
				...
				PeerManager_get_peer_node_ids(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_C2Tuple_PublicKeyCOption_SocketAddressZZZ(cType: nativeCallResult, instantiationContext: "PeerManager.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public func newOutboundConnection(theirNodeId: [UInt8], descriptor: SocketDescriptor, remoteNetworkAddress: SocketAddress?) -> Result_CVec_u8ZPeerHandleErrorZ {
						// native call variable prep
						
						let theirNodeIdPrimitiveWrapper = PublicKey(value: theirNodeId, instantiationContext: "PeerManager.swift::\(#function):\(#line)")
				
						let remoteNetworkAddressOption = Option_SocketAddressZ(some: remoteNetworkAddress, instantiationContext: "PeerManager.swift::\(#function):\(#line)").danglingClone()
				...
				PeerManager_new_outbound_connection(thisArgPointer, theirNodeIdPrimitiveWrapper.cType!, descriptor.activate().cType!, remoteNetworkAddressOption.cType!)
					...
					/// 
					/// [`socket_disconnected`]: PeerManager::socket_disconnected
					public func newInboundConnection(descriptor: SocketDescriptor, remoteNetworkAddress: SocketAddress?) -> Result_NonePeerHandleErrorZ {
						// native call variable prep
				...
				PeerManager_timer_tick_occurred(thisArgPointer)
					...
					public func broadcastNodeAnnouncement(rgb: [UInt8], alias: [UInt8], addresses: [SocketAddress]) {
						...
						let addressesVector = Vec_SocketAddressZ(array: addresses, instantiationContext: "PeerManager.swift::\(#function):\(#line)").dangle()
