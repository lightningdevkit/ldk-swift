			...
			extension Bindings {
				...
				internal class Tuple_CVec_u8ZusizeZ: NativeTypeWrapper {
					...
					internal init(cType: LDKC2Tuple_CVec_u8ZusizeZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				C2Tuple_CVec_u8ZusizeZ_clone(origPointer)
					...
					public init(a: [UInt8], b: UInt, instantiationContext: String) {
						...
						let aVector = Vec_u8Z(array: a, instantiationContext: "Tuple_CVec_u8ZusizeZ.swift::\(#function):\(#line)").dangle()
				...
				super.init(conflictAvoidingVariableName: 0, instantiationContext: instantiationContext)
					...
					public func getA() -> [UInt8] {
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: self.cType!.a, instantiationContext: "Tuple_CVec_u8ZusizeZ.swift::\(#function):\(#line)", anchor: self).dangle().getValue()
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Tuple_CVec_u8ZusizeZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Tuple_CVec_u8ZusizeZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
