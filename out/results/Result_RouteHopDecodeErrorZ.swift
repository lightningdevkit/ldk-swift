			...
			extension Bindings {
				...
				public class Result_RouteHopDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_RouteHopDecodeErrorZ) {
					...
					internal init(cType: LDKCResult_RouteHopDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_RouteHopDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> RouteHop? {
						if self.cType?.result_ok == true {
							return RouteHop(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
