			...
			extension Bindings {
				...
				TrustedCommitmentTransaction_channel_type_features(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = ChannelTypeFeatures(cType: nativeCallResult, instantiationContext: "TrustedCommitmentTransaction.swift::\(#function):\(#line)", anchor: self).dangle(false)
					...
					public func getHtlcSigs(htlcBaseKey: [UInt8], channelParameters: DirectedChannelTransactionParameters, entropySource: EntropySource) -> Result_CVec_ECDSASignatureZNoneZ {
						// native call variable prep
				...
				TrustedCommitmentTransaction_get_htlc_sigs(thisArgPointer, tupledHtlcBaseKeyPointer, channelParametersPointer, entropySourcePointer)
						...
						// return value (do some wrapping)
						let returnValue = Result_CVec_ECDSASignatureZNoneZ(cType: nativeCallResult, instantiationContext: "TrustedCommitmentTransaction.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				TrustedCommitmentTransaction_revokeable_output_index(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_usizeZ(cType: nativeCallResult, instantiationContext: "TrustedCommitmentTransaction.swift::\(#function):\(#line)", anchor: self).getValue()
					...
					public func buildToLocalJusticeTx(feeratePerKw: UInt64, destinationScript: [UInt8]) -> Result_TransactionNoneZ {
						...
						let destinationScriptVector = Vec_u8Z(array: destinationScript, instantiationContext: "TrustedCommitmentTransaction.swift::\(#function):\(#line)").dangle()
				

						// native method call
						let nativeCallResult = 
						withUnsafePointer(to: self.cType!) { (thisArgPointer: UnsafePointer<LDKTrustedCommitmentTransaction>) in
				TrustedCommitmentTransaction_build_to_local_justice_tx(thisArgPointer, feeratePerKw, destinationScriptVector.cType!)
						}
						...
						// return value (do some wrapping)
						let returnValue = Result_TransactionNoneZ(cType: nativeCallResult, instantiationContext: "TrustedCommitmentTransaction.swift::\(#function):\(#line)", anchor: self).dangle(false)
