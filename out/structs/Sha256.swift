...
extension Bindings {
	...
	public class Sha256: NativeTypeWrapper {
		...
		internal init(cType: LDKSha256, instantiationContext: String) {
		...
		internal init(cType: LDKSha256, instantiationContext: String, anchor: NativeTypeWrapper) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
			self.initialCFreeability = self.cType!.is_owned
		...
		internal init(cType: LDKSha256, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
			self.initialCFreeability = self.cType!.is_owned
		...
		internal func clone() -> Sha256 {
			...
			let returnValue = Sha256(
				cType: nativeCallResult, instantiationContext: "Sha256.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print("Freeing Sha256 \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Sha256 \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
