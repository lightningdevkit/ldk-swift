...
extension Bindings {
	...
	internal class Option_HTLCDestinationZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCOption_HTLCDestinationZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> Option_HTLCDestinationZ {
			...
			// return value (do some wrapping)
			let returnValue = Option_HTLCDestinationZ(
				cType: nativeCallResult, instantiationContext: "Option_HTLCDestinationZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> HTLCDestination? {
			...
			if self.cType!.tag == LDKCOption_HTLCDestinationZ_Some {
				return HTLCDestination(
					cType: self.cType!.some,
					instantiationContext: "Option_HTLCDestinationZ.swift::\(#function):\(#line)", anchor: self
				)
				.dangle()
			}
		...
		internal func dangleRecursively() -> Option_HTLCDestinationZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Option_HTLCDestinationZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Option_HTLCDestinationZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
