...
extension Bindings {
	...
	public class ChannelReestablish: NativeTypeWrapper {
		...
		internal init(
			cType: LDKChannelReestablish, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> ChannelReestablish {
			...
			// return value (do some wrapping)
			let returnValue = ChannelReestablish(
				cType: nativeCallResult, instantiationContext: "ChannelReestablish.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKChannelReestablish>) in
					ChannelReestablish_hash(oPointer)
				}
		...
		public class func read(ser: [UInt8]) -> Result_ChannelReestablishDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "ChannelReestablish.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing ChannelReestablish \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing ChannelReestablish \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
