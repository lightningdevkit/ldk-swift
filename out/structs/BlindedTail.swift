			...
			extension Bindings {
				...
				public class BlindedTail: NativeTypeWrapper {
					...
					internal init(cType: LDKBlindedTail) {
					...
					internal init(cType: LDKBlindedTail, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				BlindedTail_get_hops(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_BlindedHopZ(cType: nativeCallResult, anchor: self).dangle(false).getValue()
					...
					public func setHops(val: [BlindedHop]) {
						...
						let valVector = Vec_BlindedHopZ(array: val).dangle()
				...
				BlindedTail_set_final_value_msat(thisPtrPointer, val)
					...
					public init(hopsArg: [BlindedHop], blindingPointArg: [UInt8], excessFinalCltvExpiryDeltaArg: UInt32, finalValueMsatArg: UInt64) {
						...
						let hopsArgVector = Vec_BlindedHopZ(array: hopsArg).dangle()
				
						let blindingPointArgPrimitiveWrapper = PublicKey(value: blindingPointArg)
				...
				self.initialCFreeability = nativeCallResult.is_owned
						...
						/*
						// return value (do some wrapping)
						let returnValue = BlindedTail(cType: nativeCallResult)
						*/
				...
				BlindedTail_write(objPointer)
					...
					public class func read(ser: [UInt8]) -> Result_BlindedTailDecodeErrorZ {
						...
						// return value (do some wrapping)
						let returnValue = Result_BlindedTailDecodeErrorZ(cType: nativeCallResult)
					...
					deinit {
						...
						if !self.dangling {
							Bindings.print("Freeing BlindedTail \(self.instanceNumber).")
							
							self.free()
						} else {
							Bindings.print("Not freeing BlindedTail \(self.instanceNumber) due to dangle.")
						}
					}
