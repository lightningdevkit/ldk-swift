			...
			extension Bindings {
				...
				BuiltCommitmentTransaction_sign_counterparty_commitment(thisArgPointer, tupledFundingKeyPointer, fundingRedeemscriptPrimitiveWrapper.cType!, channelValueSatoshis)
						...
						// return value (do some wrapping)
						let returnValue = ECDSASignature(cType: nativeCallResult, instantiationContext: "BuiltCommitmentTransaction.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
