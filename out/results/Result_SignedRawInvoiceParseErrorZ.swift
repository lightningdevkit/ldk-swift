			...
			extension Bindings {
				...
				public class Result_SignedRawInvoiceParseErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_SignedRawInvoiceParseErrorZ, instantiationContext: String, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(cType: LDKCResult_SignedRawInvoiceParseErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: SignedRawInvoice) -> Result_SignedRawInvoiceParseErrorZ {
						...
						let returnValue = Result_SignedRawInvoiceParseErrorZ(cType: nativeCallResult, instantiationContext: "Result_SignedRawInvoiceParseErrorZ.swift::\(#function):\(#line)")
				...
				CResult_SignedRawInvoiceParseErrorZ_clone(origPointer)
					...
					public func getError() -> ParseError? {
						if self.cType?.result_ok == false {
							return ParseError(cType: self.cType!.contents.err.pointee, instantiationContext: "Result_SignedRawInvoiceParseErrorZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					public func getValue() -> SignedRawInvoice? {
						if self.cType?.result_ok == true {
							return SignedRawInvoice(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_SignedRawInvoiceParseErrorZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_SignedRawInvoiceParseErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_SignedRawInvoiceParseErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
