...
extension Bindings {
	...
	public class Pong: NativeTypeWrapper {
		...
		internal init(cType: LDKPong, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
			self.initialCFreeability = self.cType!.is_owned
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKPong>) in
					Pong_hash(oPointer)
				}
		...
		public func write() -> [UInt8] {
			...
			let returnValue = Vec_u8Z(
				cType: nativeCallResult, instantiationContext: "Pong.swift::\(#function):\(#line)", anchor: self
			)
		...
		public class func read(ser: [UInt8]) -> Result_PongDecodeErrorZ {
			...
			let returnValue = Result_PongDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "Pong.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print("Freeing Pong \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Pong \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
