...
extension Bindings {
	...
	public class Result_RouteDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_RouteDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: Route) -> Result_RouteDecodeErrorZ {
			...
			let returnValue = Result_RouteDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "Result_RouteDecodeErrorZ.swift::\(#function):\(#line)")
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_RouteDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_RouteDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_RouteDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_RouteDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
