...
extension Bindings {
	...
	public class Result_NonePeerHandleErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_NonePeerHandleErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk() -> Result_NonePeerHandleErrorZ {
			...
			let returnValue = Result_NonePeerHandleErrorZ(
				...
				instantiationContext: "Result_NonePeerHandleErrorZ.swift::\(#function):\(#line)")
		...
		public func getError() -> PeerHandleError? {
			if self.cType?.result_ok == false {
				return PeerHandleError(
					...
					instantiationContext: "Result_NonePeerHandleErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_NonePeerHandleErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_NonePeerHandleErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_NonePeerHandleErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
