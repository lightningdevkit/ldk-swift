			...
			extension Bindings {
				...
				public class Bolt12ParseError: NativeTypeWrapper {
					...
					internal init(cType: LDKBolt12ParseError, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				Bolt12ParseError_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Bolt12ParseError(cType: nativeCallResult, instantiationContext: "Bolt12ParseError.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Bolt12ParseError \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Bolt12ParseError \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
