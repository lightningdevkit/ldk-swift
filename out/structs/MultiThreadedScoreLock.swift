			...
			extension Bindings {
				...
				public class MultiThreadedScoreLock: NativeTypeWrapper {
					...
					internal init(cType: LDKMultiThreadedScoreLock, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				MultiThreadedScoreLock_as_Score(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = NativelyImplementedScore(cType: nativeCallResult, instantiationContext: "MultiThreadedScoreLock.swift::\(#function):\(#line)", anchor: self)
				...
				MultiThreadedScoreLock_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "MultiThreadedScoreLock.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing MultiThreadedScoreLock \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing MultiThreadedScoreLock \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
