...
extension Bindings {
	...
	public class MinFinalCltvExpiryDelta: NativeTypeWrapper {
		...
		internal init(
			cType: LDKMinFinalCltvExpiryDelta, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func setA(val: UInt64) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKMinFinalCltvExpiryDelta>) in
		...
		public init(aArg: UInt64) {
			...
			super
				.init(
					...
					instantiationContext: "MinFinalCltvExpiryDelta.swift::\(#function):\(#line)")
		...
		internal func clone() -> MinFinalCltvExpiryDelta {
			...
			let returnValue = MinFinalCltvExpiryDelta(
				cType: nativeCallResult, instantiationContext: "MinFinalCltvExpiryDelta.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing MinFinalCltvExpiryDelta \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing MinFinalCltvExpiryDelta \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
