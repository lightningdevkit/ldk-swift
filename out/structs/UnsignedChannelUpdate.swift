...
extension Bindings {
	...
	public class UnsignedChannelUpdate: NativeTypeWrapper {
		...
		internal init(
			cType: LDKUnsignedChannelUpdate, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func setChainHash(val: [UInt8]) {
			...
			let valPrimitiveWrapper = ThirtyTwoBytes(
				value: val, instantiationContext: "UnsignedChannelUpdate.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKUnsignedChannelUpdate>) in
		...
		public func getExcessData() -> [UInt8] {
			...
			let returnValue = Vec_u8Z(
				cType: nativeCallResult, instantiationContext: "UnsignedChannelUpdate.swift::\(#function):\(#line)",
				anchor: self
			)
		...
		public func setExcessData(val: [UInt8]) {
			...
			let valVector = Vec_u8Z(
				array: val, instantiationContext: "UnsignedChannelUpdate.swift::\(#function):\(#line)"
			)
			.dangle()
		...
		public init(
			chainHashArg: [UInt8], shortChannelIdArg: UInt64, timestampArg: UInt32, flagsArg: UInt8,
			cltvExpiryDeltaArg: UInt16, htlcMinimumMsatArg: UInt64, htlcMaximumMsatArg: UInt64, feeBaseMsatArg: UInt32,
			feeProportionalMillionthsArg: UInt32, excessDataArg: [UInt8]
		) {
			// native call variable prep

			let chainHashArgPrimitiveWrapper = ThirtyTwoBytes(
				value: chainHashArg, instantiationContext: "UnsignedChannelUpdate.swift::\(#function):\(#line)")

			let excessDataArgVector = Vec_u8Z(
				array: excessDataArg, instantiationContext: "UnsignedChannelUpdate.swift::\(#function):\(#line)"
			)
			.dangle()


			// native method call
			let nativeCallResult = UnsignedChannelUpdate_new(
				chainHashArgPrimitiveWrapper.cType!, shortChannelIdArg, timestampArg, flagsArg, cltvExpiryDeltaArg,
				htlcMinimumMsatArg, htlcMaximumMsatArg, feeBaseMsatArg, feeProportionalMillionthsArg,
			...
			super
				.init(
					...
					instantiationContext: "UnsignedChannelUpdate.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKUnsignedChannelUpdate>) in
					UnsignedChannelUpdate_hash(oPointer)
				}
		...
		public class func read(ser: [UInt8]) -> Result_UnsignedChannelUpdateDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "UnsignedChannelUpdate.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_UnsignedChannelUpdateDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "UnsignedChannelUpdate.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing UnsignedChannelUpdate \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing UnsignedChannelUpdate \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
