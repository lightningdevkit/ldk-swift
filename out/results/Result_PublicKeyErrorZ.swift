			...
			extension Bindings {
				...
				public class Result_PublicKeyErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_PublicKeyErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: [UInt8]) -> Result_PublicKeyErrorZ {
						...
						let oPrimitiveWrapper = PublicKey(value: o, instantiationContext: "Result_PublicKeyErrorZ.swift::\(#function):\(#line)")
						...
						let returnValue = Result_PublicKeyErrorZ(cType: nativeCallResult, instantiationContext: "Result_PublicKeyErrorZ.swift::\(#function):\(#line)")
				...
				CResult_PublicKeyErrorZ_clone(origPointer)
					...
					public func getValue() -> [UInt8]? {
						if self.cType?.result_ok == true {
							return PublicKey(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_PublicKeyErrorZ.swift::\(#function):\(#line)", anchor: self).getValue()
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_PublicKeyErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_PublicKeyErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
