			...
			extension Bindings {
				...
				ChannelTransactionParameters_get_channel_type_features(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = ChannelTypeFeatures(cType: nativeCallResult, instantiationContext: "ChannelTransactionParameters.swift::\(#function):\(#line)", anchor: self).dangle(false)
					...
					public func setChannelTypeFeatures(val: ChannelTypeFeatures) {
						...
						let nativeCallResult = 
						withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKChannelTransactionParameters>) in
				ChannelTransactionParameters_set_channel_type_features(thisPtrPointer, val.dynamicallyDangledClone().cType!)
						}
