			...
			extension Bindings {
				...
				PaymentPurpose_clone(origPointer)
					...
					public class func initWithInvoicePayment(paymentPreimage: [UInt8]?, paymentSecret: [UInt8]) -> PaymentPurpose {
						...
						let paymentPreimageOption = Option_ThirtyTwoBytesZ(some: paymentPreimage, instantiationContext: "PaymentPurpose.swift::\(#function):\(#line)").danglingClone()
						...
						// native method call
						let nativeCallResult = PaymentPurpose_invoice_payment(paymentPreimageOption.cType!, paymentSecretPrimitiveWrapper.cType!)
				...
				PaymentPurpose_write(objPointer)
					...
					public class InvoicePayment: NativeTypeWrapper {
						...
						public func getPaymentPreimage() -> [UInt8]? {
							// return value (do some wrapping)
							let returnValue = Option_ThirtyTwoBytesZ(cType: self.cType!.payment_preimage, instantiationContext: "PaymentPurpose.swift::\(#function):\(#line)", anchor: self).getValue()
