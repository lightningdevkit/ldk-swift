			...
			extension Bindings {
				...
				public class Result_ErrorMessageDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_ErrorMessageDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_ErrorMessageDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> ErrorMessage? {
						if self.cType?.result_ok == true {
							return ErrorMessage(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
