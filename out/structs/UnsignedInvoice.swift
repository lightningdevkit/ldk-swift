...
extension Bindings {
	...
	public class UnsignedInvoice: NativeTypeWrapper {
		...
		internal init(
			cType: LDKUnsignedInvoice, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func signingPubkey() -> [UInt8] {
			...
			let returnValue = PublicKey(
				cType: nativeCallResult, instantiationContext: "UnsignedInvoice.swift::\(#function):\(#line)",
				anchor: self
			)
		...
		internal func dangleRecursively() -> UnsignedInvoice {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing UnsignedInvoice \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing UnsignedInvoice \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
