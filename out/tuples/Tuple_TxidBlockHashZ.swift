...
extension Bindings {
	...
	internal class Tuple_TxidBlockHashZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKC2Tuple_TxidBlockHashZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> Tuple_TxidBlockHashZ {
			...
			let returnValue = Tuple_TxidBlockHashZ(
				cType: nativeCallResult, instantiationContext: "Tuple_TxidBlockHashZ.swift::\(#function):\(#line)")
		...
		public init(a: [UInt8], b: [UInt8], instantiationContext: String) {
			...
			let aPrimitiveWrapper = ThirtyTwoBytes(
				value: a, instantiationContext: "Tuple_TxidBlockHashZ.swift::\(#function):\(#line)")

			let bPrimitiveWrapper = ThirtyTwoBytes(
				value: b, instantiationContext: "Tuple_TxidBlockHashZ.swift::\(#function):\(#line)")
		...
		public func getA() -> [UInt8] {
			...
			let returnValue = ThirtyTwoBytes(
				cType: self.cType!.a, instantiationContext: "Tuple_TxidBlockHashZ.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle().getValue()
		...
		public func getB() -> [UInt8] {
			// return value (do some wrapping)
			let returnValue = ThirtyTwoBytes(
				cType: self.cType!.b, instantiationContext: "Tuple_TxidBlockHashZ.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle().getValue()
		...
		internal func dangleRecursively() -> Tuple_TxidBlockHashZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Tuple_TxidBlockHashZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Tuple_TxidBlockHashZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
