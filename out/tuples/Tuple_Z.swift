			...
			extension Bindings {
				...
				internal class Tuple_Z: NativeTypeWrapper {
					...
					internal init(cType: LDKC2Tuple_Z, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public init(a: [UInt16], b: [UInt16], instantiationContext: String) {
						...
						let aPrimitiveWrapper = ThirtyTwoU16s(value: a, instantiationContext: "Tuple_Z.swift::\(#function):\(#line)")
				
						let bPrimitiveWrapper = ThirtyTwoU16s(value: b, instantiationContext: "Tuple_Z.swift::\(#function):\(#line)")
				...
				super.init(conflictAvoidingVariableName: 0, instantiationContext: instantiationContext)
					...
					public func getA() -> [UInt16] {
						// return value (do some wrapping)
						let returnValue = ThirtyTwoU16s(cType: self.cType!.a, instantiationContext: "Tuple_Z.swift::\(#function):\(#line)", anchor: self).dangle().getValue()
					...
					public func getB() -> [UInt16] {
						// return value (do some wrapping)
						let returnValue = ThirtyTwoU16s(cType: self.cType!.b, instantiationContext: "Tuple_Z.swift::\(#function):\(#line)", anchor: self).dangle().getValue()
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing Tuple_Z \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
