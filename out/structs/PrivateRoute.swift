...
extension Bindings {
	...
	public class PrivateRoute: NativeTypeWrapper {
		...
		internal init(
			cType: LDKPrivateRoute, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> PrivateRoute {
			...
			// return value (do some wrapping)
			let returnValue = PrivateRoute(
				cType: nativeCallResult, instantiationContext: "PrivateRoute.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing PrivateRoute \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing PrivateRoute \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
