			...
			extension Bindings {
				...
				public class BlindedTail: NativeTypeWrapper {
					...
					internal init(cType: LDKBlindedTail, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				BlindedTail_get_hops(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_BlindedHopZ(cType: nativeCallResult, instantiationContext: "BlindedTail.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public func setHops(val: [BlindedHop]) {
						...
						let valVector = Vec_BlindedHopZ(array: val, instantiationContext: "BlindedTail.swift::\(#function):\(#line)").dangle()
				...
				BlindedTail_get_blinding_point(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = PublicKey(cType: nativeCallResult, instantiationContext: "BlindedTail.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public func setBlindingPoint(val: [UInt8]) {
						...
						let valPrimitiveWrapper = PublicKey(value: val, instantiationContext: "BlindedTail.swift::\(#function):\(#line)")
				...
				BlindedTail_set_final_value_msat(thisPtrPointer, val)
					...
					public init(hopsArg: [BlindedHop], blindingPointArg: [UInt8], excessFinalCltvExpiryDeltaArg: UInt32, finalValueMsatArg: UInt64) {
						// native call variable prep
						
						let hopsArgVector = Vec_BlindedHopZ(array: hopsArg, instantiationContext: "BlindedTail.swift::\(#function):\(#line)").dangle()
				
						let blindingPointArgPrimitiveWrapper = PublicKey(value: blindingPointArg, instantiationContext: "BlindedTail.swift::\(#function):\(#line)")
				

						// native method call
						let nativeCallResult = BlindedTail_new(hopsArgVector.cType!, blindingPointArgPrimitiveWrapper.cType!, excessFinalCltvExpiryDeltaArg, finalValueMsatArg)
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "BlindedTail.swift::\(#function):\(#line)")
				...
				BlindedTail_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "BlindedTail.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public class func read(ser: [UInt8]) -> Result_BlindedTailDecodeErrorZ {
						...
						let serPrimitiveWrapper = u8slice(value: ser, instantiationContext: "BlindedTail.swift::\(#function):\(#line)")
						...
						// return value (do some wrapping)
						let returnValue = Result_BlindedTailDecodeErrorZ(cType: nativeCallResult, instantiationContext: "BlindedTail.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing BlindedTail \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
