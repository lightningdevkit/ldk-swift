...
extension Bindings {
	...
	public class Hints: NativeTypeWrapper {
		...
		public class func initWithBlinded(a: [(BlindedPayInfo, BlindedPath)]) -> Hints {
			...
			let aVector = Vec_C2Tuple_BlindedPayInfoBlindedPathZZ(
				array: a, instantiationContext: "Hints.swift::\(#function):\(#line)"
			)
			.dangle()
		...
		public func getValueAsBlinded() -> [(BlindedPayInfo, BlindedPath)]? {
			...
			return Vec_C2Tuple_BlindedPayInfoBlindedPathZZ(
				cType: self.cType!.blinded, instantiationContext: "Hints.swift::\(#function):\(#line)", anchor: self
			)
			.getValue()
		}
		...
		public func getValueAsClear() -> [RouteHint]? {
			...
			return Vec_RouteHintZ(
				cType: self.cType!.clear, instantiationContext: "Hints.swift::\(#function):\(#line)", anchor: self
			)
			.getValue()
		}
		...
		internal func dangleRecursively() -> Hints {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Hints \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
