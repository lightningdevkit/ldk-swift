...
extension Bindings {
	...
	public class RecipientOnionFields: NativeTypeWrapper {
		...
		internal init(
			cType: LDKRecipientOnionFields, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func read(ser: [UInt8]) -> Result_RecipientOnionFieldsDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "RecipientOnionFields.swift::\(#function):\(#line)")
		...
		public class func initWithSecretOnly(paymentSecret: [UInt8]) -> RecipientOnionFields {
			...
			let paymentSecretPrimitiveWrapper = ThirtyTwoBytes(
				value: paymentSecret, instantiationContext: "RecipientOnionFields.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing RecipientOnionFields \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing RecipientOnionFields \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
