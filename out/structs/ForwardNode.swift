...
extension Bindings {
	...
	public class ForwardNode: NativeTypeWrapper {
		...
		internal init(
			cType: LDKForwardNode, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getTlvs() -> ForwardTlvs {
			...
			let returnValue = ForwardTlvs(
				cType: nativeCallResult, instantiationContext: "ForwardNode.swift::\(#function):\(#line)", anchor: self
			)
			.dangle(false)
		...
		public func setNodeId(val: [UInt8]) {
			...
			let valPrimitiveWrapper = PublicKey(
				value: val, instantiationContext: "ForwardNode.swift::\(#function):\(#line)")
		...
		public init(tlvsArg: ForwardTlvs, nodeIdArg: [UInt8], htlcMaximumMsatArg: UInt64) {
			...
			let nodeIdArgPrimitiveWrapper = PublicKey(
				value: nodeIdArg, instantiationContext: "ForwardNode.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult = ForwardNode_new(
				tlvsArg.dynamicallyDangledClone().cType!, nodeIdArgPrimitiveWrapper.cType!, htlcMaximumMsatArg)
			...
			self.instanceNumber = Self.instanceCounter
			super
				.init(conflictAvoidingVariableName: 0, instantiationContext: "ForwardNode.swift::\(#function):\(#line)")
		...
		internal func clone() -> ForwardNode {
			...
			// return value (do some wrapping)
			let returnValue = ForwardNode(
				cType: nativeCallResult, instantiationContext: "ForwardNode.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing ForwardNode \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
