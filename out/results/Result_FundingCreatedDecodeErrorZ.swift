...
extension Bindings {
	...
	public class Result_FundingCreatedDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_FundingCreatedDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: FundingCreated) -> Result_FundingCreatedDecodeErrorZ {
			...
			let returnValue = Result_FundingCreatedDecodeErrorZ(
				...
				instantiationContext: "Result_FundingCreatedDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_FundingCreatedDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_FundingCreatedDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_FundingCreatedDecodeErrorZ.swift::\(#function):\(#line)", anchor: self
				)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_FundingCreatedDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_FundingCreatedDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_FundingCreatedDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
