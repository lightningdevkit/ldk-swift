...
extension Bindings {
	...
	internal class Tuple__u1632_u1632Z: NativeTypeWrapper {
		...
		internal init(
			cType: LDKC2Tuple__u1632_u1632Z, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public init(a: [UInt16], b: [UInt16], instantiationContext: String) {
			...
			let aPrimitiveWrapper = ThirtyTwoU16s(
				value: a, instantiationContext: "Tuple__u1632_u1632Z.swift::\(#function):\(#line)")

			let bPrimitiveWrapper = ThirtyTwoU16s(
				value: b, instantiationContext: "Tuple__u1632_u1632Z.swift::\(#function):\(#line)")
		...
		public func getA() -> [UInt16] {
			...
			let returnValue = ThirtyTwoU16s(
				cType: self.cType!.a, instantiationContext: "Tuple__u1632_u1632Z.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle().getValue()
		...
		public func getB() -> [UInt16] {
			// return value (do some wrapping)
			let returnValue = ThirtyTwoU16s(
				cType: self.cType!.b, instantiationContext: "Tuple__u1632_u1632Z.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle().getValue()
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Tuple__u1632_u1632Z \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Tuple__u1632_u1632Z \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
