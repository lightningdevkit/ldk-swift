...
extension Bindings {
	...
	public class ProbingError: NativeTypeWrapper {
		...
		internal init(
			cType: LDKProbingError, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> ProbingError {
			...
			// return value (do some wrapping)
			let returnValue = ProbingError(
				cType: nativeCallResult, instantiationContext: "ProbingError.swift::\(#function):\(#line)")
		...
		public class func initWithInvoice(a: String) -> ProbingError {
			...
			let aPrimitiveWrapper = Str(value: a, instantiationContext: "ProbingError.swift::\(#function):\(#line)")
				.dangle()
		...
		public func getValueAsInvoice() -> String? {
			...
			return Str(
				cType: self.cType!.invoice, instantiationContext: "ProbingError.swift::\(#function):\(#line)",
				anchor: self
			)
			.getValue()
		}
		...
		public func getValueAsSending() -> ProbeSendFailure? {
			...
			return ProbeSendFailure(
				cType: self.cType!.sending, instantiationContext: "ProbingError.swift::\(#function):\(#line)",
				anchor: self)
		}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing ProbingError \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing ProbingError \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
