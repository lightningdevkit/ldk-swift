...
extension Bindings {
	...
	public class InvalidShutdownScript: NativeTypeWrapper {
		...
		internal init(
			cType: LDKInvalidShutdownScript, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func setScript(val: [UInt8]) {
			...
			let valVector = Vec_u8Z(
				array: val, instantiationContext: "InvalidShutdownScript.swift::\(#function):\(#line)"
			)
			.dangle()
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKInvalidShutdownScript>) in
		...
		public init(scriptArg: [UInt8]) {
			...
			let scriptArgVector = Vec_u8Z(
				array: scriptArg, instantiationContext: "InvalidShutdownScript.swift::\(#function):\(#line)"
			)
			.dangle()
			...
			super
				.init(
					...
					instantiationContext: "InvalidShutdownScript.swift::\(#function):\(#line)")
		...
		internal func clone() -> InvalidShutdownScript {
			...
			// return value (do some wrapping)
			let returnValue = InvalidShutdownScript(
				cType: nativeCallResult, instantiationContext: "InvalidShutdownScript.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing InvalidShutdownScript \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing InvalidShutdownScript \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
