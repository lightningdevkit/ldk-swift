...
extension Bindings {
	...
	internal class Vec_C2Tuple_u32TxOutZZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_C2Tuple_u32TxOutZZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal init(array: [(UInt32, TxOut)], instantiationContext: String) {
			...
			let rustArray = array.map { (currentValueDepth1: (UInt32, TxOut)) -> LDKC2Tuple_u32TxOutZ in
				...
				let currentValueDepth1Tuple = Tuple_u32TxOutZ(
					tuple: currentValueDepth1,
					instantiationContext: "Vec_C2Tuple_u32TxOutZZ.swift::\(#function):\(#line)"
				)
				.danglingClone()
		...
		public func getValue() -> [(UInt32, TxOut)] {
			...
			let swiftArray = array.map { (currentCType: LDKC2Tuple_u32TxOutZ) -> (UInt32, TxOut) in
				Tuple_u32TxOutZ(
					cType: currentCType, instantiationContext: "Vec_C2Tuple_u32TxOutZZ.swift::\(#function):\(#line)"
				)
				.dangle().getValue()
			}
			return swiftArray
		...
		internal func dangleRecursively() -> Vec_C2Tuple_u32TxOutZZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_C2Tuple_u32TxOutZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Vec_C2Tuple_u32TxOutZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
