			...
			extension Bindings {
				...
				internal class Vec_FutureZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_FutureZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
...
Future(cType: currentCType, instantiationContext: "Vec_FutureZ.swift::\(#function):\(#line)", anchor: self).dangle()
						}
						return swiftArray
					}
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing Vec_FutureZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
