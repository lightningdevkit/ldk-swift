...
extension Bindings {
	...
	public class Result_UnsignedChannelUpdateDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_UnsignedChannelUpdateDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(
			cType: LDKCResult_UnsignedChannelUpdateDecodeErrorZ, instantiationContext: String,
		...
		public class func initWithOk(o: UnsignedChannelUpdate) -> Result_UnsignedChannelUpdateDecodeErrorZ {
			...
			let returnValue = Result_UnsignedChannelUpdateDecodeErrorZ(
				...
				instantiationContext: "Result_UnsignedChannelUpdateDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_UnsignedChannelUpdateDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_UnsignedChannelUpdateDecodeErrorZ>) in
		...
		internal func dangleRecursively() -> Result_UnsignedChannelUpdateDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_UnsignedChannelUpdateDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_UnsignedChannelUpdateDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
