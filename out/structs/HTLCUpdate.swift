			...
			extension Bindings {
				...
				public class HTLCUpdate: NativeTypeWrapper {
					...
					internal init(cType: LDKHTLCUpdate, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				HTLCUpdate_write(objPointer)
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing HTLCUpdate \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
