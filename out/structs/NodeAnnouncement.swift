			...
			extension Bindings {
				...
				public class NodeAnnouncement: NativeTypeWrapper {
					...
					internal init(cType: LDKNodeAnnouncement) {
					...
					internal init(cType: LDKNodeAnnouncement, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				NodeAnnouncement_get_contents(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = UnsignedNodeAnnouncement(cType: nativeCallResult, anchor: self).dangle(false)
