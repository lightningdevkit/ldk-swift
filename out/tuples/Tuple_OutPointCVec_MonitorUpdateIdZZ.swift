...
extension Bindings {
	...
	internal class Tuple_OutPointCVec_MonitorUpdateIdZZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKC2Tuple_OutPointCVec_MonitorUpdateIdZZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> Tuple_OutPointCVec_MonitorUpdateIdZZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKC2Tuple_OutPointCVec_MonitorUpdateIdZZ>) in
			...
			let returnValue = Tuple_OutPointCVec_MonitorUpdateIdZZ(
				cType: nativeCallResult,
				instantiationContext: "Tuple_OutPointCVec_MonitorUpdateIdZZ.swift::\(#function):\(#line)")
		...
		public init(a: OutPoint, b: [MonitorUpdateId], instantiationContext: String) {
			...
			let bVector = Vec_MonitorUpdateIdZ(
				array: b, instantiationContext: "Tuple_OutPointCVec_MonitorUpdateIdZZ.swift::\(#function):\(#line)"
			)
			.dangle()


			// native method call
			let nativeCallResult = C2Tuple_OutPointCVec_MonitorUpdateIdZZ_new(
				a.dynamicallyDangledClone().cType!, bVector.cType!)
		...
		public func getA() -> OutPoint {
			...
			let returnValue = OutPoint(
				cType: self.cType!.a,
				instantiationContext: "Tuple_OutPointCVec_MonitorUpdateIdZZ.swift::\(#function):\(#line)", anchor: self
			)
			.dangle()
		...
		public func getB() -> [MonitorUpdateId] {
			// return value (do some wrapping)
			let returnValue = Vec_MonitorUpdateIdZ(
				cType: self.cType!.b,
				instantiationContext: "Tuple_OutPointCVec_MonitorUpdateIdZZ.swift::\(#function):\(#line)", anchor: self
			)
			.dangle().getValue()
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Tuple_OutPointCVec_MonitorUpdateIdZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Tuple_OutPointCVec_MonitorUpdateIdZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
