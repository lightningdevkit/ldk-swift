...
extension Bindings {
	...
	public class Bech32Error: NativeTypeWrapper {
		...
		internal init(
			cType: LDKBech32Error, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> Bech32Error {
			...
			let returnValue = Bech32Error(
				cType: nativeCallResult, instantiationContext: "Bech32Error.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Bech32Error \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
