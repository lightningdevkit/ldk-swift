...
extension Bindings {
	...
	public class ShutdownScript: NativeTypeWrapper {
		...
		internal init(
			cType: LDKShutdownScript, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> ShutdownScript {
			...
			let returnValue = ShutdownScript(
				cType: nativeCallResult, instantiationContext: "ShutdownScript.swift::\(#function):\(#line)")
		...
		public class func read(ser: [UInt8]) -> Result_ShutdownScriptDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "ShutdownScript.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> ShutdownScript {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing ShutdownScript \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing ShutdownScript \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
