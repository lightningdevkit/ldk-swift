			...
			extension Bindings {
				...
				internal class Option_ThirtyTwoBytesZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCOption_ThirtyTwoBytesZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(some: [UInt8]?, instantiationContext: String) {
						...
						if let some = some {
							...
							let somePrimitiveWrapper = ThirtyTwoBytes(value: some, instantiationContext: "Option_ThirtyTwoBytesZ.swift::\(#function):\(#line)")
				...
				COption_ThirtyTwoBytesZ_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_ThirtyTwoBytesZ(cType: nativeCallResult, instantiationContext: "Option_ThirtyTwoBytesZ.swift::\(#function):\(#line)")
					...
					public func getValue() -> [UInt8]? {
						...
						if self.cType!.tag == LDKCOption_ThirtyTwoBytesZ_Some {
							return ThirtyTwoBytes(cType: self.cType!.some, instantiationContext: "Option_ThirtyTwoBytesZ.swift::\(#function):\(#line)", anchor: self).dangle().getValue()
						}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Option_ThirtyTwoBytesZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Option_ThirtyTwoBytesZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
