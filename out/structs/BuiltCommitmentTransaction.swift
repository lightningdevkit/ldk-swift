...
extension Bindings {
	...
	public class BuiltCommitmentTransaction: NativeTypeWrapper {
		...
		internal init(
			cType: LDKBuiltCommitmentTransaction, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func setTransaction(val: [UInt8]) {
			...
			let valPrimitiveWrapper = Transaction(
				value: val, instantiationContext: "BuiltCommitmentTransaction.swift::\(#function):\(#line)"
			)
			.dangle()
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKBuiltCommitmentTransaction>) in
		...
		public init(transactionArg: [UInt8], txidArg: [UInt8]) {
			...
			let transactionArgPrimitiveWrapper = Transaction(
				value: transactionArg, instantiationContext: "BuiltCommitmentTransaction.swift::\(#function):\(#line)"
			)
			.dangle()

			let txidArgPrimitiveWrapper = ThirtyTwoBytes(
				value: txidArg, instantiationContext: "BuiltCommitmentTransaction.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult = BuiltCommitmentTransaction_new(
				transactionArgPrimitiveWrapper.cType!, txidArgPrimitiveWrapper.cType!)
			...
			super
				.init(
					...
					instantiationContext: "BuiltCommitmentTransaction.swift::\(#function):\(#line)")
		...
		internal func clone() -> BuiltCommitmentTransaction {
			...
			let returnValue = BuiltCommitmentTransaction(
				cType: nativeCallResult, instantiationContext: "BuiltCommitmentTransaction.swift::\(#function):\(#line)"
			)
		...
		public class func read(ser: [UInt8]) -> Result_BuiltCommitmentTransactionDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "BuiltCommitmentTransaction.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_BuiltCommitmentTransactionDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "BuiltCommitmentTransaction.swift::\(#function):\(#line)"
			)
		...
		public func signCounterpartyCommitment(
			fundingKey: [UInt8], fundingRedeemscript: [UInt8], channelValueSatoshis: UInt64
		) -> [UInt8] {
			// native call variable prep
		...
		public func signHolderCommitment(
			fundingKey: [UInt8], fundingRedeemscript: [UInt8], channelValueSatoshis: UInt64,
					...
					withUnsafePointer(to: tupledFundingKey) { (tupledFundingKeyPointer: UnsafePointer<UInt8Tuple32>) in
						...
						withUnsafePointer(to: entropySource.activate().cType!) {
							...
							BuiltCommitmentTransaction_sign_holder_commitment(
								thisArgPointer, tupledFundingKeyPointer, fundingRedeemscriptPrimitiveWrapper.cType!,
								channelValueSatoshis, entropySourcePointer)
						}
		...
		internal func dangleRecursively() -> BuiltCommitmentTransaction {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing BuiltCommitmentTransaction \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing BuiltCommitmentTransaction \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
