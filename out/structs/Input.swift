	...
	public class Input: NativeTypeWrapper {
		...
		public func getOutpoint() -> OutPoint {
			...
			let returnValue = OutPoint(
				cType: nativeCallResult, instantiationContext: "Input.swift::\(#function):\(#line)", anchor: self
			)
			.dangle(false)
		...
		public init(outpointArg: OutPoint, previousUtxoArg: TxOut, satisfactionWeightArg: UInt64) {
			...
			// native method call
			let nativeCallResult = Input_new(
				outpointArg.dynamicallyDangledClone().cType!, previousUtxoArg.danglingClone().cType!,
				satisfactionWeightArg)
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Input \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
