...
extension Bindings {
	...
	public class ClaimedHTLC: NativeTypeWrapper {
		...
		internal init(
			cType: LDKClaimedHTLC, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init(channelIdArg: [UInt8], userChannelIdArg: [UInt8], cltvExpiryArg: UInt32, valueMsatArg: UInt64) {
			...
			self.instanceNumber = Self.instanceCounter
			super
				.init(conflictAvoidingVariableName: 0, instantiationContext: "ClaimedHTLC.swift::\(#function):\(#line)")
		...
		internal func clone() -> ClaimedHTLC {
			...
			// return value (do some wrapping)
			let returnValue = ClaimedHTLC(
				cType: nativeCallResult, instantiationContext: "ClaimedHTLC.swift::\(#function):\(#line)")
		...
		public class func read(ser: [UInt8]) -> Result_ClaimedHTLCDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "ClaimedHTLC.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing ClaimedHTLC \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
