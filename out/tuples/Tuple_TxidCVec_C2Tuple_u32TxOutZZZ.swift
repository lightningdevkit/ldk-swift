...
extension Bindings {
	...
	internal class Tuple_TxidCVec_C2Tuple_u32TxOutZZZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKC2Tuple_TxidCVec_C2Tuple_u32TxOutZZZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> Tuple_TxidCVec_C2Tuple_u32TxOutZZZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKC2Tuple_TxidCVec_C2Tuple_u32TxOutZZZ>) in
			...
			let returnValue = Tuple_TxidCVec_C2Tuple_u32TxOutZZZ(
				cType: nativeCallResult,
				instantiationContext: "Tuple_TxidCVec_C2Tuple_u32TxOutZZZ.swift::\(#function):\(#line)")
		...
		public init(a: [UInt8], b: [(UInt32, TxOut)], instantiationContext: String) {
			...
			let aPrimitiveWrapper = ThirtyTwoBytes(
				value: a, instantiationContext: "Tuple_TxidCVec_C2Tuple_u32TxOutZZZ.swift::\(#function):\(#line)")

			let bVector = Vec_C2Tuple_u32TxOutZZ(
				array: b, instantiationContext: "Tuple_TxidCVec_C2Tuple_u32TxOutZZZ.swift::\(#function):\(#line)"
			)
			.dangle()
		...
		public func getA() -> [UInt8] {
			...
			let returnValue = ThirtyTwoBytes(
				cType: self.cType!.a,
				instantiationContext: "Tuple_TxidCVec_C2Tuple_u32TxOutZZZ.swift::\(#function):\(#line)", anchor: self
			)
			.dangle().getValue()
		...
		internal func dangleRecursively() -> Tuple_TxidCVec_C2Tuple_u32TxOutZZZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Tuple_TxidCVec_C2Tuple_u32TxOutZZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Tuple_TxidCVec_C2Tuple_u32TxOutZZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
