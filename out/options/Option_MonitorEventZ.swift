...
extension Bindings {
	...
	internal class Option_MonitorEventZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCOption_MonitorEventZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> Option_MonitorEventZ {
			...
			// return value (do some wrapping)
			let returnValue = Option_MonitorEventZ(
				cType: nativeCallResult, instantiationContext: "Option_MonitorEventZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> MonitorEvent? {
			...
			if self.cType!.tag == LDKCOption_MonitorEventZ_Some {
				return MonitorEvent(
					cType: self.cType!.some, instantiationContext: "Option_MonitorEventZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		internal func dangleRecursively() -> Option_MonitorEventZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Option_MonitorEventZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Option_MonitorEventZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
