...
extension Bindings {
	...
	public class BumpTransactionEvent: NativeTypeWrapper {
		...
		internal init(
			cType: LDKBumpTransactionEvent, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> BumpTransactionEvent {
			...
			// return value (do some wrapping)
			let returnValue = BumpTransactionEvent(
				cType: nativeCallResult, instantiationContext: "BumpTransactionEvent.swift::\(#function):\(#line)")
		...
		public class func initWithChannelClose(
			...
			let commitmentTxPrimitiveWrapper = Transaction(
				value: commitmentTx, instantiationContext: "BumpTransactionEvent.swift::\(#function):\(#line)"
			)
			.dangle()

			let pendingHtlcsVector = Vec_HTLCOutputInCommitmentZ(
				array: pendingHtlcs, instantiationContext: "BumpTransactionEvent.swift::\(#function):\(#line)"
			)
			.dangle()
		...
		public class func initWithHtlcresolution(
			...
			let htlcDescriptorsVector = Vec_HTLCDescriptorZ(
				array: htlcDescriptors, instantiationContext: "BumpTransactionEvent.swift::\(#function):\(#line)"
			)
			.dangle()
		...
		public func getValueAsChannelClose() -> ChannelClose? {
			...
			return BumpTransactionEvent_LDKChannelClose_Body(
				cType: self.cType!.channel_close,
		...
		public func getValueAsHtlcResolution() -> HTLCResolution? {
			...
			return BumpTransactionEvent_LDKHTLCResolution_Body(
				cType: self.cType!.htlc_resolution,
				instantiationContext: "BumpTransactionEvent.swift::\(#function):\(#line)", anchor: self)
		}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing BumpTransactionEvent \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing BumpTransactionEvent \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
		...
		public class ChannelClose: NativeTypeWrapper {
			...
			internal init(
				cType: LDKBumpTransactionEvent_LDKChannelClose_Body, instantiationContext: String,
			...
			public func getCommitmentTx() -> [UInt8] {
				...
				let returnValue = Transaction(
					cType: self.cType!.commitment_tx,
					instantiationContext: "BumpTransactionEvent.swift::\(#function):\(#line)", anchor: self
				)
				.getValue()
			...
			public func getAnchorDescriptor() -> Bindings.AnchorDescriptor {
				// return value (do some wrapping)
				let returnValue = Bindings.AnchorDescriptor(
					cType: self.cType!.anchor_descriptor,
					instantiationContext: "BumpTransactionEvent.swift::\(#function):\(#line)", anchor: self)
			...
			public func getPendingHtlcs() -> [HTLCOutputInCommitment] {
				// return value (do some wrapping)
				let returnValue = Vec_HTLCOutputInCommitmentZ(
					cType: self.cType!.pending_htlcs,
					instantiationContext: "BumpTransactionEvent.swift::\(#function):\(#line)", anchor: self
				)
				.getValue()
		...
		public class HTLCResolution: NativeTypeWrapper {
			...
			internal init(
				cType: LDKBumpTransactionEvent_LDKHTLCResolution_Body, instantiationContext: String,
				anchor: NativeTypeWrapper
			) {
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
			...
			public func getHtlcDescriptors() -> [HTLCDescriptor] {
				// return value (do some wrapping)
				let returnValue = Vec_HTLCDescriptorZ(
					cType: self.cType!.htlc_descriptors,
					instantiationContext: "BumpTransactionEvent.swift::\(#function):\(#line)", anchor: self
				)
				.getValue()
