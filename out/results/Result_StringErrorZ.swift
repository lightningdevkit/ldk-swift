			...
			extension Bindings {
				...
				public class Result_StringErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_StringErrorZ) {
					...
					internal init(cType: LDKCResult_StringErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_StringErrorZ_clone(origPointer)
					...
					public func getValue() -> String? {
						if self.cType?.result_ok == true {
							return Str(cType: self.cType!.contents.result.pointee, anchor: self).getValue()
						}
						return nil
					}
