...
extension Bindings {
	...
	internal class Tuple_u32ScriptZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKC2Tuple_u32ScriptZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> Tuple_u32ScriptZ {
			...
			let returnValue = Tuple_u32ScriptZ(
				cType: nativeCallResult, instantiationContext: "Tuple_u32ScriptZ.swift::\(#function):\(#line)")
		...
		public init(a: UInt32, b: [UInt8], instantiationContext: String) {
			...
			let bVector = Vec_u8Z(array: b, instantiationContext: "Tuple_u32ScriptZ.swift::\(#function):\(#line)")
				.dangle()
		...
		public func getB() -> [UInt8] {
			...
			let returnValue = Vec_u8Z(
				cType: self.cType!.b, instantiationContext: "Tuple_u32ScriptZ.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle().getValue()
		...
		internal func dangleRecursively() -> Tuple_u32ScriptZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Tuple_u32ScriptZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Tuple_u32ScriptZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
