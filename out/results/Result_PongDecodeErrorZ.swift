			...
			extension Bindings {
				...
				public class Result_PongDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_PongDecodeErrorZ) {
					...
					internal init(cType: LDKCResult_PongDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_PongDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> Pong? {
						if self.cType?.result_ok == true {
							return Pong(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
