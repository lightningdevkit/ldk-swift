...
extension Bindings {
	...
	internal class BigEndianScalar: NativeTypeWrapper {
		...
		internal init(
			cType: LDKBigEndianScalar, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init(bigEndianBytes: [UInt8], instantiationContext: String) {
			...
			let bigEndianBytesPrimitiveWrapper = ThirtyTwoBytes(
				value: bigEndianBytes, instantiationContext: "BigEndianScalar.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> BigEndianScalar {
			self.dangling = true
