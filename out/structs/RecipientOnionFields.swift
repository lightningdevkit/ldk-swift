			...
			extension Bindings {
				...
				public class RecipientOnionFields: NativeTypeWrapper {
					...
					internal init(cType: LDKRecipientOnionFields) {
					...
					internal init(cType: LDKRecipientOnionFields, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				RecipientOnionFields_get_payment_metadata(thisPtrPointer)
					...
					public func setPaymentMetadata(val: [UInt8]?) {
						...
						let valOption = Option_CVec_u8ZZ(some: val).danglingClone()
				...
				RecipientOnionFields_set_payment_metadata(thisPtrPointer, valOption.cType!)
					...
					public init(paymentSecretArg: [UInt8], paymentMetadataArg: [UInt8]?) {
						...
						let paymentSecretArgPrimitiveWrapper = ThirtyTwoBytes(value: paymentSecretArg)
				
						let paymentMetadataArgOption = Option_CVec_u8ZZ(some: paymentMetadataArg).danglingClone()
				...
				self.initialCFreeability = nativeCallResult.is_owned
						...
						/*
						// return value (do some wrapping)
						let returnValue = RecipientOnionFields(cType: nativeCallResult)
						*/
				...
				RecipientOnionFields_write(objPointer)
					...
					deinit {
						...
						if !self.dangling {
							Bindings.print("Freeing RecipientOnionFields \(self.instanceNumber).")
							
							self.free()
						} else {
							Bindings.print("Not freeing RecipientOnionFields \(self.instanceNumber) due to dangle.")
						}
					}
