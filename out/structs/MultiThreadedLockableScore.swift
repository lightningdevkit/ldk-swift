...
extension Bindings {
	...
	public class MultiThreadedLockableScore: NativeTypeWrapper {
		...
		internal init(
			cType: LDKMultiThreadedLockableScore, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func asLockableScore() -> LockableScore {
			...
			let returnValue = NativelyImplementedLockableScore(
				...
				instantiationContext: "MultiThreadedLockableScore.swift::\(#function):\(#line)", anchor: self)
		...
		public func asWriteableScore() -> WriteableScore {
			...
			// return value (do some wrapping)
			let returnValue = NativelyImplementedWriteableScore(
				cType: nativeCallResult,
				instantiationContext: "MultiThreadedLockableScore.swift::\(#function):\(#line)", anchor: self)
		...
		public init(score: Score) {
			...
			super
				.init(
					...
					instantiationContext: "MultiThreadedLockableScore.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing MultiThreadedLockableScore \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing MultiThreadedLockableScore \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
