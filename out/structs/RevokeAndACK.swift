...
extension Bindings {
	...
	public class RevokeAndACK: NativeTypeWrapper {
		...
		internal init(
			cType: LDKRevokeAndACK, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init(channelIdArg: [UInt8], perCommitmentSecretArg: [UInt8], nextPerCommitmentPointArg: [UInt8]) {
			...
			let nextPerCommitmentPointArgPrimitiveWrapper = PublicKey(
				value: nextPerCommitmentPointArg, instantiationContext: "RevokeAndACK.swift::\(#function):\(#line)")
			...
			super
				.init(
					conflictAvoidingVariableName: 0, instantiationContext: "RevokeAndACK.swift::\(#function):\(#line)")
		...
		internal func clone() -> RevokeAndACK {
			...
			// return value (do some wrapping)
			let returnValue = RevokeAndACK(
				cType: nativeCallResult, instantiationContext: "RevokeAndACK.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKRevokeAndACK>) in
		...
		public class func read(ser: [UInt8]) -> Result_RevokeAndACKDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "RevokeAndACK.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_RevokeAndACKDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "RevokeAndACK.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing RevokeAndACK \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing RevokeAndACK \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
