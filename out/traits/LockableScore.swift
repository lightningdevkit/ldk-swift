			...
			extension Bindings {
				...
				open class LockableScore: NativeTraitWrapper {
					...
					internal init(cType: LDKLockableScore, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public init() {
						...
						func readLockLambda(this_arg: UnsafeRawPointer?) -> LDKScoreLookUp {
							let instance: LockableScore = Bindings.pointerToInstance(pointer: this_arg!, sourceMarker: "LockableScore::readLockLambda")
						...
						func writeLockLambda(this_arg: UnsafeRawPointer?) -> LDKScoreUpdate {
							let instance: LockableScore = Bindings.pointerToInstance(pointer: this_arg!, sourceMarker: "LockableScore::writeLockLambda")
			...
			open func writeLock() -> ScoreUpdate {
				...
				Bindings.print("Error: LockableScore::writeLock MUST be overridden! Offending class: \(String(describing: self)). Aborting.", severity: .ERROR)
				abort()
			}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing LockableScore \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing LockableScore \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
				}
			...
			public override func readLock() -> ScoreLookUp {
				...
				// return value (do some wrapping)
				let returnValue = NativelyImplementedScoreLookUp(cType: nativeCallResult, instantiationContext: "LockableScore.swift::\(#function):\(#line)", anchor: self)
			...
			public override func writeLock() -> ScoreUpdate {
				...
				// return value (do some wrapping)
				let returnValue = NativelyImplementedScoreUpdate(cType: nativeCallResult, instantiationContext: "LockableScore.swift::\(#function):\(#line)", anchor: self)
