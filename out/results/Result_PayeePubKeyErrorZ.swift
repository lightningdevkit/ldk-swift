			...
			extension Bindings {
				...
				public class Result_PayeePubKeyErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_PayeePubKeyErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: PayeePubKey) -> Result_PayeePubKeyErrorZ {
						...
						let returnValue = Result_PayeePubKeyErrorZ(cType: nativeCallResult, instantiationContext: "Result_PayeePubKeyErrorZ.swift::\(#function):\(#line)")
				...
				CResult_PayeePubKeyErrorZ_clone(origPointer)
					...
					public func getValue() -> PayeePubKey? {
						if self.cType?.result_ok == true {
							return PayeePubKey(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_PayeePubKeyErrorZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_PayeePubKeyErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_PayeePubKeyErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
