...
extension Bindings {
	...
	public class ChainParameters: NativeTypeWrapper {
		...
		internal init(
			cType: LDKChainParameters, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func setNetwork(val: Network) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKChainParameters>) in
		...
		public func getBestBlock() -> BestBlock {
			...
			let returnValue = BestBlock(
				cType: nativeCallResult, instantiationContext: "ChainParameters.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle(false)
		...
		public init(networkArg: Network, bestBlockArg: BestBlock) {
			...
			// native method call
			let nativeCallResult = ChainParameters_new(
				networkArg.getCValue(), bestBlockArg.dynamicallyDangledClone().cType!)
			...
			super
				.init(
					...
					instantiationContext: "ChainParameters.swift::\(#function):\(#line)")
		...
		internal func clone() -> ChainParameters {
			...
			// return value (do some wrapping)
			let returnValue = ChainParameters(
				cType: nativeCallResult, instantiationContext: "ChainParameters.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> ChainParameters {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing ChainParameters \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing ChainParameters \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
