...
extension Bindings {
	...
	internal class Vec_StrZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_StrZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(array: [String], instantiationContext: String) {
			...
			let rustArray = array.map { (currentValueDepth1: String) -> LDKStr in
				...
				let currentValueDepth1PrimitiveWrapper = Str(
					value: currentValueDepth1, instantiationContext: "Vec_StrZ.swift::\(#function):\(#line)"
				)
				.dangle()
		...
		public func getValue() -> [String] {
			...
			let swiftArray = array.map { (currentCType: LDKStr) -> String in
				Str(cType: currentCType, instantiationContext: "Vec_StrZ.swift::\(#function):\(#line)").dangle()
					.getValue()
			}
			return swiftArray
		}
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Vec_StrZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
