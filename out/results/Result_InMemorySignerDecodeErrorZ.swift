...
extension Bindings {
	...
	public class Result_InMemorySignerDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_InMemorySignerDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: InMemorySigner) -> Result_InMemorySignerDecodeErrorZ {
			...
			let returnValue = Result_InMemorySignerDecodeErrorZ(
				...
				instantiationContext: "Result_InMemorySignerDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_InMemorySignerDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_InMemorySignerDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_InMemorySignerDecodeErrorZ.swift::\(#function):\(#line)", anchor: self
				)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_InMemorySignerDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_InMemorySignerDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_InMemorySignerDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
