...
extension Bindings {
	...
	public class Result_PaymentPurposeDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_PaymentPurposeDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: PaymentPurpose) -> Result_PaymentPurposeDecodeErrorZ {
			...
			let returnValue = Result_PaymentPurposeDecodeErrorZ(
				...
				instantiationContext: "Result_PaymentPurposeDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_PaymentPurposeDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_PaymentPurposeDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_PaymentPurposeDecodeErrorZ.swift::\(#function):\(#line)", anchor: self
				)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_PaymentPurposeDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_PaymentPurposeDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_PaymentPurposeDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
