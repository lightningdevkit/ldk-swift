...
extension Bindings {
	...
	open class FutureCallback: NativeTraitWrapper {
		...
		internal init(
			cType: LDKFutureCallback, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init() {
			...
			super
				.init(
					conflictAvoidingVariableName: 0, instantiationContext: "FutureCallback.swift::\(#function):\(#line)"
				)

			let thisArg = Bindings.instanceToPointer(instance: self)


			func callLambda(this_arg: UnsafeRawPointer?) {
				let instance: FutureCallback = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "FutureCallback::callLambda")
			...
			func freeLambda(this_arg: UnsafeMutableRawPointer?) {
				let instance: FutureCallback = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "FutureCallback::freeLambda")
		...
		open func call() {
			...
			Bindings.print(
				"Error: FutureCallback::call MUST be overridden! Offending class: \(String(describing: self)). Aborting.",
				severity: .ERROR)
			abort()
		}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing FutureCallback \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}
				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing FutureCallback \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
	}
