...
extension Bindings {
	...
	public class MultiThreadedScoreLockWrite: NativeTypeWrapper {
		...
		internal init(
			cType: LDKMultiThreadedScoreLockWrite, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func write() -> [UInt8] {
			...
			let returnValue = Vec_u8Z(
				...
				instantiationContext: "MultiThreadedScoreLockWrite.swift::\(#function):\(#line)", anchor: self
			)
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing MultiThreadedScoreLockWrite \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing MultiThreadedScoreLockWrite \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
