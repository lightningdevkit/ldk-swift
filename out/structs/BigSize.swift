			...
			extension Bindings {
				...
				BigSize_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "BigSize.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public class func read(ser: [UInt8]) -> Result_BigSizeDecodeErrorZ {
						...
						// return value (do some wrapping)
						let returnValue = Result_BigSizeDecodeErrorZ(cType: nativeCallResult, instantiationContext: "BigSize.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing BigSize \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
