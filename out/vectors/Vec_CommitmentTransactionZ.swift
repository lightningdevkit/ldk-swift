...
extension Bindings {
	...
	internal class Vec_CommitmentTransactionZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_CommitmentTransactionZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func getValue() -> [CommitmentTransaction] {
			...
			let swiftArray = array.map { (currentCType: LDKCommitmentTransaction) -> CommitmentTransaction in
				CommitmentTransaction(
					cType: currentCType,
					instantiationContext: "Vec_CommitmentTransactionZ.swift::\(#function):\(#line)", anchor: self
				)
				.dangle()
			}
			return swiftArray
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_CommitmentTransactionZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Vec_CommitmentTransactionZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
