...
extension Bindings {
	...
	public class Result_QueryChannelRangeDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_QueryChannelRangeDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: QueryChannelRange) -> Result_QueryChannelRangeDecodeErrorZ {
			...
			let returnValue = Result_QueryChannelRangeDecodeErrorZ(
				...
				instantiationContext: "Result_QueryChannelRangeDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_QueryChannelRangeDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_QueryChannelRangeDecodeErrorZ>) in
		...
		internal func dangleRecursively() -> Result_QueryChannelRangeDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_QueryChannelRangeDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_QueryChannelRangeDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
