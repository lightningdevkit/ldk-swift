			...
			extension Bindings {
				...
				public class Path: NativeTypeWrapper {
					...
					internal init(cType: LDKPath, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				Path_get_hops(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_RouteHopZ(cType: nativeCallResult, anchor: self).dangle(false).getValue()
					...
					public func setHops(val: [RouteHop]) {
						...
						let valVector = Vec_RouteHopZ(array: val).dangle()
				...
				// Type: LDKBlindedTail
					...
					if pointerValue == 0 {
						...
						// return value (do some wrapping)
						let returnValue = BlindedTail(cType: nativeCallResult, anchor: self).dangle(false)
				...
				Path_set_blinded_tail(thisPtrPointer, val.dynamicallyDangledClone().cType!)
					...
					public init(hopsArg: [RouteHop], blindedTailArg: BlindedTail) {
						...
						let hopsArgVector = Vec_RouteHopZ(array: hopsArg).dangle()
				...
				self.initialCFreeability = nativeCallResult.is_owned
						...
						/*
						// return value (do some wrapping)
						let returnValue = Path(cType: nativeCallResult)
						*/
				...
				Path_final_cltv_expiry_delta(thisArgPointer)
					...
					deinit {
						...
						if !self.dangling {
							Bindings.print("Freeing Path \(self.instanceNumber).")
							
							self.free()
						} else {
							Bindings.print("Not freeing Path \(self.instanceNumber) due to dangle.")
						}
					}
