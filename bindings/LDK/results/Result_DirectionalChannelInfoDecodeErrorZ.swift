public class Result_DirectionalChannelInfoDecodeErrorZ: NativeTypeWrapper {
	...
	public init(pointer: LDKCResult_DirectionalChannelInfoDecodeErrorZ, anchor: NativeTypeWrapper){
		Self.instanceCounter += 1
		self.instanceNumber = Self.instanceCounter
		self.cOpaqueStruct = pointer
		super.init(conflictAvoidingVariableName: 0)
    ...
    /* RESULT_METHODS_START */
		...
		public func getValue() throws -> DirectionalChannelInfo {
			if self.cOpaqueStruct?.result_ok == true {
				return DirectionalChannelInfo(pointer: self.cOpaqueStruct!.contents.result.pointee, anchor: self)
			}
    ...
    internal func free() -> Void {
        ...
        return CResult_DirectionalChannelInfoDecodeErrorZ_free(self.cOpaqueStruct!);
					...
					deinit {
						if !self.dangling {
							Bindings.print("Freeing Result_DirectionalChannelInfoDecodeErrorZ \(self.instanceNumber).")
							self.free()
						} else {
							Bindings.print("Not freeing Result_DirectionalChannelInfoDecodeErrorZ \(self.instanceNumber) due to dangle.")
						}
					}
