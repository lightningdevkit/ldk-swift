			...
			extension Bindings {
				...
				internal class Vec_ThirtyTwoBytesZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_ThirtyTwoBytesZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(array: [[UInt8]], instantiationContext: String) {
						...
						let currentValueDepth1PrimitiveWrapper = ThirtyTwoBytes(value: currentValueDepth1, instantiationContext: "Vec_ThirtyTwoBytesZ.swift::\(#function):\(#line)")
...
ThirtyTwoBytes(cType: currentCType, instantiationContext: "Vec_ThirtyTwoBytesZ.swift::\(#function):\(#line)").dangle().getValue()
						}
						return swiftArray
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Vec_ThirtyTwoBytesZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Vec_ThirtyTwoBytesZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
