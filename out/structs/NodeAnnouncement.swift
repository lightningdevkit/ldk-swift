			...
			extension Bindings {
				...
				public class NodeAnnouncement: NativeTypeWrapper {
					...
					internal init(cType: LDKNodeAnnouncement, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				NodeAnnouncement_get_signature(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = ECDSASignature(cType: nativeCallResult, instantiationContext: "NodeAnnouncement.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public func setSignature(val: [UInt8]) {
						...
						let valPrimitiveWrapper = ECDSASignature(value: val, instantiationContext: "NodeAnnouncement.swift::\(#function):\(#line)")
				...
				NodeAnnouncement_set_contents(thisPtrPointer, val.dynamicallyDangledClone().cType!)
					...
					public init(signatureArg: [UInt8], contentsArg: UnsignedNodeAnnouncement) {
						...
						let signatureArgPrimitiveWrapper = ECDSASignature(value: signatureArg, instantiationContext: "NodeAnnouncement.swift::\(#function):\(#line)")
