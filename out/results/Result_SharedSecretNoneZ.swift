			...
			extension Bindings {
				...
				public class Result_SharedSecretNoneZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_SharedSecretNoneZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: [UInt8]) -> Result_SharedSecretNoneZ {
						...
						let returnValue = Result_SharedSecretNoneZ(cType: nativeCallResult, instantiationContext: "Result_SharedSecretNoneZ.swift::\(#function):\(#line)")
					...
					*/
					...
					public func getValue() -> [UInt8]? {
						if self.cType?.result_ok == true {
							return ThirtyTwoBytes(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_SharedSecretNoneZ.swift::\(#function):\(#line)", anchor: self).getValue()
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_SharedSecretNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_SharedSecretNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
