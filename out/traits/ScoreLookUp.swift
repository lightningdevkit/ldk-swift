...
extension Bindings {
	...
	open class ScoreLookUp: NativeTraitWrapper {
		...
		internal init(
			cType: LDKScoreLookUp, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init() {
			...
			self.instanceNumber = Self.instanceCounter
			super
				.init(conflictAvoidingVariableName: 0, instantiationContext: "ScoreLookUp.swift::\(#function):\(#line)")
			...
			func channelPenaltyMsatLambda(
				...
				let instance: ScoreLookUp = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "ScoreLookUp::channelPenaltyMsatLambda")
				...
				let swiftCallbackResult = instance.channelPenaltyMsat(
					...
					.dangle().clone(),
					usage: ChannelUsage(
						cType: usage, instantiationContext: "ScoreLookUp.swift::init()::\(#function):\(#line)"),
					scoreParams: ProbabilisticScoringFeeParameters(
						cType: score_params.pointee,
						instantiationContext: "ScoreLookUp.swift::init()::\(#function):\(#line)"
					)
					.dangle().clone())
			...
			func freeLambda(this_arg: UnsafeMutableRawPointer?) {
				let instance: ScoreLookUp = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "ScoreLookUp::freeLambda")
		...
		open func channelPenaltyMsat(
			...
			Bindings.print(
				"Error: ScoreLookUp::channelPenaltyMsat MUST be overridden! Offending class: \(String(describing: self)). Aborting.",
				severity: .ERROR)
			abort()
		}
		...
		internal func free() {
			...
			Bindings.print(
				"Error: ScoreLookUp::free MUST be overridden! Offending class: \(String(describing: self)). Aborting.",
				severity: .ERROR)
			abort()
		}
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing ScoreLookUp \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
	}
