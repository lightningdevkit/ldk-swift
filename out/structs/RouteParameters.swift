			...
			extension Bindings {
				...
				RouteParameters_get_max_total_routing_fee_msat(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_u64Z(cType: nativeCallResult, instantiationContext: "RouteParameters.swift::\(#function):\(#line)", anchor: self).getValue()
					...
					public func setMaxTotalRoutingFeeMsat(val: UInt64?) {
						...
						let valOption = Option_u64Z(some: val, instantiationContext: "RouteParameters.swift::\(#function):\(#line)").danglingClone()
				...
				RouteParameters_set_max_total_routing_fee_msat(thisPtrPointer, valOption.cType!)
					...
					public init(paymentParamsArg: PaymentParameters, finalValueMsatArg: UInt64, maxTotalRoutingFeeMsatArg: UInt64?) {
						...
						let maxTotalRoutingFeeMsatArgOption = Option_u64Z(some: maxTotalRoutingFeeMsatArg, instantiationContext: "RouteParameters.swift::\(#function):\(#line)").danglingClone()
				

						// native method call
						let nativeCallResult = RouteParameters_new(paymentParamsArg.dynamicallyDangledClone().cType!, finalValueMsatArg, maxTotalRoutingFeeMsatArgOption.cType!)
				...
				RouteParameters_eq(aPointer, bPointer)
					...
					public class func initWithPaymentParamsAndValue(paymentParams: PaymentParameters, finalValueMsat: UInt64) -> RouteParameters {
						// native call variable prep
						

						// native method call
						let nativeCallResult = RouteParameters_from_payment_params_and_value(paymentParams.dynamicallyDangledClone().cType!, finalValueMsat)
