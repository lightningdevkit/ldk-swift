...
extension Bindings {
	...
	public class TxRemoveInput: NativeTypeWrapper {
		...
		internal init(
			cType: LDKTxRemoveInput, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func setChannelId(val: [UInt8]) {
			...
			let valPrimitiveWrapper = ThirtyTwoBytes(
				value: val, instantiationContext: "TxRemoveInput.swift::\(#function):\(#line)")
		...
		public init(channelIdArg: [UInt8], serialIdArg: UInt64) {
			...
			let channelIdArgPrimitiveWrapper = ThirtyTwoBytes(
				value: channelIdArg, instantiationContext: "TxRemoveInput.swift::\(#function):\(#line)")
			...
			super
				.init(
					conflictAvoidingVariableName: 0, instantiationContext: "TxRemoveInput.swift::\(#function):\(#line)")
		...
		internal func clone() -> TxRemoveInput {
			...
			// return value (do some wrapping)
			let returnValue = TxRemoveInput(
				cType: nativeCallResult, instantiationContext: "TxRemoveInput.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKTxRemoveInput>) in
					TxRemoveInput_hash(oPointer)
				}
		...
		public class func read(ser: [UInt8]) -> Result_TxRemoveInputDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "TxRemoveInput.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing TxRemoveInput \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing TxRemoveInput \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
