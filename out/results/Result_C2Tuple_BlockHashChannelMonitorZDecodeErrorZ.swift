				...
				CResult_C2Tuple_BlockHashChannelMonitorZDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> ([UInt8], ChannelMonitor)? {
						if self.cType?.result_ok == true {
							return Tuple_BlockHashChannelMonitorZ(cType: self.cType!.contents.result.pointee, instantiationContext: "#{swift_class_name}::\(#function):\(#line)", anchor: self).getValue()
						}
						return nil
					}
