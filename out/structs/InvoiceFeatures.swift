...
extension Bindings {
	...
	public class InvoiceFeatures: NativeTypeWrapper {
		...
		internal init(
			cType: LDKInvoiceFeatures, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> InvoiceFeatures {
			...
			let returnValue = InvoiceFeatures(
				cType: nativeCallResult, instantiationContext: "InvoiceFeatures.swift::\(#function):\(#line)")
		...
		public class func read(ser: [UInt8]) -> Result_InvoiceFeaturesDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "InvoiceFeatures.swift::\(#function):\(#line)")
		...
		public func setVariableLengthOnionOptional() {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisArgPointer: UnsafeMutablePointer<LDKInvoiceFeatures>) in
		...
		internal func dangleRecursively() -> InvoiceFeatures {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing InvoiceFeatures \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing InvoiceFeatures \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
