			...
			extension Bindings {
				...
				public class Result_PaymentParametersDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_PaymentParametersDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_PaymentParametersDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> PaymentParameters? {
						if self.cType?.result_ok == true {
							return PaymentParameters(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
