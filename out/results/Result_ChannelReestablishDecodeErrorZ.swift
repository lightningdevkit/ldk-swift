			...
			extension Bindings {
				...
				public class Result_ChannelReestablishDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_ChannelReestablishDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_ChannelReestablishDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> ChannelReestablish? {
						if self.cType?.result_ok == true {
							return ChannelReestablish(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
