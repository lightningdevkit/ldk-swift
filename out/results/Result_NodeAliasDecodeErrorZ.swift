			...
			extension Bindings {
				...
				public class Result_NodeAliasDecodeErrorZ: NativeTypeWrapper {
					...
					public class func initWithOk(o: NodeAlias) -> Result_NodeAliasDecodeErrorZ {
						...
						let returnValue = Result_NodeAliasDecodeErrorZ(cType: nativeCallResult, instantiationContext: "#{swift_class_name}::\(#function):\(#line)")
				...
				CResult_NodeAliasDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> NodeAlias? {
						if self.cType?.result_ok == true {
							return NodeAlias(cType: self.cType!.contents.result.pointee, instantiationContext: "#{swift_class_name}::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
