			...
			extension Bindings {
				...
				public class BlindedHopFeatures: NativeTypeWrapper {
					...
					internal init(cType: LDKBlindedHopFeatures, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				BlindedHopFeatures_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = BlindedHopFeatures(cType: nativeCallResult, instantiationContext: "BlindedHopFeatures.swift::\(#function):\(#line)")
				...
				BlindedHopFeatures_requires_unknown_bits(thisArgPointer)
					...
					public func setRequiredFeatureBit(bit: UInt) -> Result_NoneNoneZ {
						...
						// native method call
						let nativeCallResult = 
						withUnsafeMutablePointer(to: &self.cType!) { (thisArgPointer: UnsafeMutablePointer<LDKBlindedHopFeatures>) in
				BlindedHopFeatures_set_required_feature_bit(thisArgPointer, bit)
						}
						...
						// return value (do some wrapping)
						let returnValue = Result_NoneNoneZ(cType: nativeCallResult, instantiationContext: "BlindedHopFeatures.swift::\(#function):\(#line)", anchor: self).dangle(false)
					...
					public func setOptionalFeatureBit(bit: UInt) -> Result_NoneNoneZ {
						...
						let nativeCallResult = 
						withUnsafeMutablePointer(to: &self.cType!) { (thisArgPointer: UnsafeMutablePointer<LDKBlindedHopFeatures>) in
				BlindedHopFeatures_set_optional_feature_bit(thisArgPointer, bit)
						}
				...
				BlindedHopFeatures_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "BlindedHopFeatures.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public class func read(ser: [UInt8]) -> Result_BlindedHopFeaturesDecodeErrorZ {
						...
						// return value (do some wrapping)
						let returnValue = Result_BlindedHopFeaturesDecodeErrorZ(cType: nativeCallResult, instantiationContext: "BlindedHopFeatures.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing BlindedHopFeatures \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing BlindedHopFeatures \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
