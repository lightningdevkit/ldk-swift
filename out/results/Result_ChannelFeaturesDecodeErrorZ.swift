			...
			extension Bindings {
				...
				public class Result_ChannelFeaturesDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_ChannelFeaturesDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_ChannelFeaturesDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> ChannelFeatures? {
						if self.cType?.result_ok == true {
							return ChannelFeatures(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
