			...
			extension Bindings {
				...
				public class BlindedHop: NativeTypeWrapper {
					...
					internal init(cType: LDKBlindedHop, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				BlindedHop_get_blinded_node_id(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = PublicKey(cType: nativeCallResult, instantiationContext: "BlindedHop.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public func setBlindedNodeId(val: [UInt8]) {
						...
						let valPrimitiveWrapper = PublicKey(value: val, instantiationContext: "BlindedHop.swift::\(#function):\(#line)")
				...
				BlindedHop_get_encrypted_payload(thisPtrPointer)
					...
					public func setEncryptedPayload(val: [UInt8]) {
						...
						let valVector = Vec_u8Z(array: val, instantiationContext: "BlindedHop.swift::\(#function):\(#line)").dangle()
				...
				BlindedHop_set_encrypted_payload(thisPtrPointer, valVector.cType!)
					...
					public init(blindedNodeIdArg: [UInt8], encryptedPayloadArg: [UInt8]) {
						...
						let blindedNodeIdArgPrimitiveWrapper = PublicKey(value: blindedNodeIdArg, instantiationContext: "BlindedHop.swift::\(#function):\(#line)")
				
						let encryptedPayloadArgVector = Vec_u8Z(array: encryptedPayloadArg, instantiationContext: "BlindedHop.swift::\(#function):\(#line)").dangle()
				

						// native method call
						let nativeCallResult = BlindedHop_new(blindedNodeIdArgPrimitiveWrapper.cType!, encryptedPayloadArgVector.cType!)
				...
				BlindedHop_write(objPointer)
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing BlindedHop \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
