			...
			extension Bindings {
				...
				public class OnionMessagePath: NativeTypeWrapper {
					...
					internal init(cType: LDKOnionMessagePath, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				OnionMessagePath_get_intermediate_nodes(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_PublicKeyZ(cType: nativeCallResult, instantiationContext: "OnionMessagePath.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public func setIntermediateNodes(val: [[UInt8]]) {
						...
						let valVector = Vec_PublicKeyZ(array: val, instantiationContext: "OnionMessagePath.swift::\(#function):\(#line)").dangle()
				

						// native method call
						let nativeCallResult = 
						withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKOnionMessagePath>) in
				...
				OnionMessagePath_get_destination(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Destination(cType: nativeCallResult, instantiationContext: "OnionMessagePath.swift::\(#function):\(#line)", anchor: self)
				...
				OnionMessagePath_set_destination(thisPtrPointer, val.danglingClone().cType!)
					...
					public init(intermediateNodesArg: [[UInt8]], destinationArg: Destination) {
						...
						let intermediateNodesArgVector = Vec_PublicKeyZ(array: intermediateNodesArg, instantiationContext: "OnionMessagePath.swift::\(#function):\(#line)").dangle()
				

						// native method call
						let nativeCallResult = OnionMessagePath_new(intermediateNodesArgVector.cType!, destinationArg.danglingClone().cType!)
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "OnionMessagePath.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing OnionMessagePath \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing OnionMessagePath \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
