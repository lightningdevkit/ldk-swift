...
extension Bindings {
	...
	public class DefaultMessageRouter: NativeTypeWrapper {
		...
		internal init(
			cType: LDKDefaultMessageRouter, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func asMessageRouter() -> MessageRouter {
			...
			let returnValue = NativelyImplementedMessageRouter(
				cType: nativeCallResult, instantiationContext: "DefaultMessageRouter.swift::\(#function):\(#line)",
				anchor: self)
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing DefaultMessageRouter \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing DefaultMessageRouter \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
