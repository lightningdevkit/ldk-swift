			...
			extension Bindings {
				...
				public class OnionMessenger: NativeTypeWrapper {
					...
					public init(entropySource: EntropySource, nodeSigner: NodeSigner, logger: Logger, messageRouter: MessageRouter, offersHandler: OffersMessageHandler, customHandler: CustomOnionMessageHandler) {
						// native call variable prep
						

						// native method call
						let nativeCallResult = OnionMessenger_new(entropySource.activate().cType!, nodeSigner.activate().cType!, logger.activate().cType!, messageRouter.activate().cType!, offersHandler.activate().cType!, customHandler.activate().cType!)
				...
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "OnionMessenger.swift::\(#function):\(#line)")
					...
					public func sendOnionMessage(path: OnionMessagePath, message: OnionMessageContents, replyPath: BlindedPath) -> Result_NoneSendErrorZ {
						// native call variable prep
				...
				OnionMessenger_send_onion_message(thisArgPointer, path.dynamicallyDangledClone().cType!, message.danglingClone().cType!, replyPath.dynamicallyDangledClone().cType!)
						}
