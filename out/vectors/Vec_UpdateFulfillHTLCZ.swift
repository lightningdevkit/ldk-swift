...
extension Bindings {
	...
	internal class Vec_UpdateFulfillHTLCZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_UpdateFulfillHTLCZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func getValue() -> [UpdateFulfillHTLC] {
			...
			let swiftArray = array.map { (currentCType: LDKUpdateFulfillHTLC) -> UpdateFulfillHTLC in
				UpdateFulfillHTLC(
					cType: currentCType, instantiationContext: "Vec_UpdateFulfillHTLCZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		internal func dangleRecursively() -> Vec_UpdateFulfillHTLCZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_UpdateFulfillHTLCZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Vec_UpdateFulfillHTLCZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
