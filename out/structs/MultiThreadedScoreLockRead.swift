...
extension Bindings {
	...
	public class MultiThreadedScoreLockRead: NativeTypeWrapper {
		...
		internal init(
			cType: LDKMultiThreadedScoreLockRead, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func asScoreLookUp() -> ScoreLookUp {
			...
			let returnValue = NativelyImplementedScoreLookUp(
				...
				instantiationContext: "MultiThreadedScoreLockRead.swift::\(#function):\(#line)", anchor: self)
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing MultiThreadedScoreLockRead \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing MultiThreadedScoreLockRead \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
