			...
			extension Bindings {
				...
				public class Result_ThirtyTwoBytesRetryableSendFailureZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_ThirtyTwoBytesRetryableSendFailureZ, instantiationContext: String, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(cType: LDKCResult_ThirtyTwoBytesRetryableSendFailureZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: [UInt8]) -> Result_ThirtyTwoBytesRetryableSendFailureZ {
						...
						let oPrimitiveWrapper = ThirtyTwoBytes(value: o, instantiationContext: "Result_ThirtyTwoBytesRetryableSendFailureZ.swift::\(#function):\(#line)")
						...
						let returnValue = Result_ThirtyTwoBytesRetryableSendFailureZ(cType: nativeCallResult, instantiationContext: "Result_ThirtyTwoBytesRetryableSendFailureZ.swift::\(#function):\(#line)")
					...
					internal func clone() -> Result_ThirtyTwoBytesRetryableSendFailureZ {
						...
						withUnsafePointer(to: self.cType!) { (origPointer: UnsafePointer<LDKCResult_ThirtyTwoBytesRetryableSendFailureZ>) in
				CResult_ThirtyTwoBytesRetryableSendFailureZ_clone(origPointer)
					...
					public func getValue() -> [UInt8]? {
						if self.cType?.result_ok == true {
							return ThirtyTwoBytes(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_ThirtyTwoBytesRetryableSendFailureZ.swift::\(#function):\(#line)", anchor: self).getValue()
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_ThirtyTwoBytesRetryableSendFailureZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_ThirtyTwoBytesRetryableSendFailureZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
