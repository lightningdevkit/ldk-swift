			...
			extension Bindings {
				...
				public class Result_ThirtyTwoBytesAPIErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_ThirtyTwoBytesAPIErrorZ, instantiationContext: String, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(cType: LDKCResult_ThirtyTwoBytesAPIErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: [UInt8]) -> Result_ThirtyTwoBytesAPIErrorZ {
						...
						let oPrimitiveWrapper = ThirtyTwoBytes(value: o, instantiationContext: "Result_ThirtyTwoBytesAPIErrorZ.swift::\(#function):\(#line)")
						...
						let returnValue = Result_ThirtyTwoBytesAPIErrorZ(cType: nativeCallResult, instantiationContext: "Result_ThirtyTwoBytesAPIErrorZ.swift::\(#function):\(#line)")
				...
				CResult_ThirtyTwoBytesAPIErrorZ_clone(origPointer)
					...
					public func getError() -> APIError? {
						if self.cType?.result_ok == false {
							return APIError(cType: self.cType!.contents.err.pointee, instantiationContext: "Result_ThirtyTwoBytesAPIErrorZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					public func getValue() -> [UInt8]? {
						if self.cType?.result_ok == true {
							return ThirtyTwoBytes(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_ThirtyTwoBytesAPIErrorZ.swift::\(#function):\(#line)", anchor: self).getValue()
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_ThirtyTwoBytesAPIErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_ThirtyTwoBytesAPIErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
