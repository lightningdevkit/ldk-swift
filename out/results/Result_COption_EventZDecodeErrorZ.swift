...
extension Bindings {
	...
	public class Result_COption_EventZDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_COption_EventZDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: Event?) -> Result_COption_EventZDecodeErrorZ {
			...
			let oOption = Option_EventZ(
				some: o, instantiationContext: "Result_COption_EventZDecodeErrorZ.swift::\(#function):\(#line)"
			)
			.danglingClone()
			...
			let returnValue = Result_COption_EventZDecodeErrorZ(
				...
				instantiationContext: "Result_COption_EventZDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_COption_EventZDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_COption_EventZDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_COption_EventZDecodeErrorZ.swift::\(#function):\(#line)", anchor: self
				)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_COption_EventZDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_COption_EventZDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_COption_EventZDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
