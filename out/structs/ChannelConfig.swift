...
extension Bindings {
	...
	public class ChannelConfig: NativeTypeWrapper {
		...
		internal init(
			cType: LDKChannelConfig, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getMaxDustHtlcExposure() -> MaxDustHTLCExposure {
			// native call variable prep
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (thisPtrPointer: UnsafePointer<LDKChannelConfig>) in
					ChannelConfig_get_max_dust_htlc_exposure(thisPtrPointer)
				}
			...
			// return value (do some wrapping)
			let returnValue = MaxDustHTLCExposure(
				cType: nativeCallResult, instantiationContext: "ChannelConfig.swift::\(#function):\(#line)",
				anchor: self)
		...
		///
		/// Default value: [`MaxDustHTLCExposure::FeeRateMultiplier`] with a multiplier of 5000.
		public func setMaxDustHtlcExposure(val: MaxDustHTLCExposure) {
			// native call variable prep
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKChannelConfig>) in
					ChannelConfig_set_max_dust_htlc_exposure(thisPtrPointer, val.danglingClone().cType!)
				}
		...
		public func getAcceptUnderpayingHtlcs() -> Bool {
			// native call variable prep
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (thisPtrPointer: UnsafePointer<LDKChannelConfig>) in
					ChannelConfig_get_accept_underpaying_htlcs(thisPtrPointer)
				}
		...
		/// [`PaymentClaimable::amount_msat`]: crate::events::Event::PaymentClaimable::amount_msat
		/// [`PaymentClaimable::counterparty_skimmed_fee_msat`]: crate::events::Event::PaymentClaimable::counterparty_skimmed_fee_msat
		public func setAcceptUnderpayingHtlcs(val: Bool) {
			// native call variable prep
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKChannelConfig>) in
					ChannelConfig_set_accept_underpaying_htlcs(thisPtrPointer, val)
				}
		...
		public init(
			...
			cltvExpiryDeltaArg: UInt16, maxDustHtlcExposureArg: MaxDustHTLCExposure,
			forceCloseAvoidanceMaxFeeSatoshisArg: UInt64, acceptUnderpayingHtlcsArg: Bool
		) {
			// native call variable prep


			// native method call
			let nativeCallResult = ChannelConfig_new(
				forwardingFeeProportionalMillionthsArg, forwardingFeeBaseMsatArg, cltvExpiryDeltaArg,
				maxDustHtlcExposureArg.danglingClone().cType!, forceCloseAvoidanceMaxFeeSatoshisArg,
				acceptUnderpayingHtlcsArg)
			...
			super
				.init(
					conflictAvoidingVariableName: 0, instantiationContext: "ChannelConfig.swift::\(#function):\(#line)")
		...
		public func apply(update: ChannelConfigUpdate) {
			// native call variable prep


			// native method call
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) { (thisArgPointer: UnsafeMutablePointer<LDKChannelConfig>) in

					withUnsafePointer(to: update.cType!) { (updatePointer: UnsafePointer<LDKChannelConfigUpdate>) in
						ChannelConfig_apply(thisArgPointer, updatePointer)
					}
		...
		public class func read(ser: [UInt8]) -> Result_ChannelConfigDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "ChannelConfig.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_ChannelConfigDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "ChannelConfig.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing ChannelConfig \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing ChannelConfig \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
