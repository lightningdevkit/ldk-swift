			...
			extension Bindings {
				...
				internal class Tuple_OutPointCVec_MonitorUpdateIdZZ: NativeTypeWrapper {
					...
					internal init(cType: LDKC2Tuple_OutPointCVec_MonitorUpdateIdZZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				C2Tuple_OutPointCVec_MonitorUpdateIdZZ_clone(origPointer)
						...
						let returnValue = Tuple_OutPointCVec_MonitorUpdateIdZZ(cType: nativeCallResult)
				...
				super.init(conflictAvoidingVariableName: 0)
					...
					public func getB() -> [MonitorUpdateId] {
						// return value (do some wrapping)
						let returnValue = Vec_MonitorUpdateIdZ(cType: self.cType!.b, anchor: self).dangle().getValue()
