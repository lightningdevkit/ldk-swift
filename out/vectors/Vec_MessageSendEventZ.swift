...
extension Bindings {
	...
	internal class Vec_MessageSendEventZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_MessageSendEventZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal init(array: [MessageSendEvent], instantiationContext: String) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			super.init(conflictAvoidingVariableName: 0, instantiationContext: instantiationContext)
		...
		public func getValue() -> [MessageSendEvent] {
			...
			let swiftArray = array.map { (currentCType: LDKMessageSendEvent) -> MessageSendEvent in
				MessageSendEvent(
					cType: currentCType, instantiationContext: "Vec_MessageSendEventZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		internal func dangleRecursively() -> Vec_MessageSendEventZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_MessageSendEventZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Vec_MessageSendEventZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
