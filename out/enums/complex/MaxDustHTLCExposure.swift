...
extension Bindings {
	...
	public class MaxDustHTLCExposure: NativeTypeWrapper {
		...
		internal init(
			cType: LDKMaxDustHTLCExposure, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> MaxDustHTLCExposure {
			...
			let returnValue = MaxDustHTLCExposure(
				cType: nativeCallResult, instantiationContext: "MaxDustHTLCExposure.swift::\(#function):\(#line)")
		...
		public class func read(ser: [UInt8]) -> Result_MaxDustHTLCExposureDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "MaxDustHTLCExposure.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing MaxDustHTLCExposure \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing MaxDustHTLCExposure \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
