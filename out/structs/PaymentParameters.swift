			...
			extension Bindings {
				...
				PaymentParameters_get_payee(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Payee(cType: nativeCallResult, instantiationContext: "PaymentParameters.swift::\(#function):\(#line)", anchor: self)
				...
				PaymentParameters_set_previously_failed_channels(thisPtrPointer, valVector.cType!)
					...
					public init(payeeArg: Payee, expiryTimeArg: UInt64?, maxTotalCltvExpiryDeltaArg: UInt32, maxPathCountArg: UInt8, maxChannelSaturationPowerOfHalfArg: UInt8, previouslyFailedChannelsArg: [UInt64]) {
						// native call variable prep
						...
						// native method call
						let nativeCallResult = PaymentParameters_new(payeeArg.danglingClone().cType!, expiryTimeArgOption.cType!, maxTotalCltvExpiryDeltaArg, maxPathCountArg, maxChannelSaturationPowerOfHalfArg, previouslyFailedChannelsArgVector.cType!)
				...
				PaymentParameters_write(objPointer)
					...
					public class func initForKeysend(payeePubkey: [UInt8], finalCltvExpiryDelta: UInt32, allowMpp: Bool) -> PaymentParameters {
						// native call variable prep
						...
						// native method call
						let nativeCallResult = PaymentParameters_for_keysend(payeePubkeyPrimitiveWrapper.cType!, finalCltvExpiryDelta, allowMpp)
...
return returnValue
					...
					public class func initWithBlinded(blindedRouteHints: [(BlindedPayInfo, BlindedPath)]) -> PaymentParameters {
						...
						let blindedRouteHintsVector = Vec_C2Tuple_BlindedPayInfoBlindedPathZZ(array: blindedRouteHints, instantiationContext: "PaymentParameters.swift::\(#function):\(#line)").dangle()
