...
extension Bindings {
	...
	public class Result_NodeAnnouncementDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_NodeAnnouncementDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: NodeAnnouncement) -> Result_NodeAnnouncementDecodeErrorZ {
			...
			let returnValue = Result_NodeAnnouncementDecodeErrorZ(
				...
				instantiationContext: "Result_NodeAnnouncementDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_NodeAnnouncementDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_NodeAnnouncementDecodeErrorZ>) in
		...
		internal func dangleRecursively() -> Result_NodeAnnouncementDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_NodeAnnouncementDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_NodeAnnouncementDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
