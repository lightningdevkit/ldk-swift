			...
			extension Bindings {
				...
				public class Result_RetryDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_RetryDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: Retry) -> Result_RetryDecodeErrorZ {
						...
						let returnValue = Result_RetryDecodeErrorZ(cType: nativeCallResult, instantiationContext: "Result_RetryDecodeErrorZ.swift::\(#function):\(#line)")
				...
				CResult_RetryDecodeErrorZ_clone(origPointer)
					...
					public func getError() -> DecodeError? {
						if self.cType?.result_ok == false {
							return DecodeError(cType: self.cType!.contents.err.pointee, instantiationContext: "Result_RetryDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					public func getValue() -> Retry? {
						if self.cType?.result_ok == true {
							return Retry(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_RetryDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_RetryDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_RetryDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
