...
extension Bindings {
	...
	public class Result_ChannelDetailsDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_ChannelDetailsDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: ChannelDetails) -> Result_ChannelDetailsDecodeErrorZ {
			...
			let returnValue = Result_ChannelDetailsDecodeErrorZ(
				...
				instantiationContext: "Result_ChannelDetailsDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_ChannelDetailsDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_ChannelDetailsDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_ChannelDetailsDecodeErrorZ.swift::\(#function):\(#line)", anchor: self
				)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_ChannelDetailsDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_ChannelDetailsDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_ChannelDetailsDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
