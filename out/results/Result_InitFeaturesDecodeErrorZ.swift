			...
			extension Bindings {
				...
				public class Result_InitFeaturesDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_InitFeaturesDecodeErrorZ) {
					...
					internal init(cType: LDKCResult_InitFeaturesDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_InitFeaturesDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> InitFeatures? {
						if self.cType?.result_ok == true {
							return InitFeatures(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
