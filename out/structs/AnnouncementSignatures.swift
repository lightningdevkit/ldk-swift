			...
			extension Bindings {
				...
				AnnouncementSignatures_get_node_signature(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = ECDSASignature(cType: nativeCallResult, instantiationContext: "AnnouncementSignatures.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public func setNodeSignature(val: [UInt8]) {
						...
						let valPrimitiveWrapper = ECDSASignature(value: val, instantiationContext: "AnnouncementSignatures.swift::\(#function):\(#line)")
				...
				AnnouncementSignatures_set_bitcoin_signature(thisPtrPointer, valPrimitiveWrapper.cType!)
					...
					public init(channelIdArg: [UInt8], shortChannelIdArg: UInt64, nodeSignatureArg: [UInt8], bitcoinSignatureArg: [UInt8]) {
						...
						let nodeSignatureArgPrimitiveWrapper = ECDSASignature(value: nodeSignatureArg, instantiationContext: "AnnouncementSignatures.swift::\(#function):\(#line)")
				
						let bitcoinSignatureArgPrimitiveWrapper = ECDSASignature(value: bitcoinSignatureArg, instantiationContext: "AnnouncementSignatures.swift::\(#function):\(#line)")
