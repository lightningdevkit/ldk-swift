			...
			extension Bindings {
				...
				internal class Vec_C2Tuple_TxidBlockHashZZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_C2Tuple_TxidBlockHashZZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(array: [([UInt8], [UInt8])], instantiationContext: String) {
						...
						let currentValueDepth1Tuple = Tuple_TxidBlockHashZ(tuple: currentValueDepth1, instantiationContext: "Vec_C2Tuple_TxidBlockHashZZ.swift::\(#function):\(#line)").danglingClone()
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Vec_C2Tuple_TxidBlockHashZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Vec_C2Tuple_TxidBlockHashZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
