...
extension Bindings {
	...
	public class Result_PaymentHashRetryableSendFailureZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_PaymentHashRetryableSendFailureZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: [UInt8]) -> Result_PaymentHashRetryableSendFailureZ {
			...
			let oPrimitiveWrapper = ThirtyTwoBytes(
				value: o, instantiationContext: "Result_PaymentHashRetryableSendFailureZ.swift::\(#function):\(#line)")
			...
			let returnValue = Result_PaymentHashRetryableSendFailureZ(
				...
				instantiationContext: "Result_PaymentHashRetryableSendFailureZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_PaymentHashRetryableSendFailureZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_PaymentHashRetryableSendFailureZ>) in
		...
		internal func dangleRecursively() -> Result_PaymentHashRetryableSendFailureZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_PaymentHashRetryableSendFailureZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_PaymentHashRetryableSendFailureZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
