...
extension Bindings {
	...
	open class Persister: NativeTraitWrapper {
		...
		internal init(
			cType: LDKPersister, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init() {
				...
				-> LDKCResult_NoneErrorZ
			{
				...
				// Swift callback call
				let swiftCallbackResult = instance.persistManager(
					channelManager: ChannelManager(
						cType: channel_manager.pointee,
						instantiationContext: "Persister.swift::init()::\(#function):\(#line)"
					)
					.dangle())
				...
				-> LDKCResult_NoneErrorZ
			{
				...
				// Swift callback call
				let swiftCallbackResult = instance.persistGraph(
					networkGraph: NetworkGraph(
						cType: network_graph.pointee,
						instantiationContext: "Persister.swift::init()::\(#function):\(#line)"
					)
					.dangle())
				...
				-> LDKCResult_NoneErrorZ
			{
				...
				// Swift callback call
				let swiftCallbackResult = instance.persistScorer(
					scorer: NativelyImplementedWriteableScore(
						cType: scorer.pointee, instantiationContext: "Persister.swift::init()::\(#function):\(#line)"))
			...
			func freeLambda(this_arg: UnsafeMutableRawPointer?) {
				let instance: Persister = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "Persister::freeLambda")
		...
		internal func dangleRecursively() -> Persister {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Persister \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
	}
