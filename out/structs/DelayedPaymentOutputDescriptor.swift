...
extension Bindings {
	...
	public class DelayedPaymentOutputDescriptor: NativeTypeWrapper {
		...
		internal init(
			cType: LDKDelayedPaymentOutputDescriptor, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> DelayedPaymentOutputDescriptor {
			...
			// return value (do some wrapping)
			let returnValue = DelayedPaymentOutputDescriptor(
				cType: nativeCallResult,
				instantiationContext: "DelayedPaymentOutputDescriptor.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKDelayedPaymentOutputDescriptor>) in
					DelayedPaymentOutputDescriptor_hash(oPointer)
				}
		...
		public class func read(ser: [UInt8]) -> Result_DelayedPaymentOutputDescriptorDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "DelayedPaymentOutputDescriptor.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing DelayedPaymentOutputDescriptor \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing DelayedPaymentOutputDescriptor \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
