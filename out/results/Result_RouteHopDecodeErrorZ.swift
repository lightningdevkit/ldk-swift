...
extension Bindings {
	...
	public class Result_RouteHopDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_RouteHopDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: RouteHop) -> Result_RouteHopDecodeErrorZ {
			...
			let returnValue = Result_RouteHopDecodeErrorZ(
				cType: nativeCallResult,
				instantiationContext: "Result_RouteHopDecodeErrorZ.swift::\(#function):\(#line)")
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_RouteHopDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_RouteHopDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_RouteHopDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_RouteHopDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
