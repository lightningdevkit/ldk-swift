...
extension Bindings {
	...
	public class Result_PhantomRouteHintsDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_PhantomRouteHintsDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: PhantomRouteHints) -> Result_PhantomRouteHintsDecodeErrorZ {
			...
			let returnValue = Result_PhantomRouteHintsDecodeErrorZ(
				...
				instantiationContext: "Result_PhantomRouteHintsDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_PhantomRouteHintsDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_PhantomRouteHintsDecodeErrorZ>) in
		...
		internal func dangleRecursively() -> Result_PhantomRouteHintsDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_PhantomRouteHintsDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_PhantomRouteHintsDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
