...
extension Bindings {
	...
	internal class Option_TypeZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCOption_TypeZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> Option_TypeZ {
			...
			// return value (do some wrapping)
			let returnValue = Option_TypeZ(
				cType: nativeCallResult, instantiationContext: "Option_TypeZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> BindingsType? {
			...
			if self.cType!.tag == LDKCOption_TypeZ_Some {
				return NativelyImplementedBindingsType(
					cType: self.cType!.some, instantiationContext: "Option_TypeZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		internal func dangleRecursively() -> Option_TypeZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Option_TypeZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Option_TypeZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
