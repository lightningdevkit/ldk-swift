...
extension Bindings {
	...
	public class ExpiryTime: NativeTypeWrapper {
		...
		internal init(
			cType: LDKExpiryTime, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> ExpiryTime {
			...
			let returnValue = ExpiryTime(
				cType: nativeCallResult, instantiationContext: "ExpiryTime.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing ExpiryTime \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
