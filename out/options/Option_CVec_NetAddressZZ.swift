			...
			extension Bindings {
				...
				internal class Option_CVec_NetAddressZZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCOption_CVec_NetAddressZZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				COption_CVec_NetAddressZZ_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_CVec_NetAddressZZ(cType: nativeCallResult, instantiationContext: "Option_CVec_NetAddressZZ.swift::\(#function):\(#line)")
					...
					public func getValue() -> [NetAddress]? {
						...
						if self.cType!.tag == LDKCOption_CVec_NetAddressZZ_Some {
							return Vec_NetAddressZ(cType: self.cType!.some, instantiationContext: "Option_CVec_NetAddressZZ.swift::\(#function):\(#line)", anchor: self).dangle().getValue()
						}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Option_CVec_NetAddressZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Option_CVec_NetAddressZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
