			...
			extension Bindings {
				...
				internal class Vec_UpdateFailMalformedHTLCZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_UpdateFailMalformedHTLCZ, instantiationContext: String) {
					...
					internal init(cType: LDKCVec_UpdateFailMalformedHTLCZ, instantiationContext: String, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(cType: LDKCVec_UpdateFailMalformedHTLCZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
...
UpdateFailMalformedHTLC(cType: currentCType, instantiationContext: "Vec_UpdateFailMalformedHTLCZ.swift::\(#function):\(#line)", anchor: self).dangle()
						}
						return swiftArray
					}
					...
					internal func dangle(_ shouldDangle: Bool = true) -> Vec_UpdateFailMalformedHTLCZ {
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Vec_UpdateFailMalformedHTLCZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Vec_UpdateFailMalformedHTLCZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
