...
extension Bindings {
	...
	public class Result_ChannelConfigDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_ChannelConfigDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: ChannelConfig) -> Result_ChannelConfigDecodeErrorZ {
			...
			let returnValue = Result_ChannelConfigDecodeErrorZ(
				...
				instantiationContext: "Result_ChannelConfigDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_ChannelConfigDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_ChannelConfigDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_ChannelConfigDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_ChannelConfigDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_ChannelConfigDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_ChannelConfigDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
