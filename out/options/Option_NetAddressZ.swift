...
extension Bindings {
	...
	internal class Option_NetAddressZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCOption_NetAddressZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> Option_NetAddressZ {
			...
			let returnValue = Option_NetAddressZ(
				cType: nativeCallResult, instantiationContext: "Option_NetAddressZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> NetAddress? {
			...
			if self.cType!.tag == LDKCOption_NetAddressZ_Some {
				return NetAddress(
					cType: self.cType!.some, instantiationContext: "Option_NetAddressZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Option_NetAddressZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Option_NetAddressZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
