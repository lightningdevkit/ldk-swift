...
extension Bindings {
	...
	internal class Tuple__u168_u168Z: NativeTypeWrapper {
		...
		internal init(
			cType: LDKC2Tuple__u168_u168Z, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> Tuple__u168_u168Z {
			...
			let returnValue = Tuple__u168_u168Z(
				cType: nativeCallResult, instantiationContext: "Tuple__u168_u168Z.swift::\(#function):\(#line)")
		...
		public init(a: [UInt16], b: [UInt16], instantiationContext: String) {
			...
			let aPrimitiveWrapper = EightU16s(
				value: a, instantiationContext: "Tuple__u168_u168Z.swift::\(#function):\(#line)")

			let bPrimitiveWrapper = EightU16s(
				value: b, instantiationContext: "Tuple__u168_u168Z.swift::\(#function):\(#line)")
		...
		public func getA() -> [UInt16] {
			...
			let returnValue = EightU16s(
				cType: self.cType!.a, instantiationContext: "Tuple__u168_u168Z.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle().getValue()
		...
		public func getB() -> [UInt16] {
			// return value (do some wrapping)
			let returnValue = EightU16s(
				cType: self.cType!.b, instantiationContext: "Tuple__u168_u168Z.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle().getValue()
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Tuple__u168_u168Z \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Tuple__u168_u168Z \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
