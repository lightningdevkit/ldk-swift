			...
			extension Bindings {
				...
				internal class Tuple_SignatureCVec_SignatureZZ: NativeTypeWrapper {
					...
					internal init(cType: LDKC2Tuple_SignatureCVec_SignatureZZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Tuple_SignatureCVec_SignatureZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Tuple_SignatureCVec_SignatureZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
