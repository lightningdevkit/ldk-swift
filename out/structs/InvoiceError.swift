...
extension Bindings {
	...
	public class InvoiceError: NativeTypeWrapper {
		...
		internal init(
			cType: LDKInvoiceError, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getErroneousField() -> ErroneousField? {
			...
			let returnValue = ErroneousField(
				cType: nativeCallResult, instantiationContext: "InvoiceError.swift::\(#function):\(#line)", anchor: self
			)
			.dangle(false)
		...
		public init(erroneousFieldArg: ErroneousField, messageArg: UntrustedString) {
			...
			// native method call
			let nativeCallResult = InvoiceError_new(
				erroneousFieldArg.dynamicallyDangledClone().cType!, messageArg.dynamicallyDangledClone().cType!)
			...
			super
				.init(
					conflictAvoidingVariableName: 0, instantiationContext: "InvoiceError.swift::\(#function):\(#line)")
		...
		public class func read(ser: [UInt8]) -> Result_InvoiceErrorDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "InvoiceError.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing InvoiceError \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing InvoiceError \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
