...
extension Bindings {
	...
	internal class Option_PathFailureZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCOption_PathFailureZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> Option_PathFailureZ {
			...
			// return value (do some wrapping)
			let returnValue = Option_PathFailureZ(
				cType: nativeCallResult, instantiationContext: "Option_PathFailureZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> PathFailure? {
			...
			if self.cType!.tag == LDKCOption_PathFailureZ_Some {
				return PathFailure(
					cType: self.cType!.some, instantiationContext: "Option_PathFailureZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		internal func dangleRecursively() -> Option_PathFailureZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Option_PathFailureZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Option_PathFailureZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
