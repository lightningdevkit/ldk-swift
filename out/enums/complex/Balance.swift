			...
			extension Bindings {
				...
				Balance_clone(origPointer)
					...
					public class func initWithClaimableAwaitingConfirmations(amountSatoshis: UInt64, confirmationHeight: UInt32) -> Balance {
						// native call variable prep
					...
					public class func initWithContentiousClaimable(amountSatoshis: UInt64, timeoutHeight: UInt32, paymentHash: [UInt8], paymentPreimage: [UInt8]) -> Balance {
						// native call variable prep
						
						let paymentHashPrimitiveWrapper = ThirtyTwoBytes(value: paymentHash, instantiationContext: "Balance.swift::\(#function):\(#line)")
				
						let paymentPreimagePrimitiveWrapper = ThirtyTwoBytes(value: paymentPreimage, instantiationContext: "Balance.swift::\(#function):\(#line)")
				

						// native method call
						let nativeCallResult = Balance_contentious_claimable(amountSatoshis, timeoutHeight, paymentHashPrimitiveWrapper.cType!, paymentPreimagePrimitiveWrapper.cType!)
					...
					public class func initWithMaybeTimeoutClaimableHtlc(amountSatoshis: UInt64, claimableHeight: UInt32, paymentHash: [UInt8]) -> Balance {
						// native call variable prep
						...
						// native method call
						let nativeCallResult = Balance_maybe_timeout_claimable_htlc(amountSatoshis, claimableHeight, paymentHashPrimitiveWrapper.cType!)
					...
					public class func initWithMaybePreimageClaimableHtlc(amountSatoshis: UInt64, expiryHeight: UInt32, paymentHash: [UInt8]) -> Balance {
						// native call variable prep
						...
						// native method call
						let nativeCallResult = Balance_maybe_preimage_claimable_htlc(amountSatoshis, expiryHeight, paymentHashPrimitiveWrapper.cType!)
				...
				Balance_claimable_amount_satoshis(thisArgPointer)
					...
					public class ContentiousClaimable: NativeTypeWrapper {
						...
						public func getPaymentHash() -> [UInt8] {
							// return value (do some wrapping)
							let returnValue = ThirtyTwoBytes(cType: self.cType!.payment_hash, instantiationContext: "Balance.swift::\(#function):\(#line)", anchor: self).getValue()
						...
						public func getPaymentPreimage() -> [UInt8] {
							// return value (do some wrapping)
							let returnValue = ThirtyTwoBytes(cType: self.cType!.payment_preimage, instantiationContext: "Balance.swift::\(#function):\(#line)", anchor: self).getValue()
