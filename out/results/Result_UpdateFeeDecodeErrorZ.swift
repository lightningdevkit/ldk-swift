...
extension Bindings {
	...
	public class Result_UpdateFeeDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_UpdateFeeDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: UpdateFee) -> Result_UpdateFeeDecodeErrorZ {
			...
			let returnValue = Result_UpdateFeeDecodeErrorZ(
				cType: nativeCallResult,
				instantiationContext: "Result_UpdateFeeDecodeErrorZ.swift::\(#function):\(#line)")
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_UpdateFeeDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_UpdateFeeDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_UpdateFeeDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_UpdateFeeDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
