...
extension Bindings {
	...
	internal class Vec_BalanceZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_BalanceZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getValue() -> [Balance] {
			...
			let swiftArray = array.map { (currentCType: LDKBalance) -> Balance in
				Balance(
					cType: currentCType, instantiationContext: "Vec_BalanceZ.swift::\(#function):\(#line)", anchor: self
				)
				.dangle()
			}
			return swiftArray
		...
		internal func dangleRecursively() -> Vec_BalanceZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_BalanceZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Vec_BalanceZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
