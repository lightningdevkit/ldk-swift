...
extension Bindings {
	...
	public class UpdateFulfillHTLC: NativeTypeWrapper {
		...
		internal init(
			cType: LDKUpdateFulfillHTLC, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func setChannelId(val: [UInt8]) {
			...
			let valPrimitiveWrapper = ThirtyTwoBytes(
				value: val, instantiationContext: "UpdateFulfillHTLC.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKUpdateFulfillHTLC>) in
		...
		public init(channelIdArg: [UInt8], htlcIdArg: UInt64, paymentPreimageArg: [UInt8]) {
			...
			let channelIdArgPrimitiveWrapper = ThirtyTwoBytes(
				value: channelIdArg, instantiationContext: "UpdateFulfillHTLC.swift::\(#function):\(#line)")

			let paymentPreimageArgPrimitiveWrapper = ThirtyTwoBytes(
				value: paymentPreimageArg, instantiationContext: "UpdateFulfillHTLC.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult = UpdateFulfillHTLC_new(
				channelIdArgPrimitiveWrapper.cType!, htlcIdArg, paymentPreimageArgPrimitiveWrapper.cType!)
			...
			super
				.init(
					...
					instantiationContext: "UpdateFulfillHTLC.swift::\(#function):\(#line)")
		...
		internal func clone() -> UpdateFulfillHTLC {
			...
			// return value (do some wrapping)
			let returnValue = UpdateFulfillHTLC(
				cType: nativeCallResult, instantiationContext: "UpdateFulfillHTLC.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKUpdateFulfillHTLC>) in
					UpdateFulfillHTLC_hash(oPointer)
				}
		...
		public class func read(ser: [UInt8]) -> Result_UpdateFulfillHTLCDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "UpdateFulfillHTLC.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing UpdateFulfillHTLC \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing UpdateFulfillHTLC \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
