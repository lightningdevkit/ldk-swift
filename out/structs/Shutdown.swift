	...
	public class Shutdown: NativeTypeWrapper {
		...
		public func setChannelId(val: [UInt8]) {
			...
			let valPrimitiveWrapper = ThirtyTwoBytes(
				value: val, instantiationContext: "Shutdown.swift::\(#function):\(#line)")
		...
		public init(channelIdArg: [UInt8], scriptpubkeyArg: [UInt8]) {
			...
			let channelIdArgPrimitiveWrapper = ThirtyTwoBytes(
				value: channelIdArg, instantiationContext: "Shutdown.swift::\(#function):\(#line)")

			let scriptpubkeyArgVector = Vec_u8Z(
				array: scriptpubkeyArg, instantiationContext: "Shutdown.swift::\(#function):\(#line)"
			)
			.dangle()
		...
		internal func dangleRecursively() -> Shutdown {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Shutdown \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
