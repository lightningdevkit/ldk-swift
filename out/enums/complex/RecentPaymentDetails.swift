			...
			extension Bindings {
				...
				RecentPaymentDetails_clone(origPointer)
					...
					public class func initWithAwaitingInvoice(paymentId: [UInt8]) -> RecentPaymentDetails {
						...
						let paymentIdPrimitiveWrapper = ThirtyTwoBytes(value: paymentId, instantiationContext: "RecentPaymentDetails.swift::\(#function):\(#line)")
					...
					public class func initWithPending(paymentId: [UInt8], paymentHash: [UInt8], totalMsat: UInt64) -> RecentPaymentDetails {
						// native call variable prep
						...
						// native method call
						let nativeCallResult = RecentPaymentDetails_pending(paymentIdPrimitiveWrapper.cType!, paymentHashPrimitiveWrapper.cType!, totalMsat)
					...
					public class func initWithFulfilled(paymentId: [UInt8], paymentHash: [UInt8]?) -> RecentPaymentDetails {
						...
						let paymentHashOption = Option_ThirtyTwoBytesZ(some: paymentHash, instantiationContext: "RecentPaymentDetails.swift::\(#function):\(#line)").danglingClone()
				

						// native method call
						let nativeCallResult = RecentPaymentDetails_fulfilled(paymentIdPrimitiveWrapper.cType!, paymentHashOption.cType!)
					...
					public class func initWithAbandoned(paymentId: [UInt8], paymentHash: [UInt8]) -> RecentPaymentDetails {
						...
						// native method call
						let nativeCallResult = RecentPaymentDetails_abandoned(paymentIdPrimitiveWrapper.cType!, paymentHashPrimitiveWrapper.cType!)
					...
					public func getValueAsAwaitingInvoice() -> AwaitingInvoice? {
						...
						return RecentPaymentDetails_LDKAwaitingInvoice_Body(cType: self.cType!.awaiting_invoice, instantiationContext: "RecentPaymentDetails.swift::\(#function):\(#line)", anchor: self)
					}
					...
					public class AwaitingInvoice: NativeTypeWrapper {
						...
						internal init(cType: LDKRecentPaymentDetails_LDKAwaitingInvoice_Body, instantiationContext: String, anchor: NativeTypeWrapper) {
							Self.instanceCounter += 1
							self.instanceNumber = Self.instanceCounter
							self.cType = cType
						...
						internal init(cType: LDKRecentPaymentDetails_LDKAwaitingInvoice_Body, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
							Self.instanceCounter += 1
							self.instanceNumber = Self.instanceCounter
							self.cType = cType
						...
						public func getPaymentId() -> [UInt8] {
							// return value (do some wrapping)
							let returnValue = ThirtyTwoBytes(cType: self.cType!.payment_id, instantiationContext: "RecentPaymentDetails.swift::\(#function):\(#line)", anchor: self).getValue()
					...
					public class Fulfilled: NativeTypeWrapper {
						...
						public func getPaymentHash() -> [UInt8]? {
							// return value (do some wrapping)
							let returnValue = Option_ThirtyTwoBytesZ(cType: self.cType!.payment_hash, instantiationContext: "RecentPaymentDetails.swift::\(#function):\(#line)", anchor: self).getValue()
