...
extension Bindings {
	...
	public class UpdateFee: NativeTypeWrapper {
		...
		internal init(cType: LDKUpdateFee, instantiationContext: String) {
		...
		internal init(cType: LDKUpdateFee, instantiationContext: String, anchor: NativeTypeWrapper) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
			self.initialCFreeability = self.cType!.is_owned
		...
		internal init(
			cType: LDKUpdateFee, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> UpdateFee {
			...
			let returnValue = UpdateFee(
				cType: nativeCallResult, instantiationContext: "UpdateFee.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKUpdateFee>) in
					UpdateFee_hash(oPointer)
				}
		...
		public class func read(ser: [UInt8]) -> Result_UpdateFeeDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "UpdateFee.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print("Freeing UpdateFee \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing UpdateFee \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
