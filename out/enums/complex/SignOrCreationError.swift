...
extension Bindings {
	...
	public class SignOrCreationError: NativeTypeWrapper {
		...
		internal init(
			cType: LDKSignOrCreationError, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> SignOrCreationError {
			...
			// return value (do some wrapping)
			let returnValue = SignOrCreationError(
				cType: nativeCallResult, instantiationContext: "SignOrCreationError.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> SignOrCreationError {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing SignOrCreationError \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing SignOrCreationError \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
