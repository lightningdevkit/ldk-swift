			...
			extension Bindings {
				...
				internal class Option_EventZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCOption_EventZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Option_EventZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Option_EventZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
