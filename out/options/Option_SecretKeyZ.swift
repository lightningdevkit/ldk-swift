			...
			extension Bindings {
				...
				internal class Option_SecretKeyZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCOption_SecretKeyZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(some: [UInt8]?, instantiationContext: String) {
						...
						if let some = some {
							...
							let somePrimitiveWrapper = SecretKey(value: some, instantiationContext: "Option_SecretKeyZ.swift::\(#function):\(#line)")
				...
				COption_SecretKeyZ_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_SecretKeyZ(cType: nativeCallResult, instantiationContext: "Option_SecretKeyZ.swift::\(#function):\(#line)")
					...
					public func getValue() -> [UInt8]? {
						...
						if self.cType!.tag == LDKCOption_SecretKeyZ_Some {
							return SecretKey(cType: self.cType!.some, instantiationContext: "Option_SecretKeyZ.swift::\(#function):\(#line)", anchor: self).dangle().getValue()
						}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Option_SecretKeyZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Option_SecretKeyZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
