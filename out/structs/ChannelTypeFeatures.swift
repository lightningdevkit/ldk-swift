...
extension Bindings {
	...
	public class ChannelTypeFeatures: NativeTypeWrapper {
		...
		internal init(
			cType: LDKChannelTypeFeatures, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> ChannelTypeFeatures {
			...
			let returnValue = ChannelTypeFeatures(
				cType: nativeCallResult, instantiationContext: "ChannelTypeFeatures.swift::\(#function):\(#line)")
		...
		public class func read(ser: [UInt8]) -> Result_ChannelTypeFeaturesDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "ChannelTypeFeatures.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_ChannelTypeFeaturesDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "ChannelTypeFeatures.swift::\(#function):\(#line)")
		...
		public func setStaticRemoteKeyOptional() {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisArgPointer: UnsafeMutablePointer<LDKChannelTypeFeatures>) in
		...
		internal func dangleRecursively() -> ChannelTypeFeatures {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing ChannelTypeFeatures \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing ChannelTypeFeatures \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
