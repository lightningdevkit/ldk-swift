			...
			extension Bindings {
				...
				Packet_get_public_key(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = PublicKey(cType: nativeCallResult, instantiationContext: "Packet.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
				...
				Packet_get_hop_data(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "Packet.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
				...
				Packet_get_hmac(thisPtrPointer)
					...
					public func setHmac(val: [UInt8]) {
						...
						let valPrimitiveWrapper = ThirtyTwoBytes(value: val, instantiationContext: "Packet.swift::\(#function):\(#line)")
				...
				Packet_set_hmac(thisPtrPointer, valPrimitiveWrapper.cType!)
					...
					public init(versionArg: UInt8, publicKeyArg: [UInt8], hopDataArg: [UInt8], hmacArg: [UInt8]) {
						...
						let publicKeyArgPrimitiveWrapper = PublicKey(value: publicKeyArg, instantiationContext: "Packet.swift::\(#function):\(#line)")
				
						let hopDataArgVector = Vec_u8Z(array: hopDataArg, instantiationContext: "Packet.swift::\(#function):\(#line)").dangle()
				
						let hmacArgPrimitiveWrapper = ThirtyTwoBytes(value: hmacArg, instantiationContext: "Packet.swift::\(#function):\(#line)")
				

						// native method call
						let nativeCallResult = Packet_new(versionArg, publicKeyArgPrimitiveWrapper.cType!, hopDataArgVector.cType!, hmacArgPrimitiveWrapper.cType!)
				...
				Packet_write(objPointer)
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing Packet \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
