...
extension Bindings {
	...
	public class WarningMessage: NativeTypeWrapper {
		...
		internal init(
			cType: LDKWarningMessage, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init(channelIdArg: [UInt8], dataArg: String) {
			...
			super
				.init(
					conflictAvoidingVariableName: 0, instantiationContext: "WarningMessage.swift::\(#function):\(#line)"
				)
		...
		internal func clone() -> WarningMessage {
			...
			// return value (do some wrapping)
			let returnValue = WarningMessage(
				cType: nativeCallResult, instantiationContext: "WarningMessage.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKWarningMessage>) in
					WarningMessage_hash(oPointer)
				}
		...
		public class func read(ser: [UInt8]) -> Result_WarningMessageDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "WarningMessage.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing WarningMessage \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing WarningMessage \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
