...
extension Bindings {
	...
	public class Result_InFlightHtlcsDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_InFlightHtlcsDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: InFlightHtlcs) -> Result_InFlightHtlcsDecodeErrorZ {
			...
			let returnValue = Result_InFlightHtlcsDecodeErrorZ(
				cType: nativeCallResult,
				instantiationContext: "Result_InFlightHtlcsDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_InFlightHtlcsDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_InFlightHtlcsDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_InFlightHtlcsDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_InFlightHtlcsDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_InFlightHtlcsDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_InFlightHtlcsDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
