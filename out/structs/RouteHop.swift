	...
	public class RouteHop: NativeTypeWrapper {
		...
		public func getPubkey() -> [UInt8] {
			...
			let returnValue = PublicKey(
				cType: nativeCallResult, instantiationContext: "RouteHop.swift::\(#function):\(#line)", anchor: self
			)
		...
		public func setPubkey(val: [UInt8]) {
			...
			let valPrimitiveWrapper = PublicKey(
				value: val, instantiationContext: "RouteHop.swift::\(#function):\(#line)")
		...
		public init(
			...
			let pubkeyArgPrimitiveWrapper = PublicKey(
				value: pubkeyArg, instantiationContext: "RouteHop.swift::\(#function):\(#line)")
		...
		internal func clone() -> RouteHop {
			...
			// return value (do some wrapping)
			let returnValue = RouteHop(
				cType: nativeCallResult, instantiationContext: "RouteHop.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> RouteHop {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing RouteHop \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
