			...
			extension Bindings {
				...
				internal class Option_MonitorEventZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCOption_MonitorEventZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Option_MonitorEventZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Option_MonitorEventZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
