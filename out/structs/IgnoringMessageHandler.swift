			...
			extension Bindings {
				...
				public class IgnoringMessageHandler: NativeTypeWrapper {
					...
					internal init(cType: LDKIgnoringMessageHandler, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				IgnoringMessageHandler_as_OffersMessageHandler(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = NativelyImplementedOffersMessageHandler(cType: nativeCallResult, instantiationContext: "IgnoringMessageHandler.swift::\(#function):\(#line)", anchor: self)
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing IgnoringMessageHandler \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing IgnoringMessageHandler \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
