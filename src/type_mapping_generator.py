...
class TypeMappingGenerator:
    ...
    def __init__(self, consts, opaque_structs, clone_fns, unitary_enums, trait_structs, complex_enums, result_types, tuple_types, var_is_arr_regex, var_ty_regex):
        ...
        self.var_is_arr_regex = var_is_arr_regex
        self.var_ty_regex = var_ty_regex

    def java_c_types(self, fn_arg, ret_arr_len):
        java_c_type_arguments = {
            "tuple_types": self.tuple_types,
            "var_is_arr_regex": self.var_is_arr_regex,
            "java_c_types_none_allowed": False,
            "var_ty_regex": self.var_ty_regex,
            "unitary_enums": self.unitary_enums,
            "language_constants": self.consts
        }
        return java_type_mapper.java_c_types(fn_arg, ret_arr_len, **java_c_type_arguments)
                    ...
                    if not holds_ref:
                        ...
                        if ty_info.requires_clone == True: # Set in object array mapping
                            if (ty_info.rust_obj.replace("LDK", "") + "_clone") in self.clone_fns:
                                ...
                                ret_conv = (ret_conv[0] + "*" + ty_info.var_name + "_copy = ", "; // Warning: We likely need to clone here, but no clone is available for " + ty_info.rust_obj + "\n")
                        else:
