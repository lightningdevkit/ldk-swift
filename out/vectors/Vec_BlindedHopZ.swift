			...
			extension Bindings {
				...
				internal class Vec_BlindedHopZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_BlindedHopZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
...
BlindedHop(cType: currentCType, instantiationContext: "Vec_BlindedHopZ.swift::\(#function):\(#line)", anchor: self).dangle()
						}
						return swiftArray
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Vec_BlindedHopZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Vec_BlindedHopZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
