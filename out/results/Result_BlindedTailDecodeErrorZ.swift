			...
			extension Bindings {
				...
				public class Result_BlindedTailDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_BlindedTailDecodeErrorZ) {
					...
					internal init(cType: LDKCResult_BlindedTailDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_BlindedTailDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> BlindedTail? {
						if self.cType?.result_ok == true {
							return BlindedTail(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							Bindings.print("Freeing Result_BlindedTailDecodeErrorZ \(self.instanceNumber).")
							
							self.free()
						} else {
							Bindings.print("Not freeing Result_BlindedTailDecodeErrorZ \(self.instanceNumber) due to dangle.")
						}
					}
