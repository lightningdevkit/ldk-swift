			...
			extension Bindings {
				...
				public class BestBlock: NativeTypeWrapper {
					...
					internal init(cType: LDKBestBlock, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				BestBlock_height(thisArgPointer)
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing BestBlock \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
