...
extension Bindings {
	...
	public class Description: NativeTypeWrapper {
		...
		internal init(
			cType: LDKDescription, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func new(description: String) -> Result_DescriptionCreationErrorZ {
			...
			let descriptionPrimitiveWrapper = Str(
				value: description, instantiationContext: "Description.swift::\(#function):\(#line)"
			)
			.dangle()
		...
		internal func dangleRecursively() -> Description {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Description \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
