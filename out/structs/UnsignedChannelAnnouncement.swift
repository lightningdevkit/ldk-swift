...
extension Bindings {
	...
	public class UnsignedChannelAnnouncement: NativeTypeWrapper {
		...
		internal init(
			cType: LDKUnsignedChannelAnnouncement, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func setChainHash(val: [UInt8]) {
			...
			let valPrimitiveWrapper = ThirtyTwoBytes(
				value: val, instantiationContext: "UnsignedChannelAnnouncement.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKUnsignedChannelAnnouncement>) in
					UnsignedChannelAnnouncement_hash(oPointer)
				}
		...
		public class func read(ser: [UInt8]) -> Result_UnsignedChannelAnnouncementDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "UnsignedChannelAnnouncement.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing UnsignedChannelAnnouncement \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing UnsignedChannelAnnouncement \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
