			...
			extension Bindings {
				...
				// Type: LDKRouteParameters
					...
					if pointerValue == 0 {
						...
						// return value (do some wrapping)
						let returnValue = RouteParameters(cType: nativeCallResult, instantiationContext: "Route.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				Route_write(objPointer)
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing Route \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
