...
extension Bindings {
	...
	public class CommitmentSigned: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCommitmentSigned, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init(channelIdArg: [UInt8], signatureArg: [UInt8], htlcSignaturesArg: [[UInt8]]) {
			...
			super
				.init(
					...
					instantiationContext: "CommitmentSigned.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKCommitmentSigned>) in
					CommitmentSigned_hash(oPointer)
				}
		...
		public class func read(ser: [UInt8]) -> Result_CommitmentSignedDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "CommitmentSigned.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing CommitmentSigned \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing CommitmentSigned \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
