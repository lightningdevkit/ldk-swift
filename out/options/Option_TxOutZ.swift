			...
			extension Bindings {
				...
				internal class Option_TxOutZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCOption_TxOutZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				COption_TxOutZ_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_TxOutZ(cType: nativeCallResult, instantiationContext: "Option_TxOutZ.swift::\(#function):\(#line)")
					...
					public func getValue() -> TxOut? {
						...
						if self.cType!.tag == LDKCOption_TxOutZ_Some {
							return TxOut(cType: self.cType!.some, instantiationContext: "Option_TxOutZ.swift::\(#function):\(#line)", anchor: self).dangle()
						}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Option_TxOutZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Option_TxOutZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
