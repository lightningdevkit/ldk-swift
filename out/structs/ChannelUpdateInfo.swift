...
extension Bindings {
	...
	public class ChannelUpdateInfo: NativeTypeWrapper {
		...
		internal init(
			cType: LDKChannelUpdateInfo, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func setLastUpdate(val: UInt32) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKChannelUpdateInfo>) in
		...
		public func getFees() -> RoutingFees {
			...
			let returnValue = RoutingFees(
				cType: nativeCallResult, instantiationContext: "ChannelUpdateInfo.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle(false)
		...
		public init(
			...
			// native method call
			let nativeCallResult = ChannelUpdateInfo_new(
				lastUpdateArg, enabledArg, cltvExpiryDeltaArg, htlcMinimumMsatArg, htlcMaximumMsatArg,
				feesArg.dynamicallyDangledClone().cType!, lastUpdateMessageArg.dynamicallyDangledClone().cType!)
			...
			super
				.init(
					...
					instantiationContext: "ChannelUpdateInfo.swift::\(#function):\(#line)")
		...
		public class func read(ser: [UInt8]) -> Result_ChannelUpdateInfoDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "ChannelUpdateInfo.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> ChannelUpdateInfo {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing ChannelUpdateInfo \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing ChannelUpdateInfo \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
