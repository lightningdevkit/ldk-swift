			...
			extension Bindings {
				...
				internal class Tuple_usizeTransactionZ: NativeTypeWrapper {
					...
					internal init(cType: LDKC2Tuple_usizeTransactionZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				C2Tuple_usizeTransactionZ_clone(origPointer)
					...
					public init(a: UInt, b: [UInt8], instantiationContext: String) {
						...
						let bPrimitiveWrapper = Transaction(value: b, instantiationContext: "Tuple_usizeTransactionZ.swift::\(#function):\(#line)").dangle()
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Tuple_usizeTransactionZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Tuple_usizeTransactionZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
