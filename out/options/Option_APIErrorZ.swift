			...
			extension Bindings {
				...
				internal class Option_APIErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCOption_APIErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				COption_APIErrorZ_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_APIErrorZ(cType: nativeCallResult, instantiationContext: "Option_APIErrorZ.swift::\(#function):\(#line)")
					...
					public func getValue() -> APIError? {
						...
						if self.cType!.tag == LDKCOption_APIErrorZ_Some {
							return APIError(cType: self.cType!.some, instantiationContext: "Option_APIErrorZ.swift::\(#function):\(#line)", anchor: self).dangle()
						}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Option_APIErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Option_APIErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
