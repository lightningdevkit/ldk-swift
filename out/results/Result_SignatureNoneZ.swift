...
extension Bindings {
	...
	public class Result_SignatureNoneZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_SignatureNoneZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: [UInt8]) -> Result_SignatureNoneZ {
			...
			let oPrimitiveWrapper = Signature(
				value: o, instantiationContext: "Result_SignatureNoneZ.swift::\(#function):\(#line)")
			...
			let returnValue = Result_SignatureNoneZ(
				cType: nativeCallResult, instantiationContext: "Result_SignatureNoneZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> [UInt8]? {
			if self.cType?.result_ok == true {
				return Signature(
					...
					instantiationContext: "Result_SignatureNoneZ.swift::\(#function):\(#line)", anchor: self
				)
				.getValue()
			}
			return nil
		...
		internal func dangleRecursively() -> Result_SignatureNoneZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_SignatureNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Result_SignatureNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
