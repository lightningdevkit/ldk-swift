			...
			extension Bindings {
				...
				public class Sleeper: NativeTypeWrapper {
					...
					public init(futures: [Future]) {
						...
						let futuresVector = Vec_FutureZ(array: futures, instantiationContext: "Sleeper.swift::\(#function):\(#line)").dangle()
				...
				Sleeper_wait_timeout(thisArgPointer, maxWait)
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing Sleeper \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
