	...
	open class Logger: NativeTraitWrapper {
		...
		public init() {
			...
			func freeLambda(this_arg: UnsafeMutableRawPointer?) {
				let instance: Logger = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "Logger::freeLambda")
		...
		open func log(record: Record) {
			...
			Bindings.print(
				"Error: Logger::log MUST be overridden! Offending class: \(String(describing: self)). Aborting.",
				severity: .ERROR)
			abort()
		}
		...
		internal func dangleRecursively() -> Logger {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Logger \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
	}
