...
extension Bindings {
	...
	internal class Option_CVec_u8ZZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCOption_CVec_u8ZZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(some: [UInt8]?, instantiationContext: String) {
			...
			if let some = some {
				...
				let someVector = Vec_u8Z(
					array: some, instantiationContext: "Option_CVec_u8ZZ.swift::\(#function):\(#line)"
				)
				.dangle()
		...
		internal func clone() -> Option_CVec_u8ZZ {
			...
			let returnValue = Option_CVec_u8ZZ(
				cType: nativeCallResult, instantiationContext: "Option_CVec_u8ZZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> [UInt8]? {
			...
			if self.cType!.tag == LDKCOption_CVec_u8ZZ_Some {
				return Vec_u8Z(
					cType: self.cType!.some, instantiationContext: "Option_CVec_u8ZZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle().getValue()
			}
		...
		internal func dangleRecursively() -> Option_CVec_u8ZZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Option_CVec_u8ZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Option_CVec_u8ZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
