...
extension Bindings {
	...
	internal class Vec_U5Z: NativeTypeWrapper {
		...
		internal init(cType: LDKCVec_U5Z, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false)
		{
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(array: [UInt8], instantiationContext: String) {
			...
			let rustArray = array.map { (currentValueDepth1: UInt8) -> LDKU5 in
				...
				let currentValueDepth1PrimitiveWrapper = U5(
					value: currentValueDepth1, instantiationContext: "Vec_U5Z.swift::\(#function):\(#line)")
		...
		public func getValue() -> [UInt8] {
			...
			let swiftArray = array.map { (currentCType: LDKU5) -> UInt8 in
				U5(cType: currentCType, instantiationContext: "Vec_U5Z.swift::\(#function):\(#line)").dangle()
					.getValue()
			}
			return swiftArray
		}
		...
		internal func dangleRecursively() -> Vec_U5Z {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Vec_U5Z \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
