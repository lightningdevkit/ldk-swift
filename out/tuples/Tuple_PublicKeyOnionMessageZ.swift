			...
			extension Bindings {
				...
				internal class Tuple_PublicKeyOnionMessageZ: NativeTypeWrapper {
					...
					internal init(cType: LDKC2Tuple_PublicKeyOnionMessageZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				C2Tuple_PublicKeyOnionMessageZ_clone(origPointer)
					...
					public init(a: [UInt8], b: OnionMessage, instantiationContext: String) {
						...
						let aPrimitiveWrapper = PublicKey(value: a, instantiationContext: "Tuple_PublicKeyOnionMessageZ.swift::\(#function):\(#line)")
				

						// native method call
						let nativeCallResult = C2Tuple_PublicKeyOnionMessageZ_new(aPrimitiveWrapper.cType!, b.dynamicallyDangledClone().cType!)
				...
				super.init(conflictAvoidingVariableName: 0, instantiationContext: instantiationContext)
					...
					public func getA() -> [UInt8] {
						// return value (do some wrapping)
						let returnValue = PublicKey(cType: self.cType!.a, instantiationContext: "Tuple_PublicKeyOnionMessageZ.swift::\(#function):\(#line)", anchor: self).dangle().getValue()
					...
					public func getB() -> OnionMessage {
						// return value (do some wrapping)
						let returnValue = OnionMessage(cType: self.cType!.b, instantiationContext: "Tuple_PublicKeyOnionMessageZ.swift::\(#function):\(#line)", anchor: self).dangle()
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Tuple_PublicKeyOnionMessageZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Tuple_PublicKeyOnionMessageZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
