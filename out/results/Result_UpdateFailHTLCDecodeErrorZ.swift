...
extension Bindings {
	...
	public class Result_UpdateFailHTLCDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_UpdateFailHTLCDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: UpdateFailHTLC) -> Result_UpdateFailHTLCDecodeErrorZ {
			...
			let returnValue = Result_UpdateFailHTLCDecodeErrorZ(
				...
				instantiationContext: "Result_UpdateFailHTLCDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_UpdateFailHTLCDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_UpdateFailHTLCDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_UpdateFailHTLCDecodeErrorZ.swift::\(#function):\(#line)", anchor: self
				)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_UpdateFailHTLCDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_UpdateFailHTLCDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_UpdateFailHTLCDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
