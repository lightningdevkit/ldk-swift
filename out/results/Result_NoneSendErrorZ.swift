...
extension Bindings {
	...
	public class Result_NoneSendErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_NoneSendErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk() -> Result_NoneSendErrorZ {
			...
			let returnValue = Result_NoneSendErrorZ(
				cType: nativeCallResult, instantiationContext: "Result_NoneSendErrorZ.swift::\(#function):\(#line)")
		...
		public func getError() -> SendError? {
			if self.cType?.result_ok == false {
				return SendError(
					...
					instantiationContext: "Result_NoneSendErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_NoneSendErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_NoneSendErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Result_NoneSendErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
