			...
			extension Bindings {
				...
				Path_get_hops(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_RouteHopZ(cType: nativeCallResult, instantiationContext: "Path.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public func setHops(val: [RouteHop]) {
						...
						let valVector = Vec_RouteHopZ(array: val, instantiationContext: "Path.swift::\(#function):\(#line)").dangle()
				...
				// Type: LDKBlindedTail
					...
					if pointerValue == 0 {
						...
						// return value (do some wrapping)
						let returnValue = BlindedTail(cType: nativeCallResult, instantiationContext: "Path.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				Path_set_blinded_tail(thisPtrPointer, val.dynamicallyDangledClone().cType!)
					...
					public init(hopsArg: [RouteHop], blindedTailArg: BlindedTail) {
						...
						let hopsArgVector = Vec_RouteHopZ(array: hopsArg, instantiationContext: "Path.swift::\(#function):\(#line)").dangle()
				...
				Path_final_cltv_expiry_delta(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_u32Z(cType: nativeCallResult, instantiationContext: "Path.swift::\(#function):\(#line)", anchor: self).getValue()
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing Path \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
