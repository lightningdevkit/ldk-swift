			...
			extension Bindings {
				...
				public class TxIn: NativeTypeWrapper {
					...
					public init(witness: [UInt8], scriptSig: [UInt8], sequence: UInt32, previousTxid: [UInt8], previousVout: UInt32) {
						...
						let witnessPrimitiveWrapper = Witness(value: witness, instantiationContext: "TxIn.swift::\(#function):\(#line)").dynamicallyDangledClone()
				
						let scriptSigVector = Vec_u8Z(array: scriptSig, instantiationContext: "TxIn.swift::\(#function):\(#line)").dangle()
				
						let previousTxidPrimitiveWrapper = ThirtyTwoBytes(value: previousTxid, instantiationContext: "TxIn.swift::\(#function):\(#line)")
				

						// native method call
						let nativeCallResult = TxIn_new(witnessPrimitiveWrapper.cType!, scriptSigVector.cType!, sequence, previousTxidPrimitiveWrapper.cType!, previousVout)
				...
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "TxIn.swift::\(#function):\(#line)")
					...
					public func getWitness() -> [UInt8] {
						// return value (do some wrapping)
						let returnValue = Witness(cType: self.cType!.witness, instantiationContext: "TxIn.swift::\(#function):\(#line)", anchor: self).getValue()
					...
					public func getScriptSig() -> [UInt8] {
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: self.cType!.script_sig, instantiationContext: "TxIn.swift::\(#function):\(#line)", anchor: self).getValue()
					...
					public func getPreviousTxid() -> [UInt8] {
						// return value (do some wrapping)
						let returnValue = ThirtyTwoBytes(cType: self.cType!.previous_txid, instantiationContext: "TxIn.swift::\(#function):\(#line)", anchor: self).getValue()
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing TxIn \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
