			...
			extension Bindings {
				...
				open class UtxoLookup: NativeTraitWrapper {
					...
					internal init(cType: LDKUtxoLookup, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				abort()
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing UtxoLookup \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
				}
