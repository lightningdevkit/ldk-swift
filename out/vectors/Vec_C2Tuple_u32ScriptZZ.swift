			...
			extension Bindings {
				...
				internal class Vec_C2Tuple_u32ScriptZZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_C2Tuple_u32ScriptZZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Vec_C2Tuple_u32ScriptZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Vec_C2Tuple_u32ScriptZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
