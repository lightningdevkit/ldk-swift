...
extension Bindings {
	...
	public class CounterpartyCommitmentSecrets: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCounterpartyCommitmentSecrets, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> CounterpartyCommitmentSecrets {
			...
			// return value (do some wrapping)
			let returnValue = CounterpartyCommitmentSecrets(
				cType: nativeCallResult,
				instantiationContext: "CounterpartyCommitmentSecrets.swift::\(#function):\(#line)")
		...
		public func getMinSeenSecret() -> UInt64 {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(thisArgPointer: UnsafePointer<LDKCounterpartyCommitmentSecrets>) in
		...
		public func provideSecret(idx: UInt64, secret: [UInt8]) -> Result_NoneNoneZ {
			...
			let secretPrimitiveWrapper = ThirtyTwoBytes(
				value: secret, instantiationContext: "CounterpartyCommitmentSecrets.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisArgPointer: UnsafeMutablePointer<LDKCounterpartyCommitmentSecrets>) in
			...
			let returnValue = Result_NoneNoneZ(
				cType: nativeCallResult,
				instantiationContext: "CounterpartyCommitmentSecrets.swift::\(#function):\(#line)", anchor: self
			)
			.dangle(false)
		...
		public class func read(ser: [UInt8]) -> Result_CounterpartyCommitmentSecretsDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "CounterpartyCommitmentSecrets.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing CounterpartyCommitmentSecrets \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing CounterpartyCommitmentSecrets \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
