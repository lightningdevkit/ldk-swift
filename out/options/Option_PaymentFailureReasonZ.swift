			...
			extension Bindings {
				...
				internal class Option_PaymentFailureReasonZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCOption_PaymentFailureReasonZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				COption_PaymentFailureReasonZ_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_PaymentFailureReasonZ(cType: nativeCallResult, instantiationContext: "Option_PaymentFailureReasonZ.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Option_PaymentFailureReasonZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Option_PaymentFailureReasonZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
