...
extension Bindings {
	...
	internal class Vec_PrivateRouteZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_PrivateRouteZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(array: [PrivateRoute], instantiationContext: String) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			super.init(conflictAvoidingVariableName: 0, instantiationContext: instantiationContext)
		...
		public func getValue() -> [PrivateRoute] {
			...
			let swiftArray = array.map { (currentCType: LDKPrivateRoute) -> PrivateRoute in
				PrivateRoute(
					cType: currentCType, instantiationContext: "Vec_PrivateRouteZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		internal func dangleRecursively() -> Vec_PrivateRouteZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_PrivateRouteZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Vec_PrivateRouteZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
