...
extension Bindings {
	...
	public class PayeePubKey: NativeTypeWrapper {
		...
		internal init(
			cType: LDKPayeePubKey, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getA() -> [UInt8] {
			...
			let returnValue = PublicKey(
				cType: nativeCallResult, instantiationContext: "PayeePubKey.swift::\(#function):\(#line)", anchor: self
			)
		...
		public func setA(val: [UInt8]) {
			...
			let valPrimitiveWrapper = PublicKey(
				value: val, instantiationContext: "PayeePubKey.swift::\(#function):\(#line)")
		...
		public init(aArg: [UInt8]) {
			...
			let aArgPrimitiveWrapper = PublicKey(
				value: aArg, instantiationContext: "PayeePubKey.swift::\(#function):\(#line)")
			...
			self.instanceNumber = Self.instanceCounter
			super
				.init(conflictAvoidingVariableName: 0, instantiationContext: "PayeePubKey.swift::\(#function):\(#line)")
		...
		internal func clone() -> PayeePubKey {
			...
			let returnValue = PayeePubKey(
				cType: nativeCallResult, instantiationContext: "PayeePubKey.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> PayeePubKey {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing PayeePubKey \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
