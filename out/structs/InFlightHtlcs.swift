...
extension Bindings {
	...
	public class InFlightHtlcs: NativeTypeWrapper {
		...
		internal init(
			cType: LDKInFlightHtlcs, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> InFlightHtlcs {
			...
			let returnValue = InFlightHtlcs(
				cType: nativeCallResult, instantiationContext: "InFlightHtlcs.swift::\(#function):\(#line)")
		...
		public init() {
			...
			super
				.init(
					conflictAvoidingVariableName: 0, instantiationContext: "InFlightHtlcs.swift::\(#function):\(#line)")
		...
		public func processPath(path: Path, payerNodeId: [UInt8]) {
			...
			let payerNodeIdPrimitiveWrapper = PublicKey(
				value: payerNodeId, instantiationContext: "InFlightHtlcs.swift::\(#function):\(#line)")
		...
		public class func read(ser: [UInt8]) -> Result_InFlightHtlcsDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "InFlightHtlcs.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> InFlightHtlcs {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing InFlightHtlcs \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing InFlightHtlcs \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
