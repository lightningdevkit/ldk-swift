...
public class Result_ClosingSignedFeeRangeDecodeErrorZ {
    ...
    /* RESULT_METHODS_START */
			...
			public func getValue() -> ClosingSignedFeeRange? {
				if self.cOpaqueStruct?.result_ok == true {
					return ClosingSignedFeeRange(pointer: self.cOpaqueStruct!.contents.result.pointee)
				}
				return nil
			}
    ...
    internal func free() -> Void {
        ...
        return CResult_ClosingSignedFeeRangeDecodeErrorZ_free(self.cOpaqueStruct!);
					...
					deinit {
						if !self.dangling {
							print("Freeing Result_ClosingSignedFeeRangeDecodeErrorZ \(self.instanceNumber).")
							self.free()
						} else {
							print("Not freeing Result_ClosingSignedFeeRangeDecodeErrorZ \(self.instanceNumber) due to dangle.")
						}
					}
