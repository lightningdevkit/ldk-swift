...
extension Bindings {
	...
	public class Result_RouteHintHopDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_RouteHintHopDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: RouteHintHop) -> Result_RouteHintHopDecodeErrorZ {
			...
			let returnValue = Result_RouteHintHopDecodeErrorZ(
				...
				instantiationContext: "Result_RouteHintHopDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_RouteHintHopDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_RouteHintHopDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_RouteHintHopDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_RouteHintHopDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_RouteHintHopDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_RouteHintHopDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
