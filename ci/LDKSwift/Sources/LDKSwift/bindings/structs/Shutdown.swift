...
Shutdown_clone(origPointer)
		...
		public class func eq(a: Shutdown, b: Shutdown) -> Bool {
			...
			return withUnsafePointer(to: a.cOpaqueStruct!) { (aPointer: UnsafePointer<LDKShutdown>) in
withUnsafePointer(to: b.cOpaqueStruct!) { (bPointer: UnsafePointer<LDKShutdown>) in
