			...
			extension Bindings {
				...
				public class Result_DelayedPaymentOutputDescriptorDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_DelayedPaymentOutputDescriptorDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_DelayedPaymentOutputDescriptorDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> DelayedPaymentOutputDescriptor? {
						if self.cType?.result_ok == true {
							return DelayedPaymentOutputDescriptor(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
