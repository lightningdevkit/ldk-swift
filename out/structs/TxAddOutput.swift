			...
			extension Bindings {
				...
				public class TxAddOutput: NativeTypeWrapper {
					...
					internal init(cType: LDKTxAddOutput, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				TxAddOutput_get_channel_id(thisPtrPointer)
					...
					public func setChannelId(val: [UInt8]) {
						...
						let valPrimitiveWrapper = ThirtyTwoBytes(value: val, instantiationContext: "TxAddOutput.swift::\(#function):\(#line)")
				...
				TxAddOutput_get_script(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = u8slice(cType: nativeCallResult, instantiationContext: "TxAddOutput.swift::\(#function):\(#line)", anchor: self).dangle().getValue()
					...
					public func setScript(val: [UInt8]) {
						...
						let valVector = Vec_u8Z(array: val, instantiationContext: "TxAddOutput.swift::\(#function):\(#line)").dangle()
				...
				TxAddOutput_set_script(thisPtrPointer, valVector.cType!)
					...
					public init(channelIdArg: [UInt8], serialIdArg: UInt64, satsArg: UInt64, scriptArg: [UInt8]) {
						...
						let channelIdArgPrimitiveWrapper = ThirtyTwoBytes(value: channelIdArg, instantiationContext: "TxAddOutput.swift::\(#function):\(#line)")
				
						let scriptArgVector = Vec_u8Z(array: scriptArg, instantiationContext: "TxAddOutput.swift::\(#function):\(#line)").dangle()
				

						// native method call
						let nativeCallResult = TxAddOutput_new(channelIdArgPrimitiveWrapper.cType!, serialIdArg, satsArg, scriptArgVector.cType!)
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "TxAddOutput.swift::\(#function):\(#line)")
				...
				TxAddOutput_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "TxAddOutput.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public class func read(ser: [UInt8]) -> Result_TxAddOutputDecodeErrorZ {
						...
						let serPrimitiveWrapper = u8slice(value: ser, instantiationContext: "TxAddOutput.swift::\(#function):\(#line)")
						...
						// return value (do some wrapping)
						let returnValue = Result_TxAddOutputDecodeErrorZ(cType: nativeCallResult, instantiationContext: "TxAddOutput.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing TxAddOutput \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
