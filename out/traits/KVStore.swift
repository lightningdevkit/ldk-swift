...
extension Bindings {
	...
	open class KVStore: NativeTraitWrapper {
		...
		public init() {
			...
			func readLambda(
				this_arg: UnsafeRawPointer?, primary_namespace: LDKStr, secondary_namespace: LDKStr, key: LDKStr
				...
				let swiftCallbackResult = instance.read(
					...
					.getValue(),
					secondaryNamespace: Str(
						cType: secondary_namespace, instantiationContext: "KVStore.swift::init()::\(#function):\(#line)"
					)
					.getValue(),
			...
			func listLambda(this_arg: UnsafeRawPointer?, primary_namespace: LDKStr, secondary_namespace: LDKStr)
		...
		/// in the store.
		open func write(primaryNamespace: String, secondaryNamespace: String, key: String, buf: [UInt8])
			-> Result_NoneIOErrorZ
		{
		...
		/// invokation or not.
		open func remove(primaryNamespace: String, secondaryNamespace: String, key: String, lazy: Bool)
			-> Result_NoneIOErrorZ
		{
	...
	internal class NativelyImplementedKVStore: KVStore {
		...
		///
		/// [`ErrorKind::NotFound`]: io::ErrorKind::NotFound
		public override func read(primaryNamespace: String, secondaryNamespace: String, key: String)
			-> Result_CVec_u8ZIOErrorZ
		{
			...
			let secondaryNamespacePrimitiveWrapper = Str(
				value: secondaryNamespace, instantiationContext: "KVStore.swift::\(#function):\(#line)"
			)
			.dangle()
			...
			let nativeCallResult = self.cType!
				.read(
					self.cType!.this_arg, primaryNamespacePrimitiveWrapper.cType!,
					secondaryNamespacePrimitiveWrapper.cType!, keyPrimitiveWrapper.cType!)
		...
		/// Will create the given `primary_namespace` and `secondary_namespace` if not already present
		/// in the store.
		public override func write(primaryNamespace: String, secondaryNamespace: String, key: String, buf: [UInt8])
			-> Result_NoneIOErrorZ
		{
			// native call variable prep
			...
			let nativeCallResult = self.cType!
				.write(
					self.cType!.this_arg, primaryNamespacePrimitiveWrapper.cType!,
					secondaryNamespacePrimitiveWrapper.cType!, keyPrimitiveWrapper.cType!, bufPrimitiveWrapper.cType!)
		...
		/// `secondary_namespace`, and `key`, independently of whether it was present before its
		/// invokation or not.
		public override func remove(primaryNamespace: String, secondaryNamespace: String, key: String, lazy: Bool)
			-> Result_NoneIOErrorZ
		{
			// native call variable prep
			...
			let nativeCallResult = self.cType!
				.remove(
					self.cType!.this_arg, primaryNamespacePrimitiveWrapper.cType!,
					secondaryNamespacePrimitiveWrapper.cType!, keyPrimitiveWrapper.cType!, lazy)
		...
		public override func list(primaryNamespace: String, secondaryNamespace: String) -> Result_CVec_StrZIOErrorZ {
			...
			let nativeCallResult = self.cType!
				.list(
					self.cType!.this_arg, primaryNamespacePrimitiveWrapper.cType!,
					secondaryNamespacePrimitiveWrapper.cType!)
