...
extension Bindings {
	...
	public class Result_ChannelReadyDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_ChannelReadyDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: ChannelReady) -> Result_ChannelReadyDecodeErrorZ {
			...
			let returnValue = Result_ChannelReadyDecodeErrorZ(
				...
				instantiationContext: "Result_ChannelReadyDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_ChannelReadyDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_ChannelReadyDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_ChannelReadyDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_ChannelReadyDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_ChannelReadyDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_ChannelReadyDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
