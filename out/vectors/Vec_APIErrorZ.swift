			...
			extension Bindings {
				...
				internal class Vec_APIErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_APIErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
...
APIError(cType: currentCType, instantiationContext: "Vec_APIErrorZ.swift::\(#function):\(#line)", anchor: self).dangle()
					...
					internal func dangle(_ shouldDangle: Bool = true) -> Vec_APIErrorZ {
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Vec_APIErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Vec_APIErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
