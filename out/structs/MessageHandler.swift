			...
			extension Bindings {
				...
				MessageHandler_get_custom_message_handler(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = NativelyImplementedCustomMessageHandler(cType: nativeCallResult.pointee, instantiationContext: "MessageHandler.swift::\(#function):\(#line)", anchor: self)
				...
				MessageHandler_set_custom_message_handler(thisPtrPointer, val.activate().cType!)
					...
					public init(chanHandlerArg: ChannelMessageHandler, routeHandlerArg: RoutingMessageHandler, onionMessageHandlerArg: OnionMessageHandler, customMessageHandlerArg: CustomMessageHandler) {
						// native call variable prep
						

						// native method call
						let nativeCallResult = MessageHandler_new(chanHandlerArg.activate().cType!, routeHandlerArg.activate().cType!, onionMessageHandlerArg.activate().cType!, customMessageHandlerArg.activate().cType!)
