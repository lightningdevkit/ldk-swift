			...
			extension Bindings {
				...
				public class Result_ECDSASignatureNoneZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_ECDSASignatureNoneZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: [UInt8]) -> Result_ECDSASignatureNoneZ {
						...
						let oPrimitiveWrapper = ECDSASignature(value: o, instantiationContext: "Result_ECDSASignatureNoneZ.swift::\(#function):\(#line)")
						...
						let returnValue = Result_ECDSASignatureNoneZ(cType: nativeCallResult, instantiationContext: "Result_ECDSASignatureNoneZ.swift::\(#function):\(#line)")
					...
					*/
					...
					public func getValue() -> [UInt8]? {
						if self.cType?.result_ok == true {
							return ECDSASignature(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_ECDSASignatureNoneZ.swift::\(#function):\(#line)", anchor: self).getValue()
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_ECDSASignatureNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_ECDSASignatureNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
