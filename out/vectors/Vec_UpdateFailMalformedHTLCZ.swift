...
extension Bindings {
	...
	internal class Vec_UpdateFailMalformedHTLCZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_UpdateFailMalformedHTLCZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func getValue() -> [UpdateFailMalformedHTLC] {
			...
			let swiftArray = array.map { (currentCType: LDKUpdateFailMalformedHTLC) -> UpdateFailMalformedHTLC in
				UpdateFailMalformedHTLC(
					cType: currentCType,
					instantiationContext: "Vec_UpdateFailMalformedHTLCZ.swift::\(#function):\(#line)", anchor: self
				)
				.dangle()
			}
			return swiftArray
		...
		internal func dangleRecursively() -> Vec_UpdateFailMalformedHTLCZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_UpdateFailMalformedHTLCZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Vec_UpdateFailMalformedHTLCZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
