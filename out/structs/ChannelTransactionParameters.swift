...
extension Bindings {
	...
	public class ChannelTransactionParameters: NativeTypeWrapper {
		...
		internal init(
			cType: LDKChannelTransactionParameters, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func asHolderBroadcastable() -> DirectedChannelTransactionParameters {
			...
			// return value (do some wrapping)
			let returnValue = DirectedChannelTransactionParameters(
				cType: nativeCallResult,
				instantiationContext: "ChannelTransactionParameters.swift::\(#function):\(#line)", anchor: self
			)
			.dangle(false)
		...
		public class func read(ser: [UInt8]) -> Result_ChannelTransactionParametersDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "ChannelTransactionParameters.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> ChannelTransactionParameters {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing ChannelTransactionParameters \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing ChannelTransactionParameters \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
