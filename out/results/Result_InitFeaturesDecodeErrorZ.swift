...
extension Bindings {
	...
	public class Result_InitFeaturesDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_InitFeaturesDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: InitFeatures) -> Result_InitFeaturesDecodeErrorZ {
			...
			let returnValue = Result_InitFeaturesDecodeErrorZ(
				...
				instantiationContext: "Result_InitFeaturesDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_InitFeaturesDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_InitFeaturesDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_InitFeaturesDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_InitFeaturesDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_InitFeaturesDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_InitFeaturesDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
