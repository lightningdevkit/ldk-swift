			...
			extension Bindings {
				...
				public class DefaultRouter: NativeTypeWrapper {
					...
					public init(networkGraph: NetworkGraph, logger: Logger, randomSeedBytes: [UInt8], scorer: LockableScore, scoreParams: ProbabilisticScoringFeeParameters) {
						// native call variable prep
				...
				DefaultRouter_new(networkGraphPointer, logger.activate().cType!, randomSeedBytesPrimitiveWrapper.cType!, scorer.activate().cType!, scoreParams.dynamicallyDangledClone().cType!)
						}
