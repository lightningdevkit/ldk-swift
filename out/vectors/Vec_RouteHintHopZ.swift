...
extension Bindings {
	...
	internal class Vec_RouteHintHopZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_RouteHintHopZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getValue() -> [RouteHintHop] {
			...
			let swiftArray = array.map { (currentCType: LDKRouteHintHop) -> RouteHintHop in
				RouteHintHop(
					cType: currentCType, instantiationContext: "Vec_RouteHintHopZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_RouteHintHopZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Vec_RouteHintHopZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
