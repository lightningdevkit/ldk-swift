...
extension Bindings {
	...
	public class Result_TransactionNoneZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_TransactionNoneZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: [UInt8]) -> Result_TransactionNoneZ {
			...
			let oPrimitiveWrapper = Transaction(
				value: o, instantiationContext: "Result_TransactionNoneZ.swift::\(#function):\(#line)"
			)
			.dangle()
			...
			let returnValue = Result_TransactionNoneZ(
				cType: nativeCallResult, instantiationContext: "Result_TransactionNoneZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> [UInt8]? {
			if self.cType?.result_ok == true {
				return Transaction(
					...
					instantiationContext: "Result_TransactionNoneZ.swift::\(#function):\(#line)", anchor: self
				)
				.getValue()
			}
			return nil
		...
		internal func dangleRecursively() -> Result_TransactionNoneZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_TransactionNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_TransactionNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
