			...
			extension Bindings {
				...
				public class Result_NoneNoneZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_NoneNoneZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk() -> Result_NoneNoneZ {
						...
						let returnValue = Result_NoneNoneZ(cType: nativeCallResult, instantiationContext: "Result_NoneNoneZ.swift::\(#function):\(#line)")
				...
				CResult_NoneNoneZ_clone(origPointer)
					...
					*/
					...
					internal func dangle(_ shouldDangle: Bool = true) -> Result_NoneNoneZ {
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_NoneNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_NoneNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
