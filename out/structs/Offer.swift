			...
			extension Bindings {
				...
				public class Offer: NativeTypeWrapper {
					...
					internal init(cType: LDKOffer, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				Offer_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Offer(cType: nativeCallResult)
				...
				Offer_chains(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_ChainHashZ(cType: nativeCallResult, anchor: self).dangle(false).getValue()
					...
					public func supportsChain(chain: [UInt8]) -> Bool {
						...
						let chainPrimitiveWrapper = ThirtyTwoBytes(value: chain)
				...
				// Type: LDKAmount
					...
					if pointerValue == 0 {
						...
						// return value (do some wrapping)
						let returnValue = Amount(cType: nativeCallResult, anchor: self).dangle(false)
				...
				Offer_description(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = PrintableString(cType: nativeCallResult, anchor: self).dangle(false)
				...
				Offer_features(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = OfferFeatures(cType: nativeCallResult, anchor: self).dangle(false)
				...
				Offer_absolute_expiry(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = Option_DurationZ(cType: nativeCallResult, anchor: self).getValue()
				...
				Offer_paths(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_BlindedPathZ(cType: nativeCallResult, anchor: self).dangle(false).getValue()
				...
				Offer_supported_quantity(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = Quantity(cType: nativeCallResult, anchor: self).dangle(false)
				...
				Offer_write(objPointer)
					...
					deinit {
						...
						if !self.dangling {
							Bindings.print("Freeing Offer \(self.instanceNumber).")
							
							self.free()
						} else {
							Bindings.print("Not freeing Offer \(self.instanceNumber) due to dangle.")
						}
					}
