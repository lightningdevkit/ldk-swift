...
extension Bindings {
	...
	public class Result_SpendableOutputDescriptorDecodeErrorZ: NativeTypeWrapper {
		...
		internal var cOpaqueStruct: LDKCResult_SpendableOutputDescriptorDecodeErrorZ?
		...
		public init(pointer: LDKCResult_SpendableOutputDescriptorDecodeErrorZ, anchor: NativeTypeWrapper){
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cOpaqueStruct = pointer
			super.init(conflictAvoidingVariableName: 0)
		...
		/* RESULT_METHODS_START */
			...
			public func getValue() -> SpendableOutputDescriptor? {
				if self.cOpaqueStruct?.result_ok == true {
					return SpendableOutputDescriptor(pointer: self.cOpaqueStruct!.contents.result.pointee, anchor: self).dangle()
				}
				return nil
			}
