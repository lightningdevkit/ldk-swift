			...
			extension Bindings {
				...
				public class QueryShortChannelIds: NativeTypeWrapper {
					...
					internal init(cType: LDKQueryShortChannelIds, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				QueryShortChannelIds_get_short_channel_ids(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u64Z(cType: nativeCallResult, instantiationContext: "QueryShortChannelIds.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "QueryShortChannelIds.swift::\(#function):\(#line)")
				...
				QueryShortChannelIds_eq(aPointer, bPointer)
					...
					public class func read(ser: [UInt8]) -> Result_QueryShortChannelIdsDecodeErrorZ {
						...
						// return value (do some wrapping)
						let returnValue = Result_QueryShortChannelIdsDecodeErrorZ(cType: nativeCallResult, instantiationContext: "QueryShortChannelIds.swift::\(#function):\(#line)")
				...
				QueryShortChannelIds_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "QueryShortChannelIds.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing QueryShortChannelIds \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing QueryShortChannelIds \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
