			...
			extension Bindings {
				...
				public class Result_NoneIOErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_NoneIOErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk() -> Result_NoneIOErrorZ {
						...
						let returnValue = Result_NoneIOErrorZ(cType: nativeCallResult, instantiationContext: "Result_NoneIOErrorZ.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_NoneIOErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_NoneIOErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
