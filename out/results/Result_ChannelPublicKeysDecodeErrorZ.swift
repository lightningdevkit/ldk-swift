...
extension Bindings {
	...
	public class Result_ChannelPublicKeysDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_ChannelPublicKeysDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: ChannelPublicKeys) -> Result_ChannelPublicKeysDecodeErrorZ {
			...
			let returnValue = Result_ChannelPublicKeysDecodeErrorZ(
				...
				instantiationContext: "Result_ChannelPublicKeysDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_ChannelPublicKeysDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_ChannelPublicKeysDecodeErrorZ>) in
		...
		internal func dangleRecursively() -> Result_ChannelPublicKeysDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_ChannelPublicKeysDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_ChannelPublicKeysDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
