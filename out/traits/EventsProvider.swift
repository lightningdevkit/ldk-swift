...
extension Bindings {
	...
	open class EventsProvider: NativeTraitWrapper {
		...
		internal init(
			cType: LDKEventsProvider, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init() {
			...
			super
				.init(
					conflictAvoidingVariableName: 0, instantiationContext: "EventsProvider.swift::\(#function):\(#line)"
				)

			let thisArg = Bindings.instanceToPointer(instance: self)


			func processPendingEventsLambda(this_arg: UnsafeRawPointer?, handler: LDKEventHandler) {
				let instance: EventsProvider = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "EventsProvider::processPendingEventsLambda")

				// Swift callback variable prep


				// Swift callback call
				let swiftCallbackResult = instance.processPendingEvents(
					handler: NativelyImplementedEventHandler(
						cType: handler, instantiationContext: "EventsProvider.swift::init()::\(#function):\(#line)"))
			...
			func freeLambda(this_arg: UnsafeMutableRawPointer?) {
				let instance: EventsProvider = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "EventsProvider::freeLambda")
		...
		open func processPendingEvents(handler: EventHandler) {

			Bindings.print(
				"Error: EventsProvider::processPendingEvents MUST be overridden! Offending class: \(String(describing: self)). Aborting.",
				severity: .ERROR)
			abort()
		}
		...
		internal func dangleRecursively() -> EventsProvider {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing EventsProvider \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}
				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing EventsProvider \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
	}
