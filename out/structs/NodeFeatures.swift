...
extension Bindings {
	...
	public class NodeFeatures: NativeTypeWrapper {
		...
		internal init(
			cType: LDKNodeFeatures, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> NodeFeatures {
			...
			let returnValue = NodeFeatures(
				cType: nativeCallResult, instantiationContext: "NodeFeatures.swift::\(#function):\(#line)")
		...
		public func write() -> [UInt8] {
			...
			let returnValue = Vec_u8Z(
				cType: nativeCallResult, instantiationContext: "NodeFeatures.swift::\(#function):\(#line)", anchor: self
			)
		...
		public class func read(ser: [UInt8]) -> Result_NodeFeaturesDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "NodeFeatures.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> NodeFeatures {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing NodeFeatures \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing NodeFeatures \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
