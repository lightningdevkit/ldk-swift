...
extension Bindings {
	...
	public class ChannelManagerReadArgs: NativeTypeWrapper {
		...
		internal init(
			cType: LDKChannelManagerReadArgs, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func getEntropySource() -> EntropySource? {
			...
			let returnValue = NativelyImplementedEntropySource(
				cType: nativeCallResult.pointee,
				instantiationContext: "ChannelManagerReadArgs.swift::\(#function):\(#line)", anchor: self)
		...
		public func setEntropySource(val: EntropySource) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKChannelManagerReadArgs>) in
		...
		public func getNodeSigner() -> NodeSigner? {
			...
			// return value (do some wrapping)
			let returnValue = NativelyImplementedNodeSigner(
				cType: nativeCallResult.pointee,
				instantiationContext: "ChannelManagerReadArgs.swift::\(#function):\(#line)", anchor: self)
		...
		public func getSignerProvider() -> SignerProvider? {
			...
			// return value (do some wrapping)
			let returnValue = NativelyImplementedSignerProvider(
				cType: nativeCallResult.pointee,
				instantiationContext: "ChannelManagerReadArgs.swift::\(#function):\(#line)", anchor: self)
		...
		public func getFeeEstimator() -> FeeEstimator? {
			...
			// return value (do some wrapping)
			let returnValue = NativelyImplementedFeeEstimator(
				cType: nativeCallResult.pointee,
				instantiationContext: "ChannelManagerReadArgs.swift::\(#function):\(#line)", anchor: self)
		...
		public func getTxBroadcaster() -> BroadcasterInterface? {
			...
			// return value (do some wrapping)
			let returnValue = NativelyImplementedBroadcasterInterface(
				cType: nativeCallResult.pointee,
				instantiationContext: "ChannelManagerReadArgs.swift::\(#function):\(#line)", anchor: self)
		...
		public func getLogger() -> Logger? {
			...
			// return value (do some wrapping)
			let returnValue = NativelyImplementedLogger(
				cType: nativeCallResult.pointee,
				instantiationContext: "ChannelManagerReadArgs.swift::\(#function):\(#line)", anchor: self)
		...
		public func getDefaultConfig() -> UserConfig {
			...
			let returnValue = UserConfig(
				cType: nativeCallResult, instantiationContext: "ChannelManagerReadArgs.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle(false)
		...
		public init(
			entropySource: EntropySource, nodeSigner: NodeSigner, signerProvider: SignerProvider,
			feeEstimator: FeeEstimator, chainMonitor: Watch, txBroadcaster: BroadcasterInterface, router: Router,
			logger: Logger, defaultConfig: UserConfig, channelMonitors: [ChannelMonitor]
		) {
			// native call variable prep

			let channelMonitorsVector = Vec_ChannelMonitorZ(
				array: channelMonitors, instantiationContext: "ChannelManagerReadArgs.swift::\(#function):\(#line)"
			)
			.dangle()


			// native method call
			let nativeCallResult = ChannelManagerReadArgs_new(
				entropySource.activate().cType!, nodeSigner.activate().cType!, signerProvider.activate().cType!,
				feeEstimator.activate().cType!, chainMonitor.activate().cType!, txBroadcaster.activate().cType!,
				router.activate().cType!, logger.activate().cType!, defaultConfig.dynamicallyDangledClone().cType!,
				channelMonitorsVector.cType!)
			...
			super
				.init(
					...
					instantiationContext: "ChannelManagerReadArgs.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> ChannelManagerReadArgs {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing ChannelManagerReadArgs \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing ChannelManagerReadArgs \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
