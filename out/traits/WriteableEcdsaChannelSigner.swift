...
extension Bindings {
	...
	open class WriteableEcdsaChannelSigner: NativeTraitWrapper {
		...
		internal init(
			cType: LDKWriteableEcdsaChannelSigner, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public init(ecdsaChannelSigner: EcdsaChannelSigner) {
			...
			super
				.init(
					...
					instantiationContext: "WriteableEcdsaChannelSigner.swift::\(#function):\(#line)")

			let thisArg = Bindings.instanceToPointer(instance: self)


			func writeLambda(this_arg: UnsafeRawPointer?) -> LDKCVec_u8Z {
				let instance: WriteableEcdsaChannelSigner = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "WriteableEcdsaChannelSigner::writeLambda")
			...
			func freeLambda(this_arg: UnsafeMutableRawPointer?) {
				let instance: WriteableEcdsaChannelSigner = Bindings.pointerToInstance(
					pointer: this_arg!, sourceMarker: "WriteableEcdsaChannelSigner::freeLambda")
		...
		open func write() -> [UInt8] {
			...
			Bindings.print(
				"Error: WriteableEcdsaChannelSigner::write MUST be overridden! Offending class: \(String(describing: self)). Aborting.",
				severity: .ERROR)
			abort()
		}
		...
		public func getEcdsaChannelSigner() -> EcdsaChannelSigner {
			...
			let returnValue = NativelyImplementedEcdsaChannelSigner(
				cType: self.cType!.EcdsaChannelSigner,
				instantiationContext: "WriteableEcdsaChannelSigner.swift::\(#function):\(#line)", anchor: self)
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing WriteableEcdsaChannelSigner \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing WriteableEcdsaChannelSigner \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
	}
