			...
			extension Bindings {
				...
				public class InvoiceRequestFeatures: NativeTypeWrapper {
					...
					internal init(cType: LDKInvoiceRequestFeatures, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				InvoiceRequestFeatures_requires_unknown_bits(thisArgPointer)
					...
					public func setRequiredFeatureBit(bit: UInt) -> Result_NoneNoneZ {
						...
						// native method call
						let nativeCallResult = 
						withUnsafeMutablePointer(to: &self.cType!) { (thisArgPointer: UnsafeMutablePointer<LDKInvoiceRequestFeatures>) in
				InvoiceRequestFeatures_set_required_feature_bit(thisArgPointer, bit)
						...
						// return value (do some wrapping)
						let returnValue = Result_NoneNoneZ(cType: nativeCallResult, instantiationContext: "InvoiceRequestFeatures.swift::\(#function):\(#line)", anchor: self).dangle(false)
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing InvoiceRequestFeatures \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing InvoiceRequestFeatures \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
