...
extension Bindings {
	...
	public class GossipTimestampFilter: NativeTypeWrapper {
		...
		internal init(
			cType: LDKGossipTimestampFilter, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func setChainHash(val: [UInt8]) {
			...
			let valPrimitiveWrapper = ThirtyTwoBytes(
				value: val, instantiationContext: "GossipTimestampFilter.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKGossipTimestampFilter>) in
		...
		public init(chainHashArg: [UInt8], firstTimestampArg: UInt32, timestampRangeArg: UInt32) {
			...
			let chainHashArgPrimitiveWrapper = ThirtyTwoBytes(
				value: chainHashArg, instantiationContext: "GossipTimestampFilter.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult = GossipTimestampFilter_new(
				chainHashArgPrimitiveWrapper.cType!, firstTimestampArg, timestampRangeArg)
			...
			super
				.init(
					...
					instantiationContext: "GossipTimestampFilter.swift::\(#function):\(#line)")
		...
		internal func clone() -> GossipTimestampFilter {
			...
			// return value (do some wrapping)
			let returnValue = GossipTimestampFilter(
				cType: nativeCallResult, instantiationContext: "GossipTimestampFilter.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKGossipTimestampFilter>) in
					GossipTimestampFilter_hash(oPointer)
				}
		...
		public class func read(ser: [UInt8]) -> Result_GossipTimestampFilterDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "GossipTimestampFilter.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_GossipTimestampFilterDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "GossipTimestampFilter.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing GossipTimestampFilter \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing GossipTimestampFilter \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
