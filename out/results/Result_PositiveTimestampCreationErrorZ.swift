...
extension Bindings {
	...
	public class Result_PositiveTimestampCreationErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_PositiveTimestampCreationErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: PositiveTimestamp) -> Result_PositiveTimestampCreationErrorZ {
			...
			let returnValue = Result_PositiveTimestampCreationErrorZ(
				cType: nativeCallResult,
				instantiationContext: "Result_PositiveTimestampCreationErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_PositiveTimestampCreationErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_PositiveTimestampCreationErrorZ>) in
		...
		internal func dangleRecursively() -> Result_PositiveTimestampCreationErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_PositiveTimestampCreationErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_PositiveTimestampCreationErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
