			...
			extension Bindings {
				...
				public class Result_CommitmentSignedDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_CommitmentSignedDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_CommitmentSignedDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> CommitmentSigned? {
						if self.cType?.result_ok == true {
							return CommitmentSigned(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
