			...
			extension Bindings {
				...
				public class RawDataPart: NativeTypeWrapper {
					...
					internal init(cType: LDKRawDataPart, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				RawDataPart_hash(oPointer)
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing RawDataPart \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
