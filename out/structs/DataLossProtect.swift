...
extension Bindings {
	...
	public class DataLossProtect: NativeTypeWrapper {
		...
		internal init(
			cType: LDKDataLossProtect, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func setYourLastPerCommitmentSecret(val: [UInt8]) {
			...
			let valPrimitiveWrapper = ThirtyTwoBytes(
				value: val, instantiationContext: "DataLossProtect.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKDataLossProtect>) in
		...
		public func getMyCurrentPerCommitmentPoint() -> [UInt8] {
			...
			let returnValue = PublicKey(
				cType: nativeCallResult, instantiationContext: "DataLossProtect.swift::\(#function):\(#line)",
				anchor: self
			)
		...
		public init(yourLastPerCommitmentSecretArg: [UInt8], myCurrentPerCommitmentPointArg: [UInt8]) {
			...
			let yourLastPerCommitmentSecretArgPrimitiveWrapper = ThirtyTwoBytes(
				...
				instantiationContext: "DataLossProtect.swift::\(#function):\(#line)")
			...
			// native method call
			let nativeCallResult = DataLossProtect_new(
				yourLastPerCommitmentSecretArgPrimitiveWrapper.cType!,
				myCurrentPerCommitmentPointArgPrimitiveWrapper.cType!)
		...
		internal func clone() -> DataLossProtect {
			...
			// return value (do some wrapping)
			let returnValue = DataLossProtect(
				cType: nativeCallResult, instantiationContext: "DataLossProtect.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing DataLossProtect \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing DataLossProtect \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
