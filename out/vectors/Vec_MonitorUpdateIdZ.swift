...
extension Bindings {
	...
	internal class Vec_MonitorUpdateIdZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_MonitorUpdateIdZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func getValue() -> [MonitorUpdateId] {
			...
			let swiftArray = array.map { (currentCType: LDKMonitorUpdateId) -> MonitorUpdateId in
				MonitorUpdateId(
					cType: currentCType, instantiationContext: "Vec_MonitorUpdateIdZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		internal func dangleRecursively() -> Vec_MonitorUpdateIdZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_MonitorUpdateIdZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Vec_MonitorUpdateIdZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
