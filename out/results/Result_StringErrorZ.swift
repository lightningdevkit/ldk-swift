...
extension Bindings {
	...
	public class Result_StringErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_StringErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: String) -> Result_StringErrorZ {
			...
			let oPrimitiveWrapper = Str(
				value: o, instantiationContext: "Result_StringErrorZ.swift::\(#function):\(#line)"
			)
			.dangle()
			...
			let returnValue = Result_StringErrorZ(
				cType: nativeCallResult, instantiationContext: "Result_StringErrorZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> String? {
			if self.cType?.result_ok == true {
				return Str(
					...
					instantiationContext: "Result_StringErrorZ.swift::\(#function):\(#line)", anchor: self
				)
				.getValue()
			}
			return nil
		...
		internal func dangleRecursively() -> Result_StringErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_StringErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Result_StringErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
