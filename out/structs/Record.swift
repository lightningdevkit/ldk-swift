...
extension Bindings {
	...
	public class Record: NativeTypeWrapper {
		...
		internal init(cType: LDKRecord, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
			self.initialCFreeability = self.cType!.is_owned
		...
		public func setArgs(val: String) {
			...
			let valPrimitiveWrapper = Str(value: val, instantiationContext: "Record.swift::\(#function):\(#line)")
				.dangle()
		...
		internal func clone() -> Record {
			...
			let returnValue = Record(
				cType: nativeCallResult, instantiationContext: "Record.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Record \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
