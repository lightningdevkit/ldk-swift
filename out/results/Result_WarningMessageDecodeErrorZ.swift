...
extension Bindings {
	...
	public class Result_WarningMessageDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_WarningMessageDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: WarningMessage) -> Result_WarningMessageDecodeErrorZ {
			...
			let returnValue = Result_WarningMessageDecodeErrorZ(
				cType: nativeCallResult,
				instantiationContext: "Result_WarningMessageDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_WarningMessageDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_WarningMessageDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_WarningMessageDecodeErrorZ.swift::\(#function):\(#line)", anchor: self
				)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_WarningMessageDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_WarningMessageDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_WarningMessageDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
