...
extension Bindings {
	...
	internal class Vec_PublicKeyZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_PublicKeyZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(array: [[UInt8]], instantiationContext: String) {
			...
			let rustArray = array.map { (currentValueDepth1: [UInt8]) -> LDKPublicKey in
				...
				let currentValueDepth1PrimitiveWrapper = PublicKey(
					value: currentValueDepth1, instantiationContext: "Vec_PublicKeyZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> [[UInt8]] {
			...
			let swiftArray = array.map { (currentCType: LDKPublicKey) -> [UInt8] in
				PublicKey(cType: currentCType, instantiationContext: "Vec_PublicKeyZ.swift::\(#function):\(#line)")
					.dangle().getValue()
			}
			return swiftArray
		}
		...
		internal func dangleRecursively() -> Vec_PublicKeyZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_PublicKeyZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Vec_PublicKeyZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
