...
extension Bindings {
	...
	public class HolderCommitmentTransaction: NativeTypeWrapper {
		...
		internal init(
			cType: LDKHolderCommitmentTransaction, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func setCounterpartySig(val: [UInt8]) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKHolderCommitmentTransaction>) in
		...
		internal func clone() -> HolderCommitmentTransaction {
			...
			// return value (do some wrapping)
			let returnValue = HolderCommitmentTransaction(
				cType: nativeCallResult,
		...
		public class func read(ser: [UInt8]) -> Result_HolderCommitmentTransactionDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "HolderCommitmentTransaction.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_HolderCommitmentTransactionDecodeErrorZ(
				cType: nativeCallResult,
				instantiationContext: "HolderCommitmentTransaction.swift::\(#function):\(#line)")
		...
		public init(
			commitmentTx: CommitmentTransaction, counterpartySig: [UInt8], counterpartyHtlcSigs: [[UInt8]],
			holderFundingKey: [UInt8], counterpartyFundingKey: [UInt8]
		) {
			// native call variable prep
			...
			let holderFundingKeyPrimitiveWrapper = PublicKey(
				value: holderFundingKey,
				instantiationContext: "HolderCommitmentTransaction.swift::\(#function):\(#line)")

			let counterpartyFundingKeyPrimitiveWrapper = PublicKey(
				value: counterpartyFundingKey,
				instantiationContext: "HolderCommitmentTransaction.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult = HolderCommitmentTransaction_new(
				commitmentTx.dynamicallyDangledClone().cType!, counterpartySigPrimitiveWrapper.cType!,
				counterpartyHtlcSigsVector.cType!, holderFundingKeyPrimitiveWrapper.cType!,
				counterpartyFundingKeyPrimitiveWrapper.cType!)
		...
		internal func dangleRecursively() -> HolderCommitmentTransaction {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing HolderCommitmentTransaction \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing HolderCommitmentTransaction \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
