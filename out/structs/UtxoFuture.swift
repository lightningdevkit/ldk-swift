...
extension Bindings {
	...
	public class UtxoFuture: NativeTypeWrapper {
		...
		internal init(
			cType: LDKUtxoFuture, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> UtxoFuture {
			...
			// return value (do some wrapping)
			let returnValue = UtxoFuture(
				cType: nativeCallResult, instantiationContext: "UtxoFuture.swift::\(#function):\(#line)")
				...
				withUnsafePointer(to: self.cType!) { (thisArgPointer: UnsafePointer<LDKUtxoFuture>) in
					...
					withUnsafePointer(to: graph.cType!) { (graphPointer: UnsafePointer<LDKNetworkGraph>) in
						UtxoFuture_resolve_without_forwarding(
							thisArgPointer, graphPointer, result.danglingClone().cType!)
					}
				...
				withUnsafePointer(to: self.cType!) { (thisArgPointer: UnsafePointer<LDKUtxoFuture>) in
					...
					withUnsafePointer(to: graph.cType!) { (graphPointer: UnsafePointer<LDKNetworkGraph>) in
						...
						withUnsafePointer(to: gossip.cType!) { (gossipPointer: UnsafePointer<LDKP2PGossipSync>) in
							UtxoFuture_resolve(
								thisArgPointer, graphPointer, gossipPointer, result.danglingClone().cType!)
						}
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing UtxoFuture \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
