...
extension Bindings {
	...
	public class GossipSync: NativeTypeWrapper {
		...
		internal init(
			cType: LDKGossipSync, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithP2P(a: Bindings.P2PGossipSync) -> GossipSync {
			...
			// return value (do some wrapping)
			let returnValue = GossipSync(
				cType: nativeCallResult, instantiationContext: "GossipSync.swift::\(#function):\(#line)")
		...
		public func getValueAsP2p() -> Bindings.P2PGossipSync? {
			...
			return P2PGossipSync(
				cType: self.cType!.p2p, instantiationContext: "GossipSync.swift::\(#function):\(#line)", anchor: self)
		}
		...
		public func getValueAsRapid() -> Bindings.RapidGossipSync? {
			...
			return RapidGossipSync(
				cType: self.cType!.rapid, instantiationContext: "GossipSync.swift::\(#function):\(#line)", anchor: self)
		}
		...
		internal func dangleRecursively() -> GossipSync {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing GossipSync \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
