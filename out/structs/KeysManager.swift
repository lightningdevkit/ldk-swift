...
extension Bindings {
	...
	public class KeysManager: NativeTypeWrapper {
		...
		internal init(
			cType: LDKKeysManager, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init(seed: [UInt8], startingTimeSecs: UInt64, startingTimeNanos: UInt32) {
			...
			self.instanceNumber = Self.instanceCounter
			super
				.init(conflictAvoidingVariableName: 0, instantiationContext: "KeysManager.swift::\(#function):\(#line)")
		...
		public func getNodeSecretKey() -> [UInt8] {
			...
			let returnValue = SecretKey(
				cType: nativeCallResult, instantiationContext: "KeysManager.swift::\(#function):\(#line)", anchor: self
			)
		...
		public func signSpendableOutputsPsbt(descriptors: [SpendableOutputDescriptor], psbt: [UInt8])
			-> Result_CVec_u8ZNoneZ
		{
			// native call variable prep

			let descriptorsVector = Vec_SpendableOutputDescriptorZ(
				array: descriptors, instantiationContext: "KeysManager.swift::\(#function):\(#line)"
			)
			.dangle()

			let psbtVector = Vec_u8Z(array: psbt, instantiationContext: "KeysManager.swift::\(#function):\(#line)")
				.dangle()
		...
		public func spendSpendableOutputs(
			...
			let outputsVector = Vec_TxOutZ(
				array: outputs, instantiationContext: "KeysManager.swift::\(#function):\(#line)"
			)
			.dangle()

			let changeDestinationScriptVector = Vec_u8Z(
				array: changeDestinationScript, instantiationContext: "KeysManager.swift::\(#function):\(#line)"
			)
			.dangle()

			let locktimeOption = Option_u32Z(
				some: locktime, instantiationContext: "KeysManager.swift::\(#function):\(#line)"
			)
			.danglingClone()
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing KeysManager \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
