...
extension Bindings {
	...
	public class PositiveTimestamp: NativeTypeWrapper {
		...
		internal init(
			cType: LDKPositiveTimestamp, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> PositiveTimestamp {
			...
			let returnValue = PositiveTimestamp(
				cType: nativeCallResult, instantiationContext: "PositiveTimestamp.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing PositiveTimestamp \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing PositiveTimestamp \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
