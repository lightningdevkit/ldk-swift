			...
			extension Bindings {
				...
				open class Confirm: NativeTraitWrapper {
					...
					public init() {
						...
						func getRelevantTxidsLambda(this_arg: UnsafeRawPointer?) -> LDKCVec_C2Tuple_ThirtyTwoBytesCOption_ThirtyTwoBytesZZZ {
							...
							// return value (do some wrapping)
							let returnValue = Vec_C2Tuple_ThirtyTwoBytesCOption_ThirtyTwoBytesZZZ(array: swiftCallbackResult, instantiationContext: "Confirm.swift::init()::\(#function):\(#line)").dangleRecursively().cType!
			...
			public override func getRelevantTxids() -> [([UInt8], [UInt8]?)] {
				...
				// return value (do some wrapping)
				let returnValue = Vec_C2Tuple_ThirtyTwoBytesCOption_ThirtyTwoBytesZZZ(cType: nativeCallResult, instantiationContext: "Confirm.swift::\(#function):\(#line)").getValue()
