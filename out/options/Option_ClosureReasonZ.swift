...
extension Bindings {
	...
	internal class Option_ClosureReasonZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCOption_ClosureReasonZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> Option_ClosureReasonZ {
			...
			// return value (do some wrapping)
			let returnValue = Option_ClosureReasonZ(
				cType: nativeCallResult, instantiationContext: "Option_ClosureReasonZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> ClosureReason? {
			...
			if self.cType!.tag == LDKCOption_ClosureReasonZ_Some {
				return ClosureReason(
					cType: self.cType!.some, instantiationContext: "Option_ClosureReasonZ.swift::\(#function):\(#line)",
					anchor: self
				)
				.dangle()
			}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Option_ClosureReasonZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Option_ClosureReasonZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
