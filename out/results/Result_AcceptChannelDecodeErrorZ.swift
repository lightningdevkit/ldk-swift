...
extension Bindings {
	...
	public class Result_AcceptChannelDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_AcceptChannelDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: AcceptChannel) -> Result_AcceptChannelDecodeErrorZ {
			...
			let returnValue = Result_AcceptChannelDecodeErrorZ(
				...
				instantiationContext: "Result_AcceptChannelDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_AcceptChannelDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_AcceptChannelDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_AcceptChannelDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_AcceptChannelDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_AcceptChannelDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_AcceptChannelDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
