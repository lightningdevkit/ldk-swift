			...
			extension Bindings {
				...
				public class Result_OpenChannelDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_OpenChannelDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_OpenChannelDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> OpenChannel? {
						if self.cType?.result_ok == true {
							return OpenChannel(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
