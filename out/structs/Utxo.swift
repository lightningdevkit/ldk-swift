	...
	public class Utxo: NativeTypeWrapper {
		...
		public func getOutpoint() -> OutPoint {
			...
			let returnValue = OutPoint(
				cType: nativeCallResult, instantiationContext: "Utxo.swift::\(#function):\(#line)", anchor: self
			)
			.dangle(false)
		...
		public init(outpointArg: OutPoint, outputArg: TxOut, satisfactionWeightArg: UInt64) {
			...
			// native method call
			let nativeCallResult = Utxo_new(
				outpointArg.dynamicallyDangledClone().cType!, outputArg.danglingClone().cType!, satisfactionWeightArg)
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Utxo \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
