...
extension Bindings {
	...
	public class PhantomKeysManager: NativeTypeWrapper {
		...
		internal init(
			cType: LDKPhantomKeysManager, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init(seed: [UInt8], startingTimeSecs: UInt64, startingTimeNanos: UInt32, crossNodeSeed: [UInt8]) {
			...
			let nativeCallResult =
				withUnsafePointer(to: tupledSeed) { (tupledSeedPointer: UnsafePointer<UInt8Tuple32>) in
					...
					withUnsafePointer(to: tupledCrossNodeSeed) {
						(tupledCrossNodeSeedPointer: UnsafePointer<UInt8Tuple32>) in
						PhantomKeysManager_new(
							tupledSeedPointer, startingTimeSecs, startingTimeNanos, tupledCrossNodeSeedPointer)
					}
				...
				withUnsafePointer(to: self.cType!) { (thisArgPointer: UnsafePointer<LDKPhantomKeysManager>) in
					...
					withUnsafePointer(to: tupledParams) { (tupledParamsPointer: UnsafePointer<UInt8Tuple32>) in
						PhantomKeysManager_derive_channel_keys(
							thisArgPointer, channelValueSatoshis, tupledParamsPointer)
					}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing PhantomKeysManager \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing PhantomKeysManager \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
