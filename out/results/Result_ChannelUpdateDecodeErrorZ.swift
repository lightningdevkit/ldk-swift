...
extension Bindings {
	...
	public class Result_ChannelUpdateDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_ChannelUpdateDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: ChannelUpdate) -> Result_ChannelUpdateDecodeErrorZ {
			...
			let returnValue = Result_ChannelUpdateDecodeErrorZ(
				...
				instantiationContext: "Result_ChannelUpdateDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_ChannelUpdateDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_ChannelUpdateDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_ChannelUpdateDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_ChannelUpdateDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_ChannelUpdateDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_ChannelUpdateDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
