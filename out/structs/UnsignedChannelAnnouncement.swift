			...
			extension Bindings {
				...
				UnsignedChannelAnnouncement_get_excess_data(thisPtrPointer)
					...
					public func setExcessData(val: [UInt8]) {
						...
						let valVector = Vec_u8Z(array: val, instantiationContext: "UnsignedChannelAnnouncement.swift::\(#function):\(#line)").dangle()
				...
				UnsignedChannelAnnouncement_set_excess_data(thisPtrPointer, valVector.cType!)
					...
					public init(featuresArg: ChannelFeatures, chainHashArg: [UInt8], shortChannelIdArg: UInt64, nodeId1Arg: NodeId, nodeId2Arg: NodeId, bitcoinKey1Arg: NodeId, bitcoinKey2Arg: NodeId, excessDataArg: [UInt8]) {
						// native call variable prep
						
						let chainHashArgPrimitiveWrapper = ThirtyTwoBytes(value: chainHashArg, instantiationContext: "UnsignedChannelAnnouncement.swift::\(#function):\(#line)")
				
						let excessDataArgVector = Vec_u8Z(array: excessDataArg, instantiationContext: "UnsignedChannelAnnouncement.swift::\(#function):\(#line)").dangle()
				

						// native method call
						let nativeCallResult = UnsignedChannelAnnouncement_new(featuresArg.dynamicallyDangledClone().cType!, chainHashArgPrimitiveWrapper.cType!, shortChannelIdArg, nodeId1Arg.dynamicallyDangledClone().cType!, nodeId2Arg.dynamicallyDangledClone().cType!, bitcoinKey1Arg.dynamicallyDangledClone().cType!, bitcoinKey2Arg.dynamicallyDangledClone().cType!, excessDataArgVector.cType!)
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "UnsignedChannelAnnouncement.swift::\(#function):\(#line)")
