...
extension Bindings {
	...
	internal class Tuple_PublicKeyTypeZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKC2Tuple_PublicKeyTypeZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> Tuple_PublicKeyTypeZ {
			...
			let returnValue = Tuple_PublicKeyTypeZ(
				cType: nativeCallResult, instantiationContext: "Tuple_PublicKeyTypeZ.swift::\(#function):\(#line)")
		...
		public init(a: [UInt8], b: BindingsType, instantiationContext: String) {
			...
			let aPrimitiveWrapper = PublicKey(
				value: a, instantiationContext: "Tuple_PublicKeyTypeZ.swift::\(#function):\(#line)")
		...
		public func getA() -> [UInt8] {
			...
			let returnValue = PublicKey(
				cType: self.cType!.a, instantiationContext: "Tuple_PublicKeyTypeZ.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle().getValue()
		...
		public func getB() -> BindingsType {
			...
			let returnValue = NativelyImplementedBindingsType(
				cType: self.cType!.b, instantiationContext: "Tuple_PublicKeyTypeZ.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle()
		...
		internal func dangleRecursively() -> Tuple_PublicKeyTypeZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Tuple_PublicKeyTypeZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Tuple_PublicKeyTypeZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
