...
extension Bindings {
	...
	public class Result_CVec_StrZIOErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_CVec_StrZIOErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: [String]) -> Result_CVec_StrZIOErrorZ {
			...
			let oVector = Vec_StrZ(
				array: o, instantiationContext: "Result_CVec_StrZIOErrorZ.swift::\(#function):\(#line)"
			)
			.dangle()
			...
			let returnValue = Result_CVec_StrZIOErrorZ(
				cType: nativeCallResult, instantiationContext: "Result_CVec_StrZIOErrorZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> [String]? {
			if self.cType?.result_ok == true {
				return Vec_StrZ(
					...
					instantiationContext: "Result_CVec_StrZIOErrorZ.swift::\(#function):\(#line)", anchor: self
				)
				.getValue()
			}
			return nil
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_CVec_StrZIOErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_CVec_StrZIOErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
