...
extension Bindings {
	...
	public class HTLCDestination: NativeTypeWrapper {
		...
		internal init(
			cType: LDKHTLCDestination, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithFailedPayment(paymentHash: [UInt8]) -> HTLCDestination {
			...
			let paymentHashPrimitiveWrapper = ThirtyTwoBytes(
				value: paymentHash, instantiationContext: "HTLCDestination.swift::\(#function):\(#line)")
		...
		public class func read(ser: [UInt8]) -> Result_COption_HTLCDestinationZDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "HTLCDestination.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_COption_HTLCDestinationZDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "HTLCDestination.swift::\(#function):\(#line)")
		...
		public func getValueAsNextHopChannel() -> NextHopChannel? {
			...
			return HTLCDestination_LDKNextHopChannel_Body(
				cType: self.cType!.next_hop_channel,
				instantiationContext: "HTLCDestination.swift::\(#function):\(#line)", anchor: self)
		}
		...
		public func getValueAsUnknownNextHop() -> UnknownNextHop? {
			...
			return HTLCDestination_LDKUnknownNextHop_Body(
				cType: self.cType!.unknown_next_hop,
				instantiationContext: "HTLCDestination.swift::\(#function):\(#line)", anchor: self)
		}
		...
		public func getValueAsInvalidForward() -> InvalidForward? {
			...
			return HTLCDestination_LDKInvalidForward_Body(
				cType: self.cType!.invalid_forward,
				instantiationContext: "HTLCDestination.swift::\(#function):\(#line)", anchor: self)
		}
		...
		public func getValueAsFailedPayment() -> FailedPayment? {
			...
			return HTLCDestination_LDKFailedPayment_Body(
				cType: self.cType!.failed_payment, instantiationContext: "HTLCDestination.swift::\(#function):\(#line)",
				anchor: self)
		}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing HTLCDestination \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing HTLCDestination \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
		...
		public class NextHopChannel: NativeTypeWrapper {
			...
			internal init(
				cType: LDKHTLCDestination_LDKNextHopChannel_Body, instantiationContext: String,
			...
			public func getNodeId() -> [UInt8] {
				...
				let returnValue = PublicKey(
					cType: self.cType!.node_id, instantiationContext: "HTLCDestination.swift::\(#function):\(#line)",
					anchor: self
				)
				.getValue()
		...
		public class UnknownNextHop: NativeTypeWrapper {
			...
			internal init(
				cType: LDKHTLCDestination_LDKUnknownNextHop_Body, instantiationContext: String,
				anchor: NativeTypeWrapper
			) {
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
		...
		public class InvalidForward: NativeTypeWrapper {
			...
			internal init(
				cType: LDKHTLCDestination_LDKInvalidForward_Body, instantiationContext: String,
				anchor: NativeTypeWrapper
			) {
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
		...
		public class FailedPayment: NativeTypeWrapper {
			...
			internal init(
				cType: LDKHTLCDestination_LDKFailedPayment_Body, instantiationContext: String, anchor: NativeTypeWrapper
			) {
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				self.cType = cType
			...
			internal init(
				cType: LDKHTLCDestination_LDKFailedPayment_Body, instantiationContext: String,
				anchor: NativeTypeWrapper, dangle: Bool = false
			) {
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
