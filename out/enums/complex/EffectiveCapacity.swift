...
extension Bindings {
	...
	public class EffectiveCapacity: NativeTypeWrapper {
		...
		internal init(
			cType: LDKEffectiveCapacity, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getValueAsExactLiquidity() -> ExactLiquidity? {
			...
			return EffectiveCapacity_LDKExactLiquidity_Body(
				cType: self.cType!.exact_liquidity,
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing EffectiveCapacity \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing EffectiveCapacity \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
		...
		public class ExactLiquidity: NativeTypeWrapper {
			...
			internal init(
				cType: LDKEffectiveCapacity_LDKExactLiquidity_Body, instantiationContext: String,
		...
		public class Total: NativeTypeWrapper {
			...
			internal init(
				cType: LDKEffectiveCapacity_LDKTotal_Body, instantiationContext: String, anchor: NativeTypeWrapper
			) {
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				self.cType = cType
