			...
			extension Bindings {
				...
				public class Result_InitDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_InitDecodeErrorZ) {
					...
					internal init(cType: LDKCResult_InitDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_InitDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> BindingsInit? {
						if self.cType?.result_ok == true {
							return BindingsInit(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
