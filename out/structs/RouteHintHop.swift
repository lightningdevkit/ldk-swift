...
extension Bindings {
	...
	public class RouteHintHop: NativeTypeWrapper {
		...
		internal init(
			cType: LDKRouteHintHop, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getSrcNodeId() -> [UInt8] {
			...
			let returnValue = PublicKey(
				cType: nativeCallResult, instantiationContext: "RouteHintHop.swift::\(#function):\(#line)", anchor: self
			)
		...
		public func setSrcNodeId(val: [UInt8]) {
			...
			let valPrimitiveWrapper = PublicKey(
				value: val, instantiationContext: "RouteHintHop.swift::\(#function):\(#line)")
		...
		public func setHtlcMinimumMsat(val: UInt64?) {
			...
			let valOption = Option_u64Z(some: val, instantiationContext: "RouteHintHop.swift::\(#function):\(#line)")
				.danglingClone()
		...
		public init(
			srcNodeIdArg: [UInt8], shortChannelIdArg: UInt64, feesArg: RoutingFees, cltvExpiryDeltaArg: UInt16,
			htlcMinimumMsatArg: UInt64?, htlcMaximumMsatArg: UInt64?
		) {
			// native call variable prep

			let srcNodeIdArgPrimitiveWrapper = PublicKey(
				value: srcNodeIdArg, instantiationContext: "RouteHintHop.swift::\(#function):\(#line)")

			let htlcMinimumMsatArgOption = Option_u64Z(
				some: htlcMinimumMsatArg, instantiationContext: "RouteHintHop.swift::\(#function):\(#line)"
			)
			.danglingClone()

			let htlcMaximumMsatArgOption = Option_u64Z(
				some: htlcMaximumMsatArg, instantiationContext: "RouteHintHop.swift::\(#function):\(#line)"
			)
			.danglingClone()


			// native method call
			let nativeCallResult = RouteHintHop_new(
				srcNodeIdArgPrimitiveWrapper.cType!, shortChannelIdArg, feesArg.dynamicallyDangledClone().cType!,
				cltvExpiryDeltaArg, htlcMinimumMsatArgOption.cType!, htlcMaximumMsatArgOption.cType!)
			...
			super
				.init(
					conflictAvoidingVariableName: 0, instantiationContext: "RouteHintHop.swift::\(#function):\(#line)")
		...
		internal func clone() -> RouteHintHop {
			...
			// return value (do some wrapping)
			let returnValue = RouteHintHop(
				cType: nativeCallResult, instantiationContext: "RouteHintHop.swift::\(#function):\(#line)")
		...
		public class func read(ser: [UInt8]) -> Result_RouteHintHopDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "RouteHintHop.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_RouteHintHopDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "RouteHintHop.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> RouteHintHop {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing RouteHintHop \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing RouteHintHop \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
