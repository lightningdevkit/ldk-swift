...
extension Bindings {
	...
	public class NetworkUpdate: NativeTypeWrapper {
		...
		internal init(
			cType: LDKNetworkUpdate, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> NetworkUpdate {
			...
			// return value (do some wrapping)
			let returnValue = NetworkUpdate(
				cType: nativeCallResult, instantiationContext: "NetworkUpdate.swift::\(#function):\(#line)")
		...
		public class func initWithNodeFailure(nodeId: [UInt8], isPermanent: Bool) -> NetworkUpdate {
			...
			let nodeIdPrimitiveWrapper = PublicKey(
				value: nodeId, instantiationContext: "NetworkUpdate.swift::\(#function):\(#line)")
		...
		public class func read(ser: [UInt8]) -> Result_COption_NetworkUpdateZDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "NetworkUpdate.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_COption_NetworkUpdateZDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "NetworkUpdate.swift::\(#function):\(#line)")
		...
		public func getValueAsChannelUpdateMessage() -> ChannelUpdateMessage? {
			...
			return NetworkUpdate_LDKChannelUpdateMessage_Body(
				cType: self.cType!.channel_update_message,
				instantiationContext: "NetworkUpdate.swift::\(#function):\(#line)", anchor: self)
		}
		...
		public func getValueAsChannelFailure() -> ChannelFailure? {
			...
			return NetworkUpdate_LDKChannelFailure_Body(
				cType: self.cType!.channel_failure, instantiationContext: "NetworkUpdate.swift::\(#function):\(#line)",
				anchor: self)
		}
		...
		public func getValueAsNodeFailure() -> NodeFailure? {
			...
			return NetworkUpdate_LDKNodeFailure_Body(
				cType: self.cType!.node_failure, instantiationContext: "NetworkUpdate.swift::\(#function):\(#line)",
				anchor: self)
		}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing NetworkUpdate \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing NetworkUpdate \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
		...
		public class ChannelUpdateMessage: NativeTypeWrapper {
			...
			internal init(
				cType: LDKNetworkUpdate_LDKChannelUpdateMessage_Body, instantiationContext: String,
			...
			public func getMsg() -> Bindings.ChannelUpdate {
				...
				let returnValue = Bindings.ChannelUpdate(
					cType: self.cType!.msg, instantiationContext: "NetworkUpdate.swift::\(#function):\(#line)",
					anchor: self)
		...
		public class ChannelFailure: NativeTypeWrapper {
			...
			internal init(
				cType: LDKNetworkUpdate_LDKChannelFailure_Body, instantiationContext: String, anchor: NativeTypeWrapper
			) {
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				self.cType = cType
		...
		public class NodeFailure: NativeTypeWrapper {
			...
			internal init(
				cType: LDKNetworkUpdate_LDKNodeFailure_Body, instantiationContext: String, anchor: NativeTypeWrapper
			) {
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				self.cType = cType
			...
			public func getNodeId() -> [UInt8] {
				...
				let returnValue = PublicKey(
					cType: self.cType!.node_id, instantiationContext: "NetworkUpdate.swift::\(#function):\(#line)",
					anchor: self
				)
				.getValue()
