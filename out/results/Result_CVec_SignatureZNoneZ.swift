			...
			extension Bindings {
				...
				public class Result_CVec_SignatureZNoneZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_CVec_SignatureZNoneZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: [[UInt8]]) -> Result_CVec_SignatureZNoneZ {
						...
						let oVector = Vec_SignatureZ(array: o, instantiationContext: "Result_CVec_SignatureZNoneZ.swift::\(#function):\(#line)").dangle()
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_CVec_SignatureZNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_CVec_SignatureZNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
