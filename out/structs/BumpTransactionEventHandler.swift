			...
			extension Bindings {
				...
				public class BumpTransactionEventHandler: NativeTypeWrapper {
					...
					internal init(cType: LDKBumpTransactionEventHandler, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
					...
					/// 
					/// [`Event::BumpTransaction`]: crate::events::Event::BumpTransaction
					public init(broadcaster: BroadcasterInterface, utxoSource: CoinSelectionSource, signerProvider: SignerProvider, logger: Logger) {
						// native call variable prep
						

						// native method call
						let nativeCallResult = BumpTransactionEventHandler_new(broadcaster.activate().cType!, utxoSource.activate().cType!, signerProvider.activate().cType!, logger.activate().cType!)
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "BumpTransactionEventHandler.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing BumpTransactionEventHandler \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing BumpTransactionEventHandler \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
