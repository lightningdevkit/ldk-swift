	...
	public class SpliceLocked: NativeTypeWrapper {
		...
		public func setChannelId(val: [UInt8]) {
			...
			let valPrimitiveWrapper = ThirtyTwoBytes(
				value: val, instantiationContext: "SpliceLocked.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKSpliceLocked>) in
					SpliceLocked_set_channel_id(thisPtrPointer, valPrimitiveWrapper.cType!)
				}
		...
		public init(channelIdArg: [UInt8]) {
			...
			let channelIdArgPrimitiveWrapper = ThirtyTwoBytes(
				value: channelIdArg, instantiationContext: "SpliceLocked.swift::\(#function):\(#line)")


			// native method call
			let nativeCallResult = SpliceLocked_new(channelIdArgPrimitiveWrapper.cType!)
