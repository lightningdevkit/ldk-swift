			...
			extension Bindings {
				...
				public class Bolt11InvoiceFeatures: NativeTypeWrapper {
					...
					internal init(cType: LDKBolt11InvoiceFeatures, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				Bolt11InvoiceFeatures_clone(origPointer)
						...
						// return value (do some wrapping)
						let returnValue = Bolt11InvoiceFeatures(cType: nativeCallResult, instantiationContext: "Bolt11InvoiceFeatures.swift::\(#function):\(#line)")
				...
				Bolt11InvoiceFeatures_requires_unknown_bits(thisArgPointer)
					...
					public func setRequiredFeatureBit(bit: UInt) -> Result_NoneNoneZ {
						...
						// native method call
						let nativeCallResult = 
						withUnsafeMutablePointer(to: &self.cType!) { (thisArgPointer: UnsafeMutablePointer<LDKBolt11InvoiceFeatures>) in
				Bolt11InvoiceFeatures_set_required_feature_bit(thisArgPointer, bit)
						...
						// return value (do some wrapping)
						let returnValue = Result_NoneNoneZ(cType: nativeCallResult, instantiationContext: "Bolt11InvoiceFeatures.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				Bolt11InvoiceFeatures_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "Bolt11InvoiceFeatures.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public class func read(ser: [UInt8]) -> Result_Bolt11InvoiceFeaturesDecodeErrorZ {
						...
						let serPrimitiveWrapper = u8slice(value: ser, instantiationContext: "Bolt11InvoiceFeatures.swift::\(#function):\(#line)")
						...
						// return value (do some wrapping)
						let returnValue = Result_Bolt11InvoiceFeaturesDecodeErrorZ(cType: nativeCallResult, instantiationContext: "Bolt11InvoiceFeatures.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Bolt11InvoiceFeatures \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Bolt11InvoiceFeatures \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
