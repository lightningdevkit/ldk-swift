...
extension Bindings {
	...
	public class UnsignedGossipMessage: NativeTypeWrapper {
		...
		internal init(
			cType: LDKUnsignedGossipMessage, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> UnsignedGossipMessage {
			...
			// return value (do some wrapping)
			let returnValue = UnsignedGossipMessage(
				cType: nativeCallResult, instantiationContext: "UnsignedGossipMessage.swift::\(#function):\(#line)")
		...
		public func getValueAsChannelAnnouncement() -> Bindings.UnsignedChannelAnnouncement? {
			...
			return UnsignedChannelAnnouncement(
				cType: self.cType!.channel_announcement,
				instantiationContext: "UnsignedGossipMessage.swift::\(#function):\(#line)", anchor: self)
		}
		...
		public func getValueAsChannelUpdate() -> Bindings.UnsignedChannelUpdate? {
			...
			return UnsignedChannelUpdate(
				cType: self.cType!.channel_update,
				instantiationContext: "UnsignedGossipMessage.swift::\(#function):\(#line)", anchor: self)
		}
		...
		public func getValueAsNodeAnnouncement() -> Bindings.UnsignedNodeAnnouncement? {
			...
			return UnsignedNodeAnnouncement(
				cType: self.cType!.node_announcement,
				instantiationContext: "UnsignedGossipMessage.swift::\(#function):\(#line)", anchor: self)
		}
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing UnsignedGossipMessage \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing UnsignedGossipMessage \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
