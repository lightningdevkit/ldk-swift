			...
			extension Bindings {
				...
				internal class Vec_BlindedPathZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_BlindedPathZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
...
BlindedPath(cType: currentCType, instantiationContext: "Vec_BlindedPathZ.swift::\(#function):\(#line)", anchor: self).dangle()
						}
						return swiftArray
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Vec_BlindedPathZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Vec_BlindedPathZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
