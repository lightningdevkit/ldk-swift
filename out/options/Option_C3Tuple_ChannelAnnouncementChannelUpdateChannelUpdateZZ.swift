...
///
internal typealias Option_C3Tuple_ChannelAnnouncementChannelUpdateChannelUpdateZZ = Bindings
...
extension Bindings {
	...
	internal class Option_C3Tuple_ChannelAnnouncementChannelUpdateChannelUpdateZZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCOption_C3Tuple_ChannelAnnouncementChannelUpdateChannelUpdateZZ, instantiationContext: String
		) {
		...
		internal init(some: (ChannelAnnouncement, ChannelUpdate, ChannelUpdate)?, instantiationContext: String) {
			...
			if let some = some {
				...
				let someTuple = Tuple_ChannelAnnouncementChannelUpdateChannelUpdateZ(
					tuple: some,
					instantiationContext:
						"Option_C3Tuple_ChannelAnnouncementChannelUpdateChannelUpdateZZ.swift::\(#function):\(#line)"
				)
				.danglingClone()
		...
		internal func clone() -> Option_C3Tuple_ChannelAnnouncementChannelUpdateChannelUpdateZZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCOption_C3Tuple_ChannelAnnouncementChannelUpdateChannelUpdateZZ>) in
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Option_C3Tuple_ChannelAnnouncementChannelUpdateChannelUpdateZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Option_C3Tuple_ChannelAnnouncementChannelUpdateChannelUpdateZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
