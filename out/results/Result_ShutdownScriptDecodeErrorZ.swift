...
extension Bindings {
	...
	public class Result_ShutdownScriptDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_ShutdownScriptDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: ShutdownScript) -> Result_ShutdownScriptDecodeErrorZ {
			...
			let returnValue = Result_ShutdownScriptDecodeErrorZ(
				cType: nativeCallResult,
				instantiationContext: "Result_ShutdownScriptDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_ShutdownScriptDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_ShutdownScriptDecodeErrorZ>) in
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_ShutdownScriptDecodeErrorZ.swift::\(#function):\(#line)", anchor: self
				)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_ShutdownScriptDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_ShutdownScriptDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_ShutdownScriptDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
