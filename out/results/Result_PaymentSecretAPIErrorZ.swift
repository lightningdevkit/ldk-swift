			...
			extension Bindings {
				...
				public class Result_PaymentSecretAPIErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_PaymentSecretAPIErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: [UInt8]) -> Result_PaymentSecretAPIErrorZ {
						...
						let oPrimitiveWrapper = ThirtyTwoBytes(value: o, instantiationContext: "Result_PaymentSecretAPIErrorZ.swift::\(#function):\(#line)")
						...
						let returnValue = Result_PaymentSecretAPIErrorZ(cType: nativeCallResult, instantiationContext: "Result_PaymentSecretAPIErrorZ.swift::\(#function):\(#line)")
				...
				CResult_PaymentSecretAPIErrorZ_clone(origPointer)
					...
					public func getError() -> APIError? {
						if self.cType?.result_ok == false {
							return APIError(cType: self.cType!.contents.err.pointee, instantiationContext: "Result_PaymentSecretAPIErrorZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					public func getValue() -> [UInt8]? {
						if self.cType?.result_ok == true {
							return ThirtyTwoBytes(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_PaymentSecretAPIErrorZ.swift::\(#function):\(#line)", anchor: self).getValue()
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_PaymentSecretAPIErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_PaymentSecretAPIErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
