			...
			extension Bindings {
				...
				public class Result_PingDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_PingDecodeErrorZ) {
					...
					internal init(cType: LDKCResult_PingDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				CResult_PingDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> Ping? {
						if self.cType?.result_ok == true {
							return Ping(cType: self.cType!.contents.result.pointee, anchor: self)
						}
						return nil
					}
