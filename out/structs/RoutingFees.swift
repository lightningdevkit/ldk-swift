...
extension Bindings {
	...
	public class RoutingFees: NativeTypeWrapper {
		...
		internal init(
			cType: LDKRoutingFees, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init(baseMsatArg: UInt32, proportionalMillionthsArg: UInt32) {
			...
			self.instanceNumber = Self.instanceCounter
			super
				.init(conflictAvoidingVariableName: 0, instantiationContext: "RoutingFees.swift::\(#function):\(#line)")
		...
		internal func clone() -> RoutingFees {
			...
			let returnValue = RoutingFees(
				cType: nativeCallResult, instantiationContext: "RoutingFees.swift::\(#function):\(#line)")
		...
		public func write() -> [UInt8] {
			...
			let returnValue = Vec_u8Z(
				cType: nativeCallResult, instantiationContext: "RoutingFees.swift::\(#function):\(#line)", anchor: self
			)
		...
		public class func read(ser: [UInt8]) -> Result_RoutingFeesDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "RoutingFees.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing RoutingFees \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
