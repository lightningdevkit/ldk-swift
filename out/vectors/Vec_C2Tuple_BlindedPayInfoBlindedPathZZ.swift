...
extension Bindings {
	...
	internal class Vec_C2Tuple_BlindedPayInfoBlindedPathZZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_C2Tuple_BlindedPayInfoBlindedPathZZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(array: [(BlindedPayInfo, BlindedPath)], instantiationContext: String) {
			...
			let rustArray = array.map {
				(currentValueDepth1: (BlindedPayInfo, BlindedPath)) -> LDKC2Tuple_BlindedPayInfoBlindedPathZ in

				let currentValueDepth1Tuple = Tuple_BlindedPayInfoBlindedPathZ(
					tuple: currentValueDepth1,
					instantiationContext: "Vec_C2Tuple_BlindedPayInfoBlindedPathZZ.swift::\(#function):\(#line)"
				)
				.danglingClone()
			...
			let dataContainer = UnsafeMutablePointer<LDKC2Tuple_BlindedPayInfoBlindedPathZ>
		...
		public func getValue() -> [(BlindedPayInfo, BlindedPath)] {
			...
			let swiftArray = array.map {
				(currentCType: LDKC2Tuple_BlindedPayInfoBlindedPathZ) -> (BlindedPayInfo, BlindedPath) in
		...
		internal func dangleRecursively() -> Vec_C2Tuple_BlindedPayInfoBlindedPathZZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_C2Tuple_BlindedPayInfoBlindedPathZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Vec_C2Tuple_BlindedPayInfoBlindedPathZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
