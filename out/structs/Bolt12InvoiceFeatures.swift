			...
			extension Bindings {
				...
				public class Bolt12InvoiceFeatures: NativeTypeWrapper {
					...
					internal init(cType: LDKBolt12InvoiceFeatures, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				Bolt12InvoiceFeatures_set_required_feature_bit(thisArgPointer, bit)
						...
						// return value (do some wrapping)
						let returnValue = Result_NoneNoneZ(cType: nativeCallResult, instantiationContext: "Bolt12InvoiceFeatures.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				Bolt12InvoiceFeatures_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "Bolt12InvoiceFeatures.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public class func read(ser: [UInt8]) -> Result_Bolt12InvoiceFeaturesDecodeErrorZ {
						...
						let serPrimitiveWrapper = u8slice(value: ser, instantiationContext: "Bolt12InvoiceFeatures.swift::\(#function):\(#line)")
						...
						// return value (do some wrapping)
						let returnValue = Result_Bolt12InvoiceFeaturesDecodeErrorZ(cType: nativeCallResult, instantiationContext: "Bolt12InvoiceFeatures.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Bolt12InvoiceFeatures \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Bolt12InvoiceFeatures \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
