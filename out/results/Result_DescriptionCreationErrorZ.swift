...
extension Bindings {
	...
	public class Result_DescriptionCreationErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_DescriptionCreationErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: Description) -> Result_DescriptionCreationErrorZ {
			...
			let returnValue = Result_DescriptionCreationErrorZ(
				cType: nativeCallResult,
				instantiationContext: "Result_DescriptionCreationErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_DescriptionCreationErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_DescriptionCreationErrorZ>) in
		...
		public func getValue() -> Description? {
			if self.cType?.result_ok == true {
				return Description(
					...
					instantiationContext: "Result_DescriptionCreationErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}


		internal func dangle(_ shouldDangle: Bool = true) -> Result_DescriptionCreationErrorZ {
			self.dangling = shouldDangle
			return self
		}

		internal func dangleRecursively() -> Result_DescriptionCreationErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_DescriptionCreationErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_DescriptionCreationErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
