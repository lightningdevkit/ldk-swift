...
extension Bindings {
	...
	public class SpendableOutputDescriptor: NativeTypeWrapper {
		...
		public class func initWithStaticOutput(
			outpoint: Bindings.OutPoint, output: Bindings.TxOut, channelKeysId: [UInt8]
		) -> SpendableOutputDescriptor {
			// native call variable prep

			let channelKeysIdPrimitiveWrapper = ThirtyTwoBytes(
				value: channelKeysId, instantiationContext: "SpendableOutputDescriptor.swift::\(#function):\(#line)")
			...
			let nativeCallResult = SpendableOutputDescriptor_static_output(
				...
				channelKeysIdPrimitiveWrapper.cType!)
		...
		public class StaticOutput: NativeTypeWrapper {
			...
			public func getChannelKeysId() -> [UInt8] {
				...
				let returnValue = ThirtyTwoBytes(
					cType: self.cType!.channel_keys_id,
					instantiationContext: "SpendableOutputDescriptor.swift::\(#function):\(#line)", anchor: self
				)
				.getValue()
