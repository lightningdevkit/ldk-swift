			...
			extension Bindings {
				...
				public class Result_BlindedHopDecodeErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_BlindedHopDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: BlindedHop) -> Result_BlindedHopDecodeErrorZ {
						...
						let returnValue = Result_BlindedHopDecodeErrorZ(cType: nativeCallResult, instantiationContext: "Result_BlindedHopDecodeErrorZ.swift::\(#function):\(#line)")
				...
				CResult_BlindedHopDecodeErrorZ_clone(origPointer)
					...
					public func getError() -> DecodeError? {
						if self.cType?.result_ok == false {
							return DecodeError(cType: self.cType!.contents.err.pointee, instantiationContext: "Result_BlindedHopDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					public func getValue() -> BlindedHop? {
						if self.cType?.result_ok == true {
							return BlindedHop(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_BlindedHopDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_BlindedHopDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_BlindedHopDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
