			...
			extension Bindings {
				...
				public class Result_InvoiceSemanticErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_InvoiceSemanticErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: Invoice) -> Result_InvoiceSemanticErrorZ {
						...
						let returnValue = Result_InvoiceSemanticErrorZ(cType: nativeCallResult, instantiationContext: "Result_InvoiceSemanticErrorZ.swift::\(#function):\(#line)")
				...
				CResult_InvoiceSemanticErrorZ_clone(origPointer)
					...
					public func getValue() -> Invoice? {
						if self.cType?.result_ok == true {
							return Invoice(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_InvoiceSemanticErrorZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_InvoiceSemanticErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_InvoiceSemanticErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
