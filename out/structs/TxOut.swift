...
extension Bindings {
	...
	public class TxOut: NativeTypeWrapper {
		...
		public init(scriptPubkey: [UInt8], value: UInt64) {
			...
			let scriptPubkeyVector = Vec_u8Z(
				array: scriptPubkey, instantiationContext: "TxOut.swift::\(#function):\(#line)"
			)
			.dangle()
		...
		internal func dangleRecursively() -> TxOut {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing TxOut \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
