...
extension Bindings {
	...
	public class Result_CVec_u8ZPeerHandleErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_CVec_u8ZPeerHandleErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: [UInt8]) -> Result_CVec_u8ZPeerHandleErrorZ {
			...
			let oVector = Vec_u8Z(
				array: o, instantiationContext: "Result_CVec_u8ZPeerHandleErrorZ.swift::\(#function):\(#line)"
			)
			.dangle()
			...
			let returnValue = Result_CVec_u8ZPeerHandleErrorZ(
				...
				instantiationContext: "Result_CVec_u8ZPeerHandleErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_CVec_u8ZPeerHandleErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_CVec_u8ZPeerHandleErrorZ>) in
		...
		public func getError() -> PeerHandleError? {
			if self.cType?.result_ok == false {
				return PeerHandleError(
					...
					instantiationContext: "Result_CVec_u8ZPeerHandleErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_CVec_u8ZPeerHandleErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_CVec_u8ZPeerHandleErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_CVec_u8ZPeerHandleErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
