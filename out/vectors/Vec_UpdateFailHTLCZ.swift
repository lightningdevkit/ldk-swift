			...
			extension Bindings {
				...
				internal class Vec_UpdateFailHTLCZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_UpdateFailHTLCZ, instantiationContext: String) {
					...
					internal init(cType: LDKCVec_UpdateFailHTLCZ, instantiationContext: String, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(cType: LDKCVec_UpdateFailHTLCZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(array: [UpdateFailHTLC], instantiationContext: String) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						super.init(conflictAvoidingVariableName: 0, instantiationContext: instantiationContext)
...
UpdateFailHTLC(cType: currentCType, instantiationContext: "Vec_UpdateFailHTLCZ.swift::\(#function):\(#line)", anchor: self).dangle()
						}
						return swiftArray
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Vec_UpdateFailHTLCZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Vec_UpdateFailHTLCZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
