...
extension Bindings {
	...
	public class Result_NetAddressDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_NetAddressDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: NetAddress) -> Result_NetAddressDecodeErrorZ {
			...
			let returnValue = Result_NetAddressDecodeErrorZ(
				...
				instantiationContext: "Result_NetAddressDecodeErrorZ.swift::\(#function):\(#line)")
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_NetAddressDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_NetAddressDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_NetAddressDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_NetAddressDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
