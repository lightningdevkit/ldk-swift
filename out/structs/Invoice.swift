...
extension Bindings {
	...
	public class Invoice: NativeTypeWrapper {
		...
		internal func clone() -> Invoice {
			...
			// return value (do some wrapping)
			let returnValue = Invoice(
				cType: nativeCallResult, instantiationContext: "Invoice.swift::\(#function):\(#line)")
		...
		public func signableHash() -> [UInt8] {
			...
			let returnValue = ThirtyTwoBytes(
				cType: nativeCallResult, instantiationContext: "Invoice.swift::\(#function):\(#line)", anchor: self
			)
		...
		internal func dangleRecursively() -> Invoice {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing Invoice \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
			}
		}
