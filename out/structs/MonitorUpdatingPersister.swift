			...
			extension Bindings {
				...
				public class MonitorUpdatingPersister: NativeTypeWrapper {
					...
					internal init(cType: LDKMonitorUpdatingPersister, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
					...
					public init(kvStore: KVStore, logger: Logger, maximumPendingUpdates: UInt64, entropySource: EntropySource, signerProvider: SignerProvider) {
						// native call variable prep
						

						// native method call
						let nativeCallResult = MonitorUpdatingPersister_new(kvStore.activate().cType!, logger.activate().cType!, maximumPendingUpdates, entropySource.activate().cType!, signerProvider.activate().cType!)
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "MonitorUpdatingPersister.swift::\(#function):\(#line)")
					...
					public func readAllChannelMonitorsWithUpdates(broadcaster: BroadcasterInterface, feeEstimator: FeeEstimator) -> Result_CVec_C2Tuple_ThirtyTwoBytesChannelMonitorZZIOErrorZ {
						// native call variable prep
						...
						withUnsafePointer(to: feeEstimator.activate().cType!) { (feeEstimatorPointer: UnsafePointer<LDKFeeEstimator>) in
				MonitorUpdatingPersister_read_all_channel_monitors_with_updates(thisArgPointer, broadcasterPointer, feeEstimatorPointer)
						...
						// return value (do some wrapping)
						let returnValue = Result_CVec_C2Tuple_ThirtyTwoBytesChannelMonitorZZIOErrorZ(cType: nativeCallResult, instantiationContext: "MonitorUpdatingPersister.swift::\(#function):\(#line)", anchor: self).dangle(false)
					...
					public func readChannelMonitorWithUpdates(broadcaster: BroadcasterInterface, feeEstimator: FeeEstimator, monitorKey: String) -> Result_C2Tuple_ThirtyTwoBytesChannelMonitorZIOErrorZ {
						// native call variable prep
						
						let monitorKeyPrimitiveWrapper = Str(value: monitorKey, instantiationContext: "MonitorUpdatingPersister.swift::\(#function):\(#line)").dangle()
						...
						withUnsafePointer(to: feeEstimator.activate().cType!) { (feeEstimatorPointer: UnsafePointer<LDKFeeEstimator>) in
				MonitorUpdatingPersister_read_channel_monitor_with_updates(thisArgPointer, broadcasterPointer, feeEstimatorPointer, monitorKeyPrimitiveWrapper.cType!)
						}
						...
						// return value (do some wrapping)
						let returnValue = Result_C2Tuple_ThirtyTwoBytesChannelMonitorZIOErrorZ(cType: nativeCallResult, instantiationContext: "MonitorUpdatingPersister.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				MonitorUpdatingPersister_cleanup_stale_updates(thisArgPointer, lazy)
						...
						// return value (do some wrapping)
						let returnValue = Result_NoneIOErrorZ(cType: nativeCallResult, instantiationContext: "MonitorUpdatingPersister.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				MonitorUpdatingPersister_as_Persist(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = NativelyImplementedPersist(cType: nativeCallResult, instantiationContext: "MonitorUpdatingPersister.swift::\(#function):\(#line)", anchor: self)
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing MonitorUpdatingPersister \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing MonitorUpdatingPersister \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
