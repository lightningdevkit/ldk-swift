...
extension Bindings {
	...
	public class ChannelFeatures: NativeTypeWrapper {
		...
		internal init(
			cType: LDKChannelFeatures, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> ChannelFeatures {
			...
			let returnValue = ChannelFeatures(
				cType: nativeCallResult, instantiationContext: "ChannelFeatures.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKChannelFeatures>) in
					ChannelFeatures_hash(oPointer)
				}
		...
		public func setRequiredFeatureBit(bit: UInt) -> Result_NoneNoneZ {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisArgPointer: UnsafeMutablePointer<LDKChannelFeatures>) in
		...
		public class func read(ser: [UInt8]) -> Result_ChannelFeaturesDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "ChannelFeatures.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing ChannelFeatures \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing ChannelFeatures \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
