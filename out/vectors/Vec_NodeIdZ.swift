			...
			extension Bindings {
				...
				internal class Vec_NodeIdZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_NodeIdZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
...
NodeId(cType: currentCType, instantiationContext: "Vec_NodeIdZ.swift::\(#function):\(#line)", anchor: self).dangle()
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing Vec_NodeIdZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
