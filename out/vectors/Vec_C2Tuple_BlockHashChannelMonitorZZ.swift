...
extension Bindings {
	...
	internal class Vec_C2Tuple_BlockHashChannelMonitorZZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCVec_C2Tuple_BlockHashChannelMonitorZZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal init(array: [([UInt8], ChannelMonitor)], instantiationContext: String) {
			...
			let rustArray = array.map {
				(currentValueDepth1: ([UInt8], ChannelMonitor)) -> LDKC2Tuple_BlockHashChannelMonitorZ in

				let currentValueDepth1Tuple = Tuple_BlockHashChannelMonitorZ(
					tuple: currentValueDepth1,
					instantiationContext: "Vec_C2Tuple_BlockHashChannelMonitorZZ.swift::\(#function):\(#line)"
				)
				.danglingClone()
			...
			let dataContainer = UnsafeMutablePointer<LDKC2Tuple_BlockHashChannelMonitorZ>
		...
		public func getValue() -> [([UInt8], ChannelMonitor)] {
			...
			let swiftArray = array.map {
				(currentCType: LDKC2Tuple_BlockHashChannelMonitorZ) -> ([UInt8], ChannelMonitor) in
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Vec_C2Tuple_BlockHashChannelMonitorZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Vec_C2Tuple_BlockHashChannelMonitorZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
