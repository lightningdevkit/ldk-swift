...
extension Bindings {
	...
	public class FilesystemStore: NativeTypeWrapper {
		...
		internal init(
			cType: LDKFilesystemStore, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init(dataDir: String) {
			...
			let dataDirPrimitiveWrapper = Str(
				value: dataDir, instantiationContext: "FilesystemStore.swift::\(#function):\(#line)"
			)
			.dangle()
			...
			super
				.init(
					...
					instantiationContext: "FilesystemStore.swift::\(#function):\(#line)")
		...
		public func getDataDir() -> String {
			...
			// return value (do some wrapping)
			let returnValue = Str(
				cType: nativeCallResult, instantiationContext: "FilesystemStore.swift::\(#function):\(#line)",
				anchor: self
			)
		...
		public func asKVStore() -> KVStore {
			...
			// return value (do some wrapping)
			let returnValue = NativelyImplementedKVStore(
				cType: nativeCallResult, instantiationContext: "FilesystemStore.swift::\(#function):\(#line)",
				anchor: self)
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing FilesystemStore \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing FilesystemStore \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
