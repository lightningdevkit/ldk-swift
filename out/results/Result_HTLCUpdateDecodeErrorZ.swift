...
extension Bindings {
	...
	public class Result_HTLCUpdateDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_HTLCUpdateDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: HTLCUpdate) -> Result_HTLCUpdateDecodeErrorZ {
			...
			let returnValue = Result_HTLCUpdateDecodeErrorZ(
				...
				instantiationContext: "Result_HTLCUpdateDecodeErrorZ.swift::\(#function):\(#line)")
		...
		public func getError() -> DecodeError? {
			if self.cType?.result_ok == false {
				return DecodeError(
					...
					instantiationContext: "Result_HTLCUpdateDecodeErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_HTLCUpdateDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_HTLCUpdateDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_HTLCUpdateDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
