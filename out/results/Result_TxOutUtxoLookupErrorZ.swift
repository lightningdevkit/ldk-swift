			...
			extension Bindings {
				...
				public class Result_TxOutUtxoLookupErrorZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_TxOutUtxoLookupErrorZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: TxOut) -> Result_TxOutUtxoLookupErrorZ {
						...
						let returnValue = Result_TxOutUtxoLookupErrorZ(cType: nativeCallResult, instantiationContext: "Result_TxOutUtxoLookupErrorZ.swift::\(#function):\(#line)")
				...
				CResult_TxOutUtxoLookupErrorZ_clone(origPointer)
					...
					public func getValue() -> TxOut? {
						if self.cType?.result_ok == true {
							return TxOut(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_TxOutUtxoLookupErrorZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_TxOutUtxoLookupErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_TxOutUtxoLookupErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
