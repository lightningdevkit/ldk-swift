			...
			extension Bindings {
				...
				public class BlindedPayInfo: NativeTypeWrapper {
					...
					internal init(cType: LDKBlindedPayInfo, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				BlindedPayInfo_get_fee_base_msat(thisPtrPointer)
					...
					public func setFeeBaseMsat(val: UInt32) {
						...
						// native method call
						let nativeCallResult = 
						withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKBlindedPayInfo>) in
				...
				BlindedPayInfo_get_features(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = BlindedHopFeatures(cType: nativeCallResult, instantiationContext: "BlindedPayInfo.swift::\(#function):\(#line)", anchor: self).dangle(false)
				...
				BlindedPayInfo_set_features(thisPtrPointer, val.dynamicallyDangledClone().cType!)
					...
					public init(feeBaseMsatArg: UInt32, feeProportionalMillionthsArg: UInt32, cltvExpiryDeltaArg: UInt16, htlcMinimumMsatArg: UInt64, htlcMaximumMsatArg: UInt64, featuresArg: BlindedHopFeatures) {
						// native call variable prep
						

						// native method call
						let nativeCallResult = BlindedPayInfo_new(feeBaseMsatArg, feeProportionalMillionthsArg, cltvExpiryDeltaArg, htlcMinimumMsatArg, htlcMaximumMsatArg, featuresArg.dynamicallyDangledClone().cType!)
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "BlindedPayInfo.swift::\(#function):\(#line)")
				...
				BlindedPayInfo_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "BlindedPayInfo.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public class func read(ser: [UInt8]) -> Result_BlindedPayInfoDecodeErrorZ {
						...
						let serPrimitiveWrapper = u8slice(value: ser, instantiationContext: "BlindedPayInfo.swift::\(#function):\(#line)")
						...
						// return value (do some wrapping)
						let returnValue = Result_BlindedPayInfoDecodeErrorZ(cType: nativeCallResult, instantiationContext: "BlindedPayInfo.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing BlindedPayInfo \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing BlindedPayInfo \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
