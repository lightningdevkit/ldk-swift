...
extension Bindings {
	...
	public class SignedRawInvoice: NativeTypeWrapper {
		...
		internal init(
			cType: LDKSignedRawInvoice, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		internal func clone() -> SignedRawInvoice {
			...
			// return value (do some wrapping)
			let returnValue = SignedRawInvoice(
				cType: nativeCallResult, instantiationContext: "SignedRawInvoice.swift::\(#function):\(#line)")
		...
		public class func fromStr(s: String) -> Result_SignedRawInvoiceParseErrorZ {
			...
			let sPrimitiveWrapper = Str(value: s, instantiationContext: "SignedRawInvoice.swift::\(#function):\(#line)")
				.dangle()
		...
		internal func dangleRecursively() -> SignedRawInvoice {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing SignedRawInvoice \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing SignedRawInvoice \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
