			...
			extension Bindings {
				...
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "PhantomKeysManager.swift::\(#function):\(#line)")
					...
					public func spendSpendableOutputs(descriptors: [SpendableOutputDescriptor], outputs: [TxOut], changeDestinationScript: [UInt8], feerateSatPer1000Weight: UInt32, locktime: UInt32?) -> Result_TransactionNoneZ {
						...
						let locktimeOption = Option_u32Z(some: locktime, instantiationContext: "PhantomKeysManager.swift::\(#function):\(#line)").danglingClone()
				

						// native method call
						let nativeCallResult = 
						withUnsafePointer(to: self.cType!) { (thisArgPointer: UnsafePointer<LDKPhantomKeysManager>) in
				PhantomKeysManager_spend_spendable_outputs(thisArgPointer, descriptorsVector.cType!, outputsVector.cType!, changeDestinationScriptVector.cType!, feerateSatPer1000Weight, locktimeOption.cType!)
						}
