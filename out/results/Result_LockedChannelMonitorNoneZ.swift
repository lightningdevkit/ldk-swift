			...
			extension Bindings {
				...
				public class Result_LockedChannelMonitorNoneZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_LockedChannelMonitorNoneZ, instantiationContext: String, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					internal init(cType: LDKCResult_LockedChannelMonitorNoneZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: LockedChannelMonitor) -> Result_LockedChannelMonitorNoneZ {
						...
						let returnValue = Result_LockedChannelMonitorNoneZ(cType: nativeCallResult, instantiationContext: "Result_LockedChannelMonitorNoneZ.swift::\(#function):\(#line)")
					...
					*/
					...
					public func getValue() -> LockedChannelMonitor? {
						if self.cType?.result_ok == true {
							return LockedChannelMonitor(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_LockedChannelMonitorNoneZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_LockedChannelMonitorNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_LockedChannelMonitorNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
