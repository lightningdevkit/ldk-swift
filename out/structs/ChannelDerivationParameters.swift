			...
			extension Bindings {
				...
				public class ChannelDerivationParameters: NativeTypeWrapper {
					...
					internal init(cType: LDKChannelDerivationParameters, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				ChannelDerivationParameters_get_value_satoshis(thisPtrPointer)
					...
					public func setValueSatoshis(val: UInt64) {
						...
						// native method call
						let nativeCallResult = 
						withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKChannelDerivationParameters>) in
				...
				ChannelDerivationParameters_get_keys_id(thisPtrPointer)
					...
					public func setKeysId(val: [UInt8]) {
						...
						let valPrimitiveWrapper = ThirtyTwoBytes(value: val, instantiationContext: "ChannelDerivationParameters.swift::\(#function):\(#line)")
				...
				ChannelDerivationParameters_get_transaction_parameters(thisPtrPointer)
						...
						// return value (do some wrapping)
						let returnValue = ChannelTransactionParameters(cType: nativeCallResult, instantiationContext: "ChannelDerivationParameters.swift::\(#function):\(#line)", anchor: self).dangle(false)
					...
					public func setTransactionParameters(val: ChannelTransactionParameters) {
						...
						let nativeCallResult = 
						withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKChannelDerivationParameters>) in
				ChannelDerivationParameters_set_transaction_parameters(thisPtrPointer, val.dynamicallyDangledClone().cType!)
						}
					...
					public init(valueSatoshisArg: UInt64, keysIdArg: [UInt8], transactionParametersArg: ChannelTransactionParameters) {
						// native call variable prep
						
						let keysIdArgPrimitiveWrapper = ThirtyTwoBytes(value: keysIdArg, instantiationContext: "ChannelDerivationParameters.swift::\(#function):\(#line)")
				

						// native method call
						let nativeCallResult = ChannelDerivationParameters_new(valueSatoshisArg, keysIdArgPrimitiveWrapper.cType!, transactionParametersArg.dynamicallyDangledClone().cType!)
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "ChannelDerivationParameters.swift::\(#function):\(#line)")
				...
				ChannelDerivationParameters_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "ChannelDerivationParameters.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public class func read(ser: [UInt8]) -> Result_ChannelDerivationParametersDecodeErrorZ {
						...
						let serPrimitiveWrapper = u8slice(value: ser, instantiationContext: "ChannelDerivationParameters.swift::\(#function):\(#line)")
						...
						// return value (do some wrapping)
						let returnValue = Result_ChannelDerivationParametersDecodeErrorZ(cType: nativeCallResult, instantiationContext: "ChannelDerivationParameters.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing ChannelDerivationParameters \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing ChannelDerivationParameters \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
