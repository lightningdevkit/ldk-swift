...
extension Bindings {
	...
	public class Result_boolLightningErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_boolLightningErrorZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: Bool) -> Result_boolLightningErrorZ {
			...
			let returnValue = Result_boolLightningErrorZ(
				cType: nativeCallResult, instantiationContext: "Result_boolLightningErrorZ.swift::\(#function):\(#line)"
			)
		...
		public func getError() -> LightningError? {
			if self.cType?.result_ok == false {
				return LightningError(
					...
					instantiationContext: "Result_boolLightningErrorZ.swift::\(#function):\(#line)", anchor: self)
			}
			return nil
		}
		...
		internal func dangleRecursively() -> Result_boolLightningErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_boolLightningErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_boolLightningErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
