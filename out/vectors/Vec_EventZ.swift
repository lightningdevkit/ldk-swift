			...
			extension Bindings {
				...
				internal class Vec_EventZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_EventZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
...
Event(cType: currentCType, instantiationContext: "Vec_EventZ.swift::\(#function):\(#line)", anchor: self).dangle()
					...
					deinit {
						...
						if !self.dangling {
							...
							Bindings.print("Not freeing Vec_EventZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
