			...
			extension Bindings {
				...
				internal class Tuple_PaymentHashPaymentIdZ: NativeTypeWrapper {
					...
					internal init(cType: LDKC2Tuple_PaymentHashPaymentIdZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
				...
				super.init(conflictAvoidingVariableName: 0, instantiationContext: instantiationContext)
					...
					public func getA() -> [UInt8] {
						// return value (do some wrapping)
						let returnValue = ThirtyTwoBytes(cType: self.cType!.a, instantiationContext: "Tuple_PaymentHashPaymentIdZ.swift::\(#function):\(#line)", anchor: self).dangle().getValue()
					...
					public func getB() -> [UInt8] {
						// return value (do some wrapping)
						let returnValue = ThirtyTwoBytes(cType: self.cType!.b, instantiationContext: "Tuple_PaymentHashPaymentIdZ.swift::\(#function):\(#line)", anchor: self).dangle().getValue()
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Tuple_PaymentHashPaymentIdZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Tuple_PaymentHashPaymentIdZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
