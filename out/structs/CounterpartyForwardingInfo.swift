...
extension Bindings {
	...
	public class CounterpartyForwardingInfo: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCounterpartyForwardingInfo, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func setFeeBaseMsat(val: UInt32) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKCounterpartyForwardingInfo>) in
		...
		public init(feeBaseMsatArg: UInt32, feeProportionalMillionthsArg: UInt32, cltvExpiryDeltaArg: UInt16) {
			...
			// native method call
			let nativeCallResult = CounterpartyForwardingInfo_new(
				feeBaseMsatArg, feeProportionalMillionthsArg, cltvExpiryDeltaArg)
			...
			super
				.init(
					...
					instantiationContext: "CounterpartyForwardingInfo.swift::\(#function):\(#line)")
		...
		internal func clone() -> CounterpartyForwardingInfo {
			...
			let returnValue = CounterpartyForwardingInfo(
				cType: nativeCallResult, instantiationContext: "CounterpartyForwardingInfo.swift::\(#function):\(#line)"
			)
		...
		public func write() -> [UInt8] {
			...
			let returnValue = Vec_u8Z(
				...
				instantiationContext: "CounterpartyForwardingInfo.swift::\(#function):\(#line)", anchor: self
			)
		...
		public class func read(ser: [UInt8]) -> Result_CounterpartyForwardingInfoDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "CounterpartyForwardingInfo.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_CounterpartyForwardingInfoDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "CounterpartyForwardingInfo.swift::\(#function):\(#line)"
			)
		...
		internal func dangleRecursively() -> CounterpartyForwardingInfo {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing CounterpartyForwardingInfo \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing CounterpartyForwardingInfo \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
