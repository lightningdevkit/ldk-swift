...
extension Bindings {
	...
	public class ChannelHandshakeLimits: NativeTypeWrapper {
		...
		internal init(
			cType: LDKChannelHandshakeLimits, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public func setMinFundingSatoshis(val: UInt64) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKChannelHandshakeLimits>) in
		...
		public init(
			minFundingSatoshisArg: UInt64, maxFundingSatoshisArg: UInt64, maxHtlcMinimumMsatArg: UInt64,
			minMaxHtlcValueInFlightMsatArg: UInt64, maxChannelReserveSatoshisArg: UInt64,
			minMaxAcceptedHtlcsArg: UInt16, maxMinimumDepthArg: UInt32, trustOwnFunding0confArg: Bool,
			forceAnnouncedChannelPreferenceArg: Bool, theirToSelfDelayArg: UInt16
		) {
			// native call variable prep


			// native method call
			let nativeCallResult = ChannelHandshakeLimits_new(
				minFundingSatoshisArg, maxFundingSatoshisArg, maxHtlcMinimumMsatArg, minMaxHtlcValueInFlightMsatArg,
				maxChannelReserveSatoshisArg, minMaxAcceptedHtlcsArg, maxMinimumDepthArg, trustOwnFunding0confArg,
				forceAnnouncedChannelPreferenceArg, theirToSelfDelayArg)
			...
			super
				.init(
					...
					instantiationContext: "ChannelHandshakeLimits.swift::\(#function):\(#line)")
		...
		internal func clone() -> ChannelHandshakeLimits {
			...
			// return value (do some wrapping)
			let returnValue = ChannelHandshakeLimits(
				cType: nativeCallResult, instantiationContext: "ChannelHandshakeLimits.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> ChannelHandshakeLimits {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing ChannelHandshakeLimits \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing ChannelHandshakeLimits \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
