			...
			extension Bindings {
				...
				public class Result_ThirtyTwoBytesNoneZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_ThirtyTwoBytesNoneZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: [UInt8]) -> Result_ThirtyTwoBytesNoneZ {
						...
						let oPrimitiveWrapper = ThirtyTwoBytes(value: o, instantiationContext: "Result_ThirtyTwoBytesNoneZ.swift::\(#function):\(#line)")
						...
						let returnValue = Result_ThirtyTwoBytesNoneZ(cType: nativeCallResult, instantiationContext: "Result_ThirtyTwoBytesNoneZ.swift::\(#function):\(#line)")
					...
					*/
					...
					public func getValue() -> [UInt8]? {
						if self.cType?.result_ok == true {
							return ThirtyTwoBytes(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_ThirtyTwoBytesNoneZ.swift::\(#function):\(#line)", anchor: self).getValue()
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_ThirtyTwoBytesNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_ThirtyTwoBytesNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
