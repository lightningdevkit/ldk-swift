			...
			extension Bindings {
				...
				public class ScorerAccountingForInFlightHtlcs: NativeTypeWrapper {
					...
					internal init(cType: LDKScorerAccountingForInFlightHtlcs, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				ScorerAccountingForInFlightHtlcs_as_ScoreLookUp(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = NativelyImplementedScoreLookUp(cType: nativeCallResult, instantiationContext: "ScorerAccountingForInFlightHtlcs.swift::\(#function):\(#line)", anchor: self)
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing ScorerAccountingForInFlightHtlcs \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing ScorerAccountingForInFlightHtlcs \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
