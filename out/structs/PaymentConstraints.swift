...
extension Bindings {
	...
	public class PaymentConstraints: NativeTypeWrapper {
		...
		internal init(
			cType: LDKPaymentConstraints, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func setMaxCltvExpiry(val: UInt32) {
			...
			let nativeCallResult =
				withUnsafeMutablePointer(to: &self.cType!) {
					(thisPtrPointer: UnsafeMutablePointer<LDKPaymentConstraints>) in
		...
		public init(maxCltvExpiryArg: UInt32, htlcMinimumMsatArg: UInt64) {
			...
			super
				.init(
					...
					instantiationContext: "PaymentConstraints.swift::\(#function):\(#line)")
		...
		internal func clone() -> PaymentConstraints {
			...
			let returnValue = PaymentConstraints(
				cType: nativeCallResult, instantiationContext: "PaymentConstraints.swift::\(#function):\(#line)")
		...
		public class func read(ser: [UInt8]) -> Result_PaymentConstraintsDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "PaymentConstraints.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing PaymentConstraints \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing PaymentConstraints \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
