...
extension Bindings {
	...
	public class Result_FixedPenaltyScorerDecodeErrorZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_FixedPenaltyScorerDecodeErrorZ, instantiationContext: String, anchor: NativeTypeWrapper
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public class func initWithOk(o: FixedPenaltyScorer) -> Result_FixedPenaltyScorerDecodeErrorZ {
			...
			let returnValue = Result_FixedPenaltyScorerDecodeErrorZ(
				...
				instantiationContext: "Result_FixedPenaltyScorerDecodeErrorZ.swift::\(#function):\(#line)")
		...
		internal func clone() -> Result_FixedPenaltyScorerDecodeErrorZ {
			...
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) {
					(origPointer: UnsafePointer<LDKCResult_FixedPenaltyScorerDecodeErrorZ>) in
		...
		internal func dangleRecursively() -> Result_FixedPenaltyScorerDecodeErrorZ {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_FixedPenaltyScorerDecodeErrorZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Result_FixedPenaltyScorerDecodeErrorZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
