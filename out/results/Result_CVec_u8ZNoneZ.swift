...
extension Bindings {
	...
	public class Result_CVec_u8ZNoneZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKCResult_CVec_u8ZNoneZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		public class func initWithOk(o: [UInt8]) -> Result_CVec_u8ZNoneZ {
			...
			let oVector = Vec_u8Z(array: o, instantiationContext: "Result_CVec_u8ZNoneZ.swift::\(#function):\(#line)")
				.dangle()
			...
			let returnValue = Result_CVec_u8ZNoneZ(
				cType: nativeCallResult, instantiationContext: "Result_CVec_u8ZNoneZ.swift::\(#function):\(#line)")
		...
		public func getValue() -> [UInt8]? {
			if self.cType?.result_ok == true {
				return Vec_u8Z(
					...
					instantiationContext: "Result_CVec_u8ZNoneZ.swift::\(#function):\(#line)", anchor: self
				)
				.getValue()
			}
			return nil
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Result_CVec_u8ZNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing Result_CVec_u8ZNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
