...
extension Bindings {
	...
	public class AnnouncementSignatures: NativeTypeWrapper {
		...
		public init(
			...
			let bitcoinSignatureArgPrimitiveWrapper = ECDSASignature(
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKAnnouncementSignatures>) in
					AnnouncementSignatures_hash(oPointer)
				}
