...
extension Bindings {
	...
	public class RawInvoice: NativeTypeWrapper {
		...
		internal init(
			cType: LDKRawInvoice, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public func getData() -> RawDataPart {
			...
			let returnValue = RawDataPart(
				cType: nativeCallResult, instantiationContext: "RawInvoice.swift::\(#function):\(#line)", anchor: self
			)
			.dangle(false)
		...
		internal func clone() -> RawInvoice {
			...
			let returnValue = RawInvoice(
				cType: nativeCallResult, instantiationContext: "RawInvoice.swift::\(#function):\(#line)")
		...
		internal func dangleRecursively() -> RawInvoice {
			self.dangling = true
		...
		deinit {
			...
			if !self.dangling {
				...
				Bindings.print(
					"Not freeing RawInvoice \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
