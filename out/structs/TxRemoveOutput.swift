			...
			extension Bindings {
				...
				public class TxRemoveOutput: NativeTypeWrapper {
					...
					internal init(cType: LDKTxRemoveOutput, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						self.initialCFreeability = self.cType!.is_owned
				...
				TxRemoveOutput_get_channel_id(thisPtrPointer)
					...
					public func setChannelId(val: [UInt8]) {
						...
						let valPrimitiveWrapper = ThirtyTwoBytes(value: val, instantiationContext: "TxRemoveOutput.swift::\(#function):\(#line)")
				

						// native method call
						let nativeCallResult = 
						withUnsafeMutablePointer(to: &self.cType!) { (thisPtrPointer: UnsafeMutablePointer<LDKTxRemoveOutput>) in
				...
				TxRemoveOutput_set_serial_id(thisPtrPointer, val)
					...
					public init(channelIdArg: [UInt8], serialIdArg: UInt64) {
						...
						let channelIdArgPrimitiveWrapper = ThirtyTwoBytes(value: channelIdArg, instantiationContext: "TxRemoveOutput.swift::\(#function):\(#line)")
				...
				Self.instanceCounter += 1
				self.instanceNumber = Self.instanceCounter
				super.init(conflictAvoidingVariableName: 0, instantiationContext: "TxRemoveOutput.swift::\(#function):\(#line)")
				...
				TxRemoveOutput_write(objPointer)
						...
						// return value (do some wrapping)
						let returnValue = Vec_u8Z(cType: nativeCallResult, instantiationContext: "TxRemoveOutput.swift::\(#function):\(#line)", anchor: self).dangle(false).getValue()
					...
					public class func read(ser: [UInt8]) -> Result_TxRemoveOutputDecodeErrorZ {
						...
						let serPrimitiveWrapper = u8slice(value: ser, instantiationContext: "TxRemoveOutput.swift::\(#function):\(#line)")
						...
						// return value (do some wrapping)
						let returnValue = Result_TxRemoveOutputDecodeErrorZ(cType: nativeCallResult, instantiationContext: "TxRemoveOutput.swift::\(#function):\(#line)")
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing TxRemoveOutput \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing TxRemoveOutput \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
