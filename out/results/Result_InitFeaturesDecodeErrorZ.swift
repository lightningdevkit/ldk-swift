			...
			extension Bindings {
				...
				public class Result_InitFeaturesDecodeErrorZ: NativeTypeWrapper {
					...
					public class func initWithOk(o: InitFeatures) -> Result_InitFeaturesDecodeErrorZ {
						...
						let returnValue = Result_InitFeaturesDecodeErrorZ(cType: nativeCallResult, instantiationContext: "#{swift_class_name}::\(#function):\(#line)")
				...
				CResult_InitFeaturesDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> InitFeatures? {
						if self.cType?.result_ok == true {
							return InitFeatures(cType: self.cType!.contents.result.pointee, instantiationContext: "#{swift_class_name}::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
