			...
			extension Bindings {
				...
				internal class Vec_PaymentPreimageZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCVec_PaymentPreimageZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Vec_PaymentPreimageZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Vec_PaymentPreimageZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
