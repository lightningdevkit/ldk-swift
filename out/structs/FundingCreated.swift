...
extension Bindings {
	...
	public class FundingCreated: NativeTypeWrapper {
		...
		internal init(
			cType: LDKFundingCreated, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
			self.cType = cType
		...
		public init(
			...
			let signatureArgPrimitiveWrapper = ECDSASignature(
				value: signatureArg, instantiationContext: "FundingCreated.swift::\(#function):\(#line)")
			...
			super
				.init(
					conflictAvoidingVariableName: 0, instantiationContext: "FundingCreated.swift::\(#function):\(#line)"
				)
		...
		internal func clone() -> FundingCreated {
			...
			// return value (do some wrapping)
			let returnValue = FundingCreated(
				cType: nativeCallResult, instantiationContext: "FundingCreated.swift::\(#function):\(#line)")
		...
		public func hash() -> UInt64 {
			...
			// native method call
			let nativeCallResult =
				withUnsafePointer(to: self.cType!) { (oPointer: UnsafePointer<LDKFundingCreated>) in
					FundingCreated_hash(oPointer)
				}
		...
		public class func read(ser: [UInt8]) -> Result_FundingCreatedDecodeErrorZ {
			...
			let serPrimitiveWrapper = u8slice(
				value: ser, instantiationContext: "FundingCreated.swift::\(#function):\(#line)")
			...
			// return value (do some wrapping)
			let returnValue = Result_FundingCreatedDecodeErrorZ(
				cType: nativeCallResult, instantiationContext: "FundingCreated.swift::\(#function):\(#line)")
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing FundingCreated \(self.instanceNumber). (Origin: \(self.instantiationContext))")
				}

				self.free()
			} else if Self.enableDeinitLogging {
				Bindings.print(
					"Not freeing FundingCreated \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
