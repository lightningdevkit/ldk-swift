			...
			extension Bindings {
				...
				public class Result_PublicKeyNoneZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_PublicKeyNoneZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: [UInt8]) -> Result_PublicKeyNoneZ {
						...
						let returnValue = Result_PublicKeyNoneZ(cType: nativeCallResult, instantiationContext: "Result_PublicKeyNoneZ.swift::\(#function):\(#line)")
					...
					*/
					...
					public func getValue() -> [UInt8]? {
						if self.cType?.result_ok == true {
							return PublicKey(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_PublicKeyNoneZ.swift::\(#function):\(#line)", anchor: self).getValue()
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_PublicKeyNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_PublicKeyNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
