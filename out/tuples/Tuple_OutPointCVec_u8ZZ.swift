...
extension Bindings {
	...
	internal class Tuple_OutPointCVec_u8ZZ: NativeTypeWrapper {
		...
		internal init(
			cType: LDKC2Tuple_OutPointCVec_u8ZZ, instantiationContext: String, anchor: NativeTypeWrapper,
			dangle: Bool = false
		) {
			Self.instanceCounter += 1
			self.instanceNumber = Self.instanceCounter
		...
		internal func clone() -> Tuple_OutPointCVec_u8ZZ {
			...
			let returnValue = Tuple_OutPointCVec_u8ZZ(
				cType: nativeCallResult, instantiationContext: "Tuple_OutPointCVec_u8ZZ.swift::\(#function):\(#line)")
		...
		public init(a: OutPoint, b: [UInt8], instantiationContext: String) {
			...
			let bVector = Vec_u8Z(
				array: b, instantiationContext: "Tuple_OutPointCVec_u8ZZ.swift::\(#function):\(#line)"
			)
			.dangle()
		...
		public func getA() -> OutPoint {
			...
			let returnValue = OutPoint(
				cType: self.cType!.a, instantiationContext: "Tuple_OutPointCVec_u8ZZ.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle()
		...
		public func getB() -> [UInt8] {
			...
			let returnValue = Vec_u8Z(
				cType: self.cType!.b, instantiationContext: "Tuple_OutPointCVec_u8ZZ.swift::\(#function):\(#line)",
				anchor: self
			)
			.dangle().getValue()
		...
		deinit {
			...
			if !self.dangling {
				if Self.enableDeinitLogging {
					Bindings.print(
						"Freeing Tuple_OutPointCVec_u8ZZ \(self.instanceNumber). (Origin: \(self.instantiationContext))"
					)
				}
				...
				Bindings.print(
					"Not freeing Tuple_OutPointCVec_u8ZZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))"
				)
			}
		}
