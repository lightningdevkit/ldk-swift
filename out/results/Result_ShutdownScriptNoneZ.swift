			...
			extension Bindings {
				...
				public class Result_ShutdownScriptNoneZ: NativeTypeWrapper {
					...
					internal init(cType: LDKCResult_ShutdownScriptNoneZ, instantiationContext: String, anchor: NativeTypeWrapper, dangle: Bool = false) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
					...
					public class func initWithOk(o: ShutdownScript) -> Result_ShutdownScriptNoneZ {
						...
						let returnValue = Result_ShutdownScriptNoneZ(cType: nativeCallResult, instantiationContext: "Result_ShutdownScriptNoneZ.swift::\(#function):\(#line)")
					...
					*/
					...
					public func getValue() -> ShutdownScript? {
						if self.cType?.result_ok == true {
							return ShutdownScript(cType: self.cType!.contents.result.pointee, instantiationContext: "Result_ShutdownScriptNoneZ.swift::\(#function):\(#line)", anchor: self)
						}
						return nil
					}
					...
					deinit {
						...
						if !self.dangling {
							if Self.enableDeinitLogging {
								Bindings.print("Freeing Result_ShutdownScriptNoneZ \(self.instanceNumber). (Origin: \(self.instantiationContext))")
							}
							
							self.free()
						} else if Self.enableDeinitLogging {
							Bindings.print("Not freeing Result_ShutdownScriptNoneZ \(self.instanceNumber) due to dangle. (Origin: \(self.instantiationContext))")
						}
					}
